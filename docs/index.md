# KRYON - Autonomous Cybersecurity Intelligence System

A lightweight, ergonomic framework for building autonomous cybersecurity AI agents.

## Key Features

- **300+ AI Models**: Support for OpenAI, Anthropic, DeepSeek, Ollama, and more
- **Built-in Security Tools**: Ready-to-use tools for reconnaissance, exploitation, and privilege escalation
- **Battle-tested**: Proven in HackTheBox CTFs, bug bounties, and real-world security case studies
- **Agent-based Architecture**: Modular framework design to build specialized agents for different security tasks
- **Guardrails Protection**: Built-in defenses against prompt injection and dangerous command execution
- **Research-oriented**: Research foundation to democratize cybersecurity AI for the community

## Quick Start

```bash
# Install KRYON framework
pip install kryon

# Launch KRYON
kryon
```

## Documentation

### Getting Started

- [Installation Guide](getting-started/installation.md)
- [Quick Start](getting-started/quickstart.md)
- [Configuration](getting-started/configuration.md)

### Guides

- [Agents](guides/agents.md) - Building and using agents
- [Tools](guides/tools.md) - Available security tools
- [Autonomy](guides/autonomy.md) - Autonomous operation system
- [RAG](guides/rag.md) - Knowledge retrieval system
- [Guardrails](guides/guardrails.md) - Safety mechanisms
- [Models](guides/models.md) - LLM configuration
- [Tracing](guides/tracing.md) - Operation tracing
- [MCP](guides/mcp.md) - Model Context Protocol
- [Multi-Agent](guides/multi_agent.md) - Multi-agent coordination
- [Handoffs](guides/handoffs.md) - Agent handoffs
- [Security Tools](guides/security-tools.md) - Security tools inventory

### Tutorials

- [TryHackMe Guide](tutorials/tryhackme.md) - TryHackMe integration
- [Ollama on Kali](tutorials/ollama-kali.md) - Local LLM setup
- [CTF Guide](tutorials/ctf-guide.md) - CTF operations guide
- [Examples](tutorials/examples.md) - Usage examples

### API Reference

- [API Reference](api-reference/index.md) - Complete API documentation
- [Voice API](api-reference/voice/quickstart.md) - Voice interface

### Providers

- [Azure](providers/azure.md)
- [Ollama](providers/ollama.md)
- [OpenRouter](providers/openrouter.md)

### Enterprise

- [Admin Guide](admin-guide/index.md) - System administration
- [API Guide](api-guide/index.md) - REST API reference
- [Architecture](architecture/overview.md) - System architecture
- [Deployment](deployment/docker.md) - Production deployment

### Other

- [FAQ](faq.md) - Frequently asked questions
- [Troubleshooting](troubleshooting.md) - Common issues and solutions

## Supported Model Providers

* **Anthropic**: Claude 3.7, Claude 3.5, Claude 3, Claude 3 Opus
* **OpenAI**: O1, O1 Mini, O3 Mini, GPT-4o, GPT-4.5 Preview
* **DeepSeek**: DeepSeek V3, DeepSeek R1
* **Ollama**: Qwen2.5 72B, Qwen2.5 14B, and many more
