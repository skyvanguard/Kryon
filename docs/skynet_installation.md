...
# SKYNET requieres python 3.12, lets install it (CAI for kali in Android)
...
SKYNET leverages the `.env` file to load configuration at launch. To facilitate the setup, the repo provides an exemplary `.env.example` file provides a template for configuring CAI's setup and your LLM API keys to work with desired LLM models.
...
```
⚠️ SKYNET does NOT provide API keys for any model by default. 
