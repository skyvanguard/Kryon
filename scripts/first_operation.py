...
def run_reconnaissance(target_ip: str, deep: bool = False):
    ...
    try:
        results = full_auto_enumeration(
            target_ip=target_ip, deep_scan=deep, timeout=1800 if deep else 600
        )
        ...
        output_file = (
            f"/workspace/results/operations/recon_{target_ip.replace('.', '_')}_{timestamp}.json"
        )
...
Ejemplos:
  ...
  python3 first_operation.py --target 10.10.10.5 --deep
    ...
    parser.add_argument(
        "--deep", action="store_true", help="Enable deep scan (slower but more thorough)"
    )
