...
def add_to_knowledge_base(items: List[Dict], source: str):
    ...
    for i, item in enumerate(items, 1):
        try:
            rag.add_knowledge(
                content=item["content"], source=source, metadata=item.get("metadata", {})
            )
            added += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exploits", type=int, default=500, help="Number of exploits to scrape from Exploit-DB"
    )
    ...
    parser.add_argument(
        "--github-count", type=int, default=50, help="Number of GitHub repositories"
    )
