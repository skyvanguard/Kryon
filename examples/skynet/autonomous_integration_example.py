...
def scenario_1_basic_ctf_with_learning():
    ...
    recommendations = get_learned_recommendations(
        target_profile=target1_profile,
        top_n=3,
        min_confidence=0.3
    )
    ...
    recommendations2 = get_learned_recommendations(
        target_profile=target2_profile,
        top_n=3,
        min_confidence=0.3
    )
...
def scenario_3_context_analysis_and_adaptation():
    ...
    # Extract credentials
    credentials = analyzer.extract_credentials_from_text(
        text=captured_logs,
        context="server_logs"
    )
    ...
    analysis_result = analyzer.autonomous_context_analysis(
        ...
        operation_objective="gain_initial_access"
    )
