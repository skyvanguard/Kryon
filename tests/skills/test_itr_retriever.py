...
def test_select_with_itr_always_include_is_guaranteed_even_if_not_indexed():
    """Contract: an ALWAYS_INCLUDE name MUST be in the result even when the
    embedded index lags (a newly-pinned tool not yet re-embedded). The old code
    dropped it silently while still returning non-None, so the caller never fell
    back to the static selector — a critical tool (execute_planner_directive) just
    vanished. The caller resolves names against the registry, so a not-indexed
    name still binds if it exists."""
    index = _gradient_index(10)
    embedder = _StaticEmbedder([1.0, 0.0])
    selected = select_with_itr(
        "q",
    ...
    assert selected is not None
    assert "some_tool_not_in_index" in selected  # guaranteed now, not dropped
