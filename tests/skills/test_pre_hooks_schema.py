...
def test_pre_hook_spec_hashable_with_list_valued_args() -> None:
    """A list-valued arg (a documented, schema-supported shape) must stay hashable —
    tuple(sorted(args.items())) raised 'unhashable type: list' before the JSON fix."""
    hook = parse_pre_hooks([{"tool": "run_command", "args": {"hosts": ["{ctx.host}", "foo"]}}])[0]
    hash(hook)  # must not raise
    # And put it in a set (the actual dedupe use case).
    assert len({hook, hook}) == 1
