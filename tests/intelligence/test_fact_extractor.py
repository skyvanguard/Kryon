...
def test_render_truncates_long_noisy_lists() -> None:
    """Noisy fields (paths) truncate at max_per_field with a "(+N more)" marker."""
    facts = ExtractedFacts(paths=tuple(f"/p{i:02d}" for i in range(30)))
    rendered = facts.render_for_prompt(max_per_field=10)
    assert "(+20 more)" in rendered


def test_render_does_not_truncate_high_value_fields() -> None:
    """A single truncated hash/cred/user is a lost foothold — high-value fields get
    a much larger cap and must NOT be cut at max_per_field."""
    facts = ExtractedFacts(
        users=tuple(f"user{i:02d}" for i in range(50)),
        hashes=tuple(f"$6$hash{i:03d}" for i in range(50)),
    )
    rendered = facts.render_for_prompt(max_per_field=10)
    assert "user49" in rendered
    assert "$6$hash049" in rendered
    assert "(+40 more)" not in rendered  # not truncated at 10
...
def test_web_param_path_drives_planner_to_sqlmap() -> None:
    ...
    rec = plan_next_action(
        facts,
        ["service_scan --top-ports 2000", "# loot_web [LOOT] ran", "searchsploit fingerprint ran"],
        "audita http://10.10.10.10",
    )
...
def test_nuclei_extracts_cve_and_filters_info() -> None:
    sample = "[CVE-2021-44228] [http] [critical] http://t/api\n[tech-detect] [http] [info] http://t/\n"
