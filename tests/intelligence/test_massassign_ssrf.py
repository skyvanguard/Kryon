...
def test_mass_assignment_adds_privilege_fields():
    ...
    # Juice Shop coverage: /api/Users endpoint + passwordRepeat confirmation field
    assert "/api/Users" in rec.args
    assert "passwordRepeat" in rec.args


def test_mass_assignment_closes_proof_of_exploit_link():
    # the rule must not stop at "registered"; it logs in and PROVES the elevation
    # against an admin-only endpoint (the eslabón that turns detection into proof).
    rec = _rule_mass_assignment_register(_WEB, [], "")
    assert rec is not None
    assert "MASSASSIGN-ADMIN" in rec.args  # proof marker
    assert "Authorization: Bearer" in rec.args  # JWT reuse
    assert "/rest/user/login" in rec.args  # canonical login (not sed-derived from register path)
    # regression guards learned from the cred-reuse rule (only caught live):
    assert "%{http_code}" in rec.args and "%{{" not in rec.args
    assert "<(!doctype|html|!--)" in rec.args  # SPA/HTML fallback filtered


def test_mass_assignment_abstains_with_creds_or_no_surface():
    assert (
        _rule_mass_assignment_register(
            ExtractedFacts(services=((80, "http"),), creds=(("a", "b"),), paths=("/x",)), [], ""
        )
        is None
    )
