...
def test_cmdi_with_inline_comment_still_detected():
    # T4-A5: a '#' anywhere used to suppress the finding; only a real comment line should.
    assert "sast-py-cmdi" in _rules("os.system(user_input)  # run the thing", "a.py")


def test_cmdi_on_real_comment_line_suppressed():
    assert "sast-py-cmdi" not in _rules("# os.system(user_input) is dangerous", "a.py")


def test_eval_with_inline_comment_still_detected():
    assert "sast-py-eval" in _rules("eval(user_data)  # trusted input", "a.py")


def test_js_eval_with_url_still_detected():
    # The '//' of http:// must not suppress the eval finding.
    assert "sast-js-eval" in _rules('eval(fetch("http://api/x"))', "a.js")


def test_js_eval_on_real_comment_line_suppressed():
    assert "sast-js-eval" not in _rules("// eval(x) example", "a.js")
