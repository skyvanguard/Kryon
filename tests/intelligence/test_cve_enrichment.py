...
@pytest.mark.asyncio
async def test_check_exploit_db_returns_refs_regression(enricher, monkeypatch):
    """Regression: check_exploit_db imported a non-existent `search_exploitdb`
    → ImportError swallowed → always []. Now the function exists and the
    cross-reference actually returns urls."""
    import kryon.knowledge.exploitdb_scraper as eds

    monkeypatch.setattr(
        eds,
        "search_exploitdb",
        lambda cve_id: [{"url": "https://www.exploit-db.com/exploits/42"}],
    )
    refs = await enricher.check_exploit_db("CVE-2021-1234")
    assert refs == ["https://www.exploit-db.com/exploits/42"]
