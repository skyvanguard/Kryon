...
# ---------------------------------------------------------------------------
# OPSEC — ffuf must route through the anonymizing proxy (it bypasses torsocks)
# ---------------------------------------------------------------------------


def test_dir_cmd_injects_socks_proxy():
    cmd = build_ffuf_dir_cmd("http://t", "/wl", proxy="socks5://127.0.0.1:9050")
    assert "-x socks5://127.0.0.1:9050" in cmd


def test_vhost_cmd_injects_socks_proxy():
    cmd = build_ffuf_vhost_cmd("http://t", "creative.thm", "/wl", proxy="socks5://127.0.0.1:9050")
    assert "-x socks5://127.0.0.1:9050" in cmd


def test_cmd_without_proxy_has_no_x_flag():
    assert " -x " not in build_ffuf_dir_cmd("http://t", "/wl")


def test_run_web_enum_defaults_to_anon_proxy_env(monkeypatch):
    # OPSEC regression: with KRYON_ANON_PROXY set, every ffuf command run_web_enum
    # issues must carry -x, or the Go binary leaks the real IP (the live ffuf leak).
    ...
    run_web_enum("http://creative.thm", runner=fake_runner)
    assert calls, "expected at least one ffuf invocation"
    assert all("-x socks5://127.0.0.1:9050" in c for c in calls)


def test_run_web_enum_no_proxy_when_env_unset(monkeypatch):
    ...
    run_web_enum("http://creative.thm", runner=fake_runner)
    assert all(" -x " not in c for c in calls)


def test_falls_back_when_default_wordlist_missing(monkeypatch, tmp_path):
    # T3-A4: seclists is absent on the non-kali image, so ffuf ran with a missing -w
    # and produced 0 dirs silently. Fall back to the installed dirb/common.txt.
    from kryon.intelligence import web_enum

    fallback = tmp_path / "common.txt"
    fallback.write_text("admin\nlogin\n")
    monkeypatch.setattr(web_enum, "DEFAULT_DIR_WORDLIST", "/nonexistent/seclists/common.txt")
    monkeypatch.setattr(web_enum, "FALLBACK_DIR_WORDLIST", str(fallback))
    monkeypatch.delenv("KRYON_WEBENUM_DEEP", raising=False)
    cmds: list[str] = []
    ...
    web_enum.run_web_enum("http://10.0.0.5", runner=fake_runner)
    assert any(str(fallback) in c for c in cmds)
    assert not any("/nonexistent/seclists" in c for c in cmds)
