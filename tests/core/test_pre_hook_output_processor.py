...
# ---------------------------------------------------------------------------
# F210 — nuclei severity floor (info noise suppression)
# ---------------------------------------------------------------------------


def test_nuclei_info_dropped_by_default():
    """``info`` templates are tech-detection / header noise — they must
    not reach the model as findings by default."""
    out = summarize_pre_hook_output("nuclei_pre_scan", _NUCLEI_SAMPLE)
    # The [info] missing-headers line is gone.
    assert "missing-headers" not in out
    assert "[info]" not in out
    # low/high/critical survive.
    assert "exposed-files" in out  # [low]
    assert "xss-reflected" in out  # [high]
    assert "sql-injection" in out  # [critical]


def test_nuclei_floor_info_keeps_everything(monkeypatch):
    """Operators doing pure recon opt back into info via env."""
    monkeypatch.setenv("KRYON_NUCLEI_SEVERITY_FLOOR", "info")
    out = summarize_pre_hook_output("nuclei_pre_scan", _NUCLEI_SAMPLE)
    assert "missing-headers" in out
    assert "[info]" in out


def test_nuclei_floor_medium_drops_low_and_info(monkeypatch):
    """A stricter offensive sweep raises the floor to medium."""
    monkeypatch.setenv("KRYON_NUCLEI_SEVERITY_FLOOR", "medium")
    out = summarize_pre_hook_output("nuclei_pre_scan", _NUCLEI_SAMPLE)
    assert "missing-headers" not in out  # info dropped
    assert "exposed-files" not in out  # low dropped
    assert "xss-reflected" in out  # high kept
    assert "sql-injection" in out  # critical kept


def test_nuclei_invalid_floor_falls_back_to_default(monkeypatch):
    monkeypatch.setenv("KRYON_NUCLEI_SEVERITY_FLOOR", "bogus")
    out = summarize_pre_hook_output("nuclei_pre_scan", _NUCLEI_SAMPLE)
    # Default = low → info dropped, low+ kept.
    assert "[info]" not in out
    assert "exposed-files" in out
...
def test_imperative_suffix_short():
    ...
    os.environ.pop("KRYON_CAPABLE_MODEL", None)
    assert len(imperative_findings_suffix()) <= 800


def test_imperative_suffix_capable_allows_chaining(monkeypatch):
    # Regression (G2): the perverse coupling was "better signal → tighter cage".
    # A capable model must NOT be forbidden to re-invoke; it chains from the leads.
    monkeypatch.setenv("KRYON_CAPABLE_MODEL", "true")
    suf = imperative_findings_suffix(evidence_present=True).lower()
    assert "no re-invoc" not in suf
    assert "encaden" in suf or "re-invocá" in suf.replace("́", "") or "re-invoc" in suf


def test_imperative_suffix_4b_still_forbids_rerun(monkeypatch):
    monkeypatch.delenv("KRYON_CAPABLE_MODEL", raising=False)
    suf = imperative_findings_suffix(evidence_present=True).lower()
    assert "no re-invoc" in suf
