...
from kryon.sdk.agents._run_impl import RunImpl
...
async def test_handoffs_parsed_correctly():
    ...
    handoff_agent = await handoff.handoff.on_invoke_handoff(RunContextWrapper(None), handoff.tool_call.arguments)
...
async def test_file_search_tool_call_parsed_correctly():
    ...
    assert any(isinstance(item, ToolCallItem) and item.raw_item is file_search_call for item in result.new_items)
    assert not result.functions
    assert not result.handoffs
...
async def test_function_web_search_tool_call_parsed_correctly():
    ...
    assert any(isinstance(item, ToolCallItem) and item.raw_item is web_search_call for item in result.new_items)
    assert not result.functions
    assert not result.handoffs
...
async def test_reasoning_item_parsed_correctly():
    ...
    reasoning = ResponseReasoningItem(id="r1", type="reasoning", summary=[Summary(text="why", type="summary_text")])
    response = ModelResponse(
        output=[reasoning],
        usage=Usage(),
        referenceable_id=None,
    ...
    assert any(isinstance(item, ReasoningItem) and item.raw_item is reasoning for item in result.new_items)
...
async def test_computer_tool_call_with_computer_tool_parsed_correctly():
    ...
    assert any(isinstance(item, ToolCallItem) and item.raw_item is computer_call for item in result.new_items)
...
async def test_tool_and_handoff_parsed_correctly():
    agent_1 = Agent(name="test_1")
    agent_2 = Agent(name="test_2")
    agent_3 = Agent(name="test_3", tools=[get_function_tool(name="test")], handoffs=[agent_1, agent_2])
    response = ModelResponse(
        output=[
            get_text_message("Hello, world!"),
