...
class TestAddToMessageHistoryGuardrails:
    ...
    def test_prose_plan_assistant_is_not_appended(self, monkeypatch) -> None:
        monkeypatch.delenv("KRYON_CAPABLE_MODEL", raising=False)
        model = _make_model()
        model.message_history = []
        assert len(self._POISON) >= 200  # precondition: meets min length
        model.add_to_message_history({"role": "assistant", "content": self._POISON})
        assert len(model.message_history) == 0

    def test_capable_model_keeps_prose_plan(self, monkeypatch) -> None:
        # G11: a capable model emits native tool_calls and doesn't imitate the
        # pattern — dropping its text erases its own reasoning between turns.
        monkeypatch.setenv("KRYON_CAPABLE_MODEL", "true")
        model = _make_model()
        model.message_history = []
        model.add_to_message_history({"role": "assistant", "content": self._POISON})
        assert len(model.message_history) == 1  # kept, not dropped
...
class TestDirectiveToolChoiceForcing:
    ...
    def test_returns_false_when_env_explicitly_false(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from kryon.sdk.agents.models.openai_chatcompletions import (
            _should_force_directive_tool_choice,
        )
    ...
    def test_returns_true_when_env_on_and_probe_true(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_false_when_env_on_but_probe_false(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_false_when_probe_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_accepts_various_truthy_env_values(self, monkeypatch: pytest.MonkeyPatch) -> None:
