...
def test_jamon_cve_dropped_for_express_target(monkeypatch):
...
def test_capable_model_keeps_gated_finding_as_needs_verification(monkeypatch):
    # Regression (G17/G18/G19): under KRYON_CAPABLE_MODEL an applicability gate must
    # DEGRADE a finding to low-confidence needs_verification, not silently drop it —
    # a capable model finds real CVEs the heuristic gates wrongly reject.
    monkeypatch.setenv("KRYON_CAPABLE_MODEL", "true")
    findings = _parse_agent_findings(_JAMON_FP_BLOCK, target_host="juice_shop")
    by_id = {f.rule_id: f for f in findings}
    assert "CVE-2013-6235" in by_id  # kept, not dropped
    jamon = by_id["CVE-2013-6235"]
    assert jamon.needs_verification is True
    assert jamon.confidence <= 0.3  # gate-flagged → low confidence
