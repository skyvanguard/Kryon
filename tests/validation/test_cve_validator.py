...
# ---------------------------------------------------------------------------
# _default_cache_path — must be stable across cwd (bug fix: was cwd-relative)
# ---------------------------------------------------------------------------


def test_default_cache_path_is_under_home(monkeypatch):
    monkeypatch.delenv("KRYON_CVE_CACHE_PATH", raising=False)
    assert _default_cache_path() == Path.home() / ".kryon" / "nvd_cache" / "cves.txt"


def test_default_cache_path_honors_env_override(monkeypatch):
    monkeypatch.setenv("KRYON_CVE_CACHE_PATH", str(Path("custom") / "cves.txt"))
    assert _default_cache_path() == Path("custom") / "cves.txt"
