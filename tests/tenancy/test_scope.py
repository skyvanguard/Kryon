...
def test_tenant_slug_normalises():
    assert TenantContext("Britimp Bank S.A.").slug == "Britimp_Bank_S_A"
...
def test_target_matches_glob():
    policy = ScopePolicy(tenant_id="t", allowed_targets=("*.britimp.com.py",))
    ok, _ = is_target_in_scope("cashbox.britimp.com.py", policy)
    assert ok is True


def test_target_matches_cidr():
    policy = ScopePolicy(tenant_id="t", allowed_targets=("172.18.200.0/24",))
    ok, reason = is_target_in_scope("172.18.200.50", policy)
    assert ok is True
...
def test_load_missing_returns_none(tmp_path):
    assert load_scope_policy("britimp", path=tmp_path / "no.json") is None
...
def test_load_parses_tenant_block(tmp_path):
    ...
    _write_policy(
        p,
        {
            "tenants": {
                "britimp": {
                    "allowed_targets": ["www.britimp.com.py"],
    ...
    pol = load_scope_policy("britimp", path=p)
    assert pol is not None
    assert "www.britimp.com.py" in pol.allowed_targets
...
def test_load_malformed_returns_none(tmp_path):
    ...
    assert load_scope_policy("britimp", path=p) is None
...
def test_namespaced_engagement_id_with_tenant():
    tc = TenantContext("britimp")
    assert namespaced_engagement_id("eng-1", tenant=tc) == "britimp/eng-1"
...
def test_namespaced_path_with_tenant():
    ...
    out = namespaced_path(base, tenant=TenantContext("britimp"))
    assert out == base / "britimp"
