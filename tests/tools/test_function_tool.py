...
def test_func_schema_is_strict():
    ...
    assert "additionalProperties" in tool.params_json_schema and not tool.params_json_schema["additionalProperties"]
...
async def test_manual_function_tool_creation_works():
    ...
    result = await tool_not_strict.on_invoke_tool(RunContextWrapper(None), '{"data": "hello", "bar": "baz"}')
    assert result == "hello_done"
