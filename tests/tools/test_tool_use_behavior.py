...
def _make_function_tool_result(agent: Agent, output: str, tool_name: str | None = None) -> FunctionToolResult:
...
async def test_custom_tool_use_behavior_sync() -> None:
    ...
    def behavior(context: RunContextWrapper, results: list[FunctionToolResult]) -> ToolsToFinalOutputResult:
...
async def test_custom_tool_use_behavior_async() -> None:
    ...
    async def behavior(context: RunContextWrapper, results: list[FunctionToolResult]) -> ToolsToFinalOutputResult:
        assert len(results) == 3
