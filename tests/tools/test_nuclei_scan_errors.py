...
@pytest.mark.asyncio
async def test_nuclei_scan_retries_bad_template_path_with_defaults(monkeypatch):
    """#3 fix — a legacy template dir (``vulnerabilities/`` → nuclei v3 nests
    it under ``http/``) fails with 'could not find template'. The tool must
    auto-retry ONCE with the default set instead of surfacing an error and
    making the agent loop."""
    calls: list[str] = []

    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if "-t vulnerabilities/" in cmd:
            return "[FTL] Could not run nuclei: could not find template 'vulnerabilities/'\n"
        # Retry without templates → default scan finds something.
        return (
            "[INF] Templates loaded for scan: 9000\n"
            "[HIGH] exposed-panel detected at https://nuclei-retry-test-1.example.invalid/\n"
        )
    ...
    result = await _invoke(
        nuclei_scan,
        {"target": "https://nuclei-retry-test-1.example.invalid", "templates": "vulnerabilities/"},
    )

    text = str(result)
    assert "[KRYON_TOOL_ERROR]" not in text  # recovered, not surfaced as error
    assert "se reintentó con el" in text
    assert "exposed-panel" in text
    assert len(calls) == 2  # first (with -t) failed, retry (without) succeeded
    assert "-t vulnerabilities/" in calls[0]
