...
def test_f84_complete_5_protocols_10_checks() -> None:
    ...
    for prefix, expected in expected_per_protocol.items():
        for check_id in expected:
            assert check_id in ids, f"F84 protocol {prefix!r} missing check {check_id!r}"

    ot_check_count = sum(1 for cid in ids if any(cid.startswith(p) for p in expected_per_protocol))
    assert ot_check_count == 10, f"F84 should have exactly 10 OT checks, found {ot_check_count}"
