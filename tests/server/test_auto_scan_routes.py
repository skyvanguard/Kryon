...
class TestAutoScanEndpoints:
    ...
    def test_start_auto_scan_missing_targets(self, client):
        ...
        resp = client.post("/api/scans/auto", json={"profile": "quick"})
    ...
    def test_get_auto_scan_status(self, client):
        ...
        # Then check status
        resp = client.get(f"/api/scans/auto/{scan_id}")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_get_auto_scan_status_not_found(self, client):
        ...
        resp = client.get("/api/scans/auto/nonexistent")
    ...
    def test_get_auto_scan_findings(self, client):
        ...
        resp = client.get(f"/api/scans/auto/{scan_id}/findings")
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)

    def test_get_auto_scan_findings_not_found(self, client):
        resp = client.get("/api/scans/auto/nonexistent/findings")
        assert resp.status_code == 404
    ...
    def test_cancel_auto_scan(self, client):
        ...
        resp = client.delete(f"/api/scans/auto/{scan_id}")
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "cancelled"

    def test_cancel_nonexistent_scan(self, client):
        resp = client.delete("/api/scans/auto/nonexistent")
        assert resp.status_code == 404
...
class TestAutoScanSSE:
    def test_stream_endpoint_exists(self, client):
        ...
        resp = client.get(f"/api/scans/auto/{scan_id}/stream")
        assert resp.status_code == 200
        assert "text/event-stream" in resp.headers.get("content-type", "")

    def test_stream_not_found(self, client):
        resp = client.get("/api/scans/auto/nonexistent/stream")
        assert resp.status_code == 404
