...
def fetch_normalized_spans(keep_span_id: bool = False, keep_trace_id: bool = False) -> list[dict[str, Any]]:
    ...
    # First pass: add all spans to nodes
    span_parent_map: list[tuple[str, str | None, dict[str, Any]]] = []
    for span_obj in fetch_ordered_spans():
        ...
        span.pop("trace_id", None)  # Remove trace_id if present
        ...
        span_parent_map.append((span_obj.trace_id, parent_id, span))

    # Second pass: build hierarchy
    for trace_id, parent_id, span in span_parent_map:
        nodes[(trace_id, parent_id)].setdefault("children", []).append(span)

    # Third pass: sort children by id for deterministic output (async spans may finish in any order)
    def sort_children(node: dict[str, Any]) -> None:
        if "children" in node:
            node["children"].sort(key=lambda x: x.get("id", ""))
