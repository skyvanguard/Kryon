...
# --------------------------------------------------------------------------- #
# get_model_input_tokens — KRYON_MODEL_MAX_TOKENS override. An aliased local   #
# model (qwen-unc served at 262k) fell through to the hard 32k floor, so a     #
# 38k-token turn read 121% in the footer and tripped spurious compaction. The  #
# override must win so the ctx% is honest.                                     #
# --------------------------------------------------------------------------- #


def test_model_max_tokens_override_wins_for_unknown_alias(monkeypatch):
    monkeypatch.setenv("KRYON_MODEL_MAX_TOKENS", "262144")
    ct = _reload_cost_tracker()
    # qwen-unc is not in CONTEXT_WINDOW and matches no prefix → would be 32k.
    assert ct.get_model_input_tokens("qwen-unc") == 262144


def test_no_override_keeps_legacy_32k_floor(monkeypatch):
    monkeypatch.delenv("KRYON_MODEL_MAX_TOKENS", raising=False)
    ct = _reload_cost_tracker()
    # Unknown alias falls through to the 32k floor (unchanged, banca-safe: never
    # over-estimate a swapped-in small model's window).
    assert ct.get_model_input_tokens("qwen-unc") == 32_000


def test_override_ignored_when_malformed(monkeypatch):
    monkeypatch.setenv("KRYON_MODEL_MAX_TOKENS", "not-a-number")
    ct = _reload_cost_tracker()
    assert ct.get_model_input_tokens("qwen-unc") == 32_000


def test_known_model_map_still_resolves_without_override(monkeypatch):
    monkeypatch.delenv("KRYON_MODEL_MAX_TOKENS", raising=False)
    ct = _reload_cost_tracker()
    assert ct.get_model_input_tokens("deepseek-v4-flash") == 1_000_000
