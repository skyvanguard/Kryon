...
class TestHelpCommand:
    ...
    def test_handle_help_aliases(self, help_command, mock_console, mock_commands_registry, mock_aliases_registry):
    ...
    def test_handle_help_platform_manager_with_extensions(self, help_command, mock_console):
        ...
        with patch("kryon.repl.commands.help.HAS_PLATFORM_EXTENSIONS", True):
            with patch("kryon.is_kryon_extensions_platform_available", return_value=True):
                # Mock the platform manager without importing kryonextensions
                with patch(
                    "sys.modules",
                    {"kryonextensions.platform.base": Mock(platform_manager=mock_platform_manager)},
                ):
