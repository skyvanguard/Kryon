...
def test_paid_models_return_nonzero_cost():
    ...
    paid_models_with_expected_pricing = {
        ...
        "gpt4o": {"input_cost_per_token": 0.0000025, "output_cost_per_token": 0.00001},
...
def test_private_model_gpt4o_with_pricing_json():
    """Test that gpt4o works correctly when defined in pricing.json"""
    if not PYTEST_AVAILABLE:
        setup_test()

    # Create a pricing.json with gpt4o configuration
    pricing_config = {
        "gpt4o": {
            "max_tokens": 128000,
    ...
    print("\n" + "=" * 80)
    print("PRIVATE MODEL (gpt4o) PRICING TEST")
    print("=" * 80)

    # Clear the cache to ensure fresh calculation
    COST_TRACKER.calculated_costs_cache.clear()

    # Mock litellm to force fallback to pricing.json
    with patch("litellm.completion_cost", side_effect=Exception("Force fallback")):
        # Mock the file reading to simulate pricing.json with gpt4o
        with patch("pathlib.Path") as mock_path:
            ...
            with patch("builtins.open", create=True) as mock_open:
                ...
                with patch("json.load", return_value=pricing_config):
                    pricing = COST_TRACKER.get_model_pricing("gpt4o")
                    cost = calculate_model_cost("gpt4o", 100, 50)

                    expected_pricing = (5e-06, 5e-05)
                    expected_cost = 100 * 5e-06 + 50 * 5e-05  # 0.0030

                    # Show pricing information
                    print(f"Model: gpt4o                  | Pricing: {pricing} | Cost (100/50 tokens): ${cost:.6f}")
                    ...
                    if PYTEST_AVAILABLE:
                        ...
                        if pricing != expected_pricing:
                            raise AssertionError(f"gpt4o should have pricing {expected_pricing}, got {pricing}")
                        if abs(cost - expected_cost) >= 1e-10:
                            raise AssertionError(f"gpt4o should have cost {expected_cost}, got {cost}")
...
def test_reset_cost_for_local_model():
    ...
    with patch("requests.get") as mock_get:
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"gpt-4": {"input_cost_per_token": 0.00003, "output_cost_per_token": 0.00006}}
        mock_get.return_value = mock_response
...
def test_model_not_in_pricing_json_falls_back_to_litellm():
    ...
    # Create a pricing.json with only gpt4o
    pricing_config = {"gpt4o": {"input_cost_per_token": 5e-06, "output_cost_per_token": 5e-05}}
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            with patch("json.load", return_value=pricing_config):
                with patch("requests.get") as mock_get:
                    ...
                    # Test that gpt4o (in pricing.json) uses local pricing
                    pricing_gpt4o = COST_TRACKER.get_model_pricing("gpt4o")
                    expected_gpt4o_pricing = (5e-06, 5e-05)
                    cost_gpt4o = calculate_model_cost("gpt4o", 100, 50)
                    ...
                    print(
                        f"Model: gpt4o (from pricing.json) | Pricing: {pricing_gpt4o} | Cost (100/50 tokens): ${cost_gpt4o:.6f}"
                    )

                    if PYTEST_AVAILABLE:
                        assert pricing == expected_gpt4_pricing, f"gpt-4 should use LiteLLM pricing, got {pricing}"
                        assert pricing_gpt4o == expected_gpt4o_pricing, (
                            f"gpt4o should use local pricing, got {pricing_gpt4o}"
                        )
                    else:
                        if pricing != expected_gpt4_pricing:
                            raise AssertionError(f"gpt-4 should use LiteLLM pricing, got {pricing}")
                        if pricing_gpt4o != expected_gpt4o_pricing:
                            raise AssertionError(f"gpt4o should use local pricing, got {pricing_gpt4o}")
...
def run_all_tests():
    ...
    test_functions = [
        ...
        test_private_model_gpt4o_with_pricing_json,
    ...
    if failed == 0:
        ...
        print("- Private models (gpt4o) work with pricing.json")
