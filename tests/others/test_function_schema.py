...
def test_simple_function():
    ...
    assert func_schema.params_json_schema.get("properties", {}).get("a").get("description") == "The first argument"
    assert func_schema.params_json_schema.get("properties", {}).get("b").get("description") == "The second argument"
    assert not func_schema.takes_context
...
def test_complex_args_and_docs_function():
    ...
    with pytest.raises(ValidationError):
        func_schema.params_pydantic_model(**{"model": model.model_dump(), "some_flag": "not an int"})
...
def test_enum_and_literal_function():
    ...
    assert func_schema.params_json_schema.get("properties", {}).get("a", {}).get("$ref") == "#/$defs/MyEnum"
