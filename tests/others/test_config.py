...
def test_cc_no_default_key_errors(monkeypatch):
    ...
    with pytest.raises(openai.OpenAIError):
        OpenAIProvider(use_responses=False).get_model(skynet_model)


def test_cc_set_default_openai_key():
    set_default_openai_key("test_key")
    chat_model = OpenAIProvider(use_responses=False).get_model(skynet_model)
...
def test_resp_no_default_key_errors(monkeypatch):
    ...
    with pytest.raises(openai.OpenAIError):
        OpenAIProvider(use_responses=True).get_model(skynet_model)


def test_resp_set_default_openai_key():
    set_default_openai_key("test_key")
    resp_model = OpenAIProvider(use_responses=True).get_model(skynet_model)
...
def test_set_default_openai_api():
    assert isinstance(OpenAIProvider().get_model(skynet_model), OpenAIResponsesModel), "Default should be responses"

    set_default_openai_api("chat_completions")
    assert isinstance(OpenAIProvider().get_model(skynet_model), OpenAIChatCompletionsModel), (
        "Should be chat completions model"
    )

    set_default_openai_api("responses")
    assert isinstance(OpenAIProvider().get_model(skynet_model), OpenAIResponsesModel), "Should be responses model"
