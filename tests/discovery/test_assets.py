...
_FAKE_CRT_JSON = json.dumps(
    [
        {"name_value": "www.britimp.com.py\ncashbox.britimp.com.py"},
        {"name_value": "*.britimp.com.py"},
        {"name_value": "admin.britimp.com.py"},
    ]
).encode("utf-8")
...
def test_subdomains_dedupes_and_skips_wildcards():
    with patch("kryon.discovery.assets.urllib.request.urlopen", side_effect=_fake_urlopen_crt):
        result = discover_subdomains("britimp.com.py")
    targets = {a.target for a in result}
    assert "www.britimp.com.py" in targets
    assert "cashbox.britimp.com.py" in targets
    assert "admin.britimp.com.py" in targets
    assert "*.britimp.com.py" not in targets  # wildcard skipped
...
def test_valid_hostname():
    assert is_valid_target("www.britimp.com.py") is True
