...
from skynet.sdk.agents.items import RunItem
from skynet.sdk.agents.run import RunConfig
from skynet.sdk.agents.stream_events import AgentUpdatedStreamEvent
...
async def test_simple_first_run():
    ...
    result = Runner.run_streamed(agent, input=[get_text_input_item("message"), get_text_input_item("another_message")])
    async for _ in result.stream_events():
        pass
...
async def test_tool_call_runs():
    ...
    assert len(result.raw_responses) == 2, (
        "should have two responses: the first which produces a tool call, and the second whichhandles the tool result"
    )

    assert len(result.to_input_list()) == 5, (
        "should have five inputs: the original input, the message, the tool call, the tool result and the done message"
    )
...
async def test_handoffs():
    ...
    assert len(result.to_input_list()) == 7, (
        "should have 7 inputs: orig input, tool call, tool result, message, handoff, handoffresult, and done message"
    )
...
async def test_handoff_filters():
    ...
    assert len(result.to_input_list()) == 2, "should only have 2 inputs: orig input and last message"
...
async def test_input_guardrail_tripwire_triggered_causes_exception_streamed():
    def guardrail_function(context: RunContextWrapper[Any], agent: Agent[Any], input: Any) -> GuardrailFunctionOutput:
...
async def test_run_input_guardrail_tripwire_triggered_causes_exception_streamed():
    ...
    with pytest.raises(InputGuardrailTripwireTriggered):
        result = Runner.run_streamed(
            agent,
            input="user_message",
            run_config=RunConfig(input_guardrails=[InputGuardrail(guardrail_function=guardrail_function)]),
        )
        async for _ in result.stream_events():
            pass
...
async def test_run_output_guardrail_tripwire_triggered_causes_exception_streamed():
    ...
    with pytest.raises(OutputGuardrailTripwireTriggered):
        result = Runner.run_streamed(
            agent,
            input="user_message",
            run_config=RunConfig(output_guardrails=[OutputGuardrail(guardrail_function=guardrail_function)]),
        )
        async for _ in result.stream_events():
            pass
...
async def test_streaming_events():
    ...
    assert len(item_data) == total_expected_item_count, f"should have {total_expected_item_count} run items"
