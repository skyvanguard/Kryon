...
async def test_simple_first_run():
    ...
    result = await Runner.run(agent, input=[get_text_input_item("message"), get_text_input_item("another_message")])
    assert len(result.new_items) == 1, "exactly one item should be generated"
...
async def test_input_guardrail_tripwire_triggered_causes_exception():
    ...
    agent = Agent(name="test", input_guardrails=[InputGuardrail(guardrail_function=guardrail_function)])
    model = FakeModel()
...
async def test_tool_use_behavior_first_output():
    ...
    assert result.final_output == Foo(bar="tool_one_result"), "should have used the first tool result"
...
async def test_model_settings_override():
    model = FakeModel()
    agent = Agent(name="test", model=model, model_settings=ModelSettings(temperature=1.0, max_tokens=1000))
