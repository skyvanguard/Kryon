...
Surfaced POC Example .200.26 mediavault: smbclient -L -N anonymous
login successful + share `rpa-share` listable (cliente example RPA).
...
def _svc(host: str = "192.0.2.26", port: int = 445, state: str = "open") -> DiscoveredService:
...
# Sample outputs
_SMB_EXAMPLE_RPA_SHARE = """
Anonymous login successful

	Sharename       Type      Comment
	---------       ----      -------
	rpa-share       Disk
	IPC$            IPC       IPC Service (rpa-share server)
...
# ---------------------------------------------------------------------------
# Positive — POC Example .200.26 RPA-SHARE case (MEDIUM)
# ---------------------------------------------------------------------------


class TestExampleRpaShare:
    def test_rpa_share_promotes_medium(self):
        """The exact .200.26 case: anonymous login OK + `rpa-share`
        share visible. 'rpa' es keyword sensitive -> MEDIUM."""
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_smbclient(_SMB_EXAMPLE_RPA_SHARE)):
            finding = _check_smb_anonymous_shares(_svc())
        assert finding is not None
        assert finding.severity == "MEDIUM"
        assert finding.cwe == "CWE-200"
        assert finding.rule_id == "smb-anonymous-list"
        assert "rpa-share" in finding.message or "rpa-share" in finding.evidence
