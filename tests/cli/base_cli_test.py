...
class BaseCLITest:
    ...
    def run_cli_simulation(
        ...
        for i, response_content in enumerate(expected_responses):
            ...
            mock_responses.append(self.create_mock_completion(response_content, response_tool_calls))
        ...
        [
            ...
            patch("kryon.repl.ui.banner.display_compact_banner"),
            patch("kryon.repl.ui.banner.display_first_run_welcome"),
            # LLM calls
        ...
        def apply_patches_and_run():
            with (
                ...
                patch.multiple("kryon.repl.ui.banner", display_compact_banner=Mock(), display_first_run_welcome=Mock()),
                patch.multiple(
                    "kryon.util",
        ...
        if verify_message_flow:
            results["message_flow_valid"] = self._verify_message_flow(user_inputs, expected_responses, tool_calls)
    ...
    def assert_message_history_contains(self, role: str, content_substring: str):
        ...
        for msg in message_history:
            if msg.get("role") == role and content_substring in str(msg.get("content", "")):
                return True
        raise AssertionError(f"Message history does not contain {role} message with content '{content_substring}'")
