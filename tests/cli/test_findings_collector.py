...
class TestSaveFindings:
    ...
    def test_save_dedups_same_finding_across_reruns(self):
        # Re-running the same analysis must NOT duplicate the finding in the DB.
        mock_store = MagicMock()
        collector = CLIFindingsCollector(store=mock_store)
        f = Finding(title="SPF missing", description="d", severity=Severity.MEDIUM, affected_asset="example.com")

        assert collector.save_findings([f]) == 1  # first run persists
        assert collector.save_findings([f]) == 0  # re-run is a no-op
        assert mock_store.save_finding.call_count == 1

    def test_finding_signature_is_content_based(self):
        a = Finding(title="X", description="d1", severity=Severity.HIGH, affected_asset="h")
        b = Finding(title="X", description="d2-different", severity=Severity.HIGH, affected_asset="h")
        c = Finding(title="X", description="d1", severity=Severity.LOW, affected_asset="h")
        sig = CLIFindingsCollector.finding_signature
        assert sig(a) == sig(b)  # same title+asset+severity (id/desc ignored)
        assert sig(a) != sig(c)  # different severity → different finding
