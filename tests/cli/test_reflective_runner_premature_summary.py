...
def test_has_foothold_privesc_hint() -> None:
    # G16: privesc primitives (sudo/SUID/caps from _extract_privesc) are foothold.
    facts = ExtractedFacts(hints=("privesc:sudo-nopasswd:/usr/bin/find",))
    assert _has_foothold(facts) is True


def test_has_foothold_web_marker() -> None:
    # G16: a web engagement rarely yields a shell prompt — webshell/flag are foothold.
    assert _has_foothold(ExtractedFacts(hints=("webshell uploaded to /uploads/x.php",))) is True
    assert _has_foothold(ExtractedFacts(hints=("flag{captured_it}",))) is True


def test_has_foothold_recon_only_is_false() -> None:
    # Users/discovered paths alone is recon, not foothold.
    facts = ExtractedFacts(users=("alice", "bob"), hints=("discovered:login.php",))
