...
class TestCLIStreaming(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for (_name, _instance_id), model_ref in list(ACTIVE_MODEL_INSTANCES.items()):
    ...
    def get_combined_message_history(self):
        ...
        for _agent_name, history in histories.items():
    ...
    def add_to_test_message_history(self, msg):
        ...
        if not test_model:
            ...
            # Register the agent in AGENT_MANAGER and link the message history
            # This ensures get_all_histories() returns the model's history
            AGENT_MANAGER._active_agent_name = test_agent_name
            AGENT_MANAGER._agent_registry[test_agent_name] = "P1"
            AGENT_MANAGER._message_history[test_agent_name] = test_model.message_history
    ...
    def test_ctrl_c_cleanup_message_consistency(self):
        ...
        assert len(self.get_combined_message_history()) >= 3, "Should have user, assistant, tool messages"
        ...
        assert tool_msg["tool_call_id"] == assistant_msg["tool_calls"][0]["id"], "Tool call ID should match"
        assert "interrupted" in tool_msg["content"].lower(), "Tool result should indicate interruption"
    ...
    def test_fix_message_list_with_interrupted_tools(self):
        ...
        try:
            ...
            assert len(fixed_messages) > len(incomplete_messages), "fix_message_list should add missing tool result"
            ...
            for i, msg in enumerate(fixed_messages):
                ...
                assert msg["role"] in ["user", "assistant", "system", "tool"], f"Fixed message {i} has invalid role"
    ...
    def test_message_history_openai_format_compliance(self):
        ...
        for i, msg in enumerate(self.get_combined_message_history()):
            ...
            if msg["role"] == "assistant" and msg.get("tool_calls"):
                assert isinstance(msg["tool_calls"], list), f"Assistant message {i} tool_calls must be a list"
                for j, tc in enumerate(msg["tool_calls"]):
                    ...
                    assert "name" in tc["function"], f"Tool call {j} function in message {i} missing 'name'"
                    assert "arguments" in tc["function"], f"Tool call {j} function in message {i} missing 'arguments'"
    ...
    def test_multiple_interrupt_scenarios(self):
        ...
        for user_input, call_id, result_content in scenarios:
            ...
            # Add tool result
            self.add_to_test_message_history({"role": "tool", "tool_call_id": call_id, "content": result_content})
    ...
    def verify_message_history_openai_compliance(self):
        ...
        for i, msg in enumerate(self.get_combined_message_history()):
            ...
            # Role validation
            valid_roles = ["user", "assistant", "system", "tool", "developer"]
            assert msg["role"] in valid_roles, f"Message {i} role '{msg['role']}' not in valid roles {valid_roles}"
            ...
            if msg["role"] in ["user", "system", "developer"]:
                ...
                assert has_content or has_tool_calls, f"Assistant message {i} must have content or tool_calls"
    ...
    def test_ctrl_c_during_tool_execution_real_behavior(self):
        ...
        try:
            ...
            for call_id_check, _call_info in list(test_model._converter.recent_tool_calls.items()):
        ...
        self.assertEqual(self.get_combined_message_history()[1]["tool_calls"][0]["function"]["name"], tool_name)
