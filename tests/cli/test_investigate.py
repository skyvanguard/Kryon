...
class TestIntentClassification:
    ...
    def test_url_query_with_stray_dot_stays_web_audit(self):
        # Regression: a lone "." token (the CWD) must not hijack an URL-audit query
        # into code_sast. This is the exact bug that made an example web-audit query
        # ("…app.example.com . El dueño…") run a SAST review of the local repo.
        hints = _classify_intent("Descubri la API de https://app.example.com . El dueno confirmo el acceso")
        assert hints["mode"] == "web_audit"
        assert "code_path" not in hints

    def test_bare_cwd_tokens_never_select_code_sast(self):
        for q in (
            "audita https://x.example .",
            "revisa https://x.example ..",
            "mira https://x.example ./",
        ):
            hints = _classify_intent(q)
            assert hints["mode"] == "web_audit", q
            assert "code_path" not in hints

    def test_local_dir_still_detected_without_url(self):
        # Guard: the fix must not break the legitimate `investigate ./code/` path.
        with tempfile.TemporaryDirectory() as tmp:
            hints = _classify_intent(f"audita {tmp}")
            assert hints["mode"] == "code_sast"
            assert hints["code_path"] == tmp

    def test_explicit_url_overrides_code_sast(self):
        # --url is authoritative even if the prompt would classify as code_sast.
        with tempfile.TemporaryDirectory() as tmp:
            hints = _route_investigate_mode(f"revisa {tmp}", "https://target.example")
            assert hints["mode"] == "web_audit"
            assert "code_path" not in hints
            assert "https://target.example" in hints["urls"]
...
class TestPromptBuilder:
    ...
    def test_has_unified_rerun_rule(self):
        # G6: one consistent re-run rule (re-run when the hypothesis warrants it, not
        # to re-confirm injected ground truth) instead of contradicting the base prompt.
        hints = {"mode": "web_audit", "urls": [], "keywords": []}
        prompt = _build_investigate_prompt("audita x", hints, active=False).lower()
        assert "re-ejecución" in prompt or "re-corré" in prompt
...
class TestArgparseWiring:
    ...
    def test_subparser_recon_only_flag(self):
        parser = argparse.ArgumentParser()
        sub = parser.add_subparsers(dest="command")
        add_investigate_subparser(sub)

        args = parser.parse_args(["investigate", "x"])
        assert args.recon_only is False
        args = parser.parse_args(["investigate", "x", "--active", "--recon-only"])
        assert args.recon_only is True
