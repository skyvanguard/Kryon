...
def test_spring_actuator1_root_level(monkeypatch):
    # Spring Boot 1.x serves actuators at the root (no /actuator prefix).
    _mock_get(monkeypatch, {"/env": (200, '{"profiles":["prod"],"systemProperties":{}}')})
    out = wa._check_spring_actuator1(_svc(), "http")
    assert out and out[0].rule_id == "spring-actuator-env" and out[0].severity == "HIGH"


def test_spring_actuator1_html_no_fp(monkeypatch):
    _mock_get(monkeypatch, {p: (200, "<html>app</html>") for p, *_ in wa._ACTUATORS_V1})
    assert wa._check_spring_actuator1(_svc(), "http") == []


def test_product_grafana_detected(monkeypatch):
    monkeypatch.setattr(wa, "_get_full", lambda host, port, path, scheme:
        (200, '{"commit":"abc","database":"ok","version":"9.4.3"}', {}) if path == "/api/health" else None)
    assert any(f.rule_id == "grafana-exposed" for f in wa._check_products(_svc(), "http"))


def test_product_jenkins_by_header(monkeypatch):
    monkeypatch.setattr(wa, "_get_full", lambda host, port, path, scheme:
        (200, "<html>Jenkins</html>", {"x-jenkins": "2.426.1"}) if path == "/login" else None)
    assert any(f.rule_id == "jenkins-exposed" for f in wa._check_products(_svc(), "http"))


def test_product_telerik_and_ofbiz(monkeypatch):
    resp = {
        "/Telerik.Web.UI.WebResource.axd?type=rau": (200, "RadAsyncUpload handler is registered succesfully", {}),
        "/webtools/control/main": (200, "<title>Apache OFBiz</title>", {}),
    }
    monkeypatch.setattr(wa, "_get_full", lambda host, port, path, scheme: resp.get(path))
    rules = {f.rule_id for f in wa._check_products(_svc(), "http")}
    assert "telerik-ui-exposed" in rules and "ofbiz-exposed" in rules


def test_products_no_fp_on_generic_app(monkeypatch):
    monkeypatch.setattr(wa, "_get_full", lambda host, port, path, scheme:
        (200, "<!doctype html><html>generic app</html>", {}))
    assert wa._check_products(_svc(), "http") == []
