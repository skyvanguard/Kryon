...
def test_dispatch_tables_well_formed():
    assert len(_HTTP_PROBES) == 6 and len(_TCP_PROBES) == 13  # +1: ajp-ghostcat (8009)
    for _n, m, p in (*_HTTP_PROBES, *_TCP_PROBES):
        assert callable(m) and callable(p)


def test_ajp_ghostcat_dispatch_and_graceful():
    # Wired into the open-port sweep (was only in the tomcat-audit framework). Port 8009 -> _check_ajp.
    fired = [n for n, m, _ in _TCP_PROBES if m(_svc(8009))]
    assert "ajp-ghostcat" in fired
    # Graceful on a dead port (no AJP CPong), never raises.
    assert ip._check_ajp(_svc(8009)) is None


def test_ajp_ghostcat_detects_cpong(monkeypatch):
    # An AJP/1.3 connector answers a CPing with a CPong (magic 0x41 0x42 'AB') -> Ghostcat finding.
    monkeypatch.setattr(ip, "_tcp", lambda h, p, payload, n: b"\x41\x42\x00\x01\x09")
    f = ip._check_ajp(_svc(8009))
    assert f is not None and f.rule_id == "ajp-ghostcat-exposed" and f.severity == "CRITICAL"


def test_docker_registry_open(monkeypatch):
    # No real registry behind the manifest pulls → they fail gracefully, leaving just the open finding.
    monkeypatch.setattr(ip, "_http_get", lambda *a, **k: (200, '{"repositories":["app","db"]}'))
    out = ip._check_docker_registry(_svc(5000))
    assert any(f.rule_id == "docker-registry-open" and f.severity == "HIGH" for f in out)


def test_docker_registry_authed_returns_empty(monkeypatch):
    monkeypatch.setattr(ip, "_http_get", lambda *a, **k: (401, "unauthorized"))
    assert ip._check_docker_registry(_svc(5000)) == []


def test_secret_env_regex_matches_creds_not_versions():
    blob = "NODE_VERSION=19.3.0\nDB_USER=root\nDB_PASS=Ng1-f3!Pe7-e5?Nf3xe5\nYARN_VERSION=1.22\nAPI_TOKEN=abc123"
    found = dict(ip._SECRET_ENV_RE.findall(blob))
    assert found.get("DB_PASS") == "Ng1-f3!Pe7-e5?Nf3xe5" and "DB_USER" in found and "API_TOKEN" in found
    assert "NODE_VERSION" not in found and "YARN_VERSION" not in found  # version pins are not secrets


def test_docker_registry_extracts_baked_secrets(monkeypatch):
    """The Umbrella gap: a detect-only check left the MySQL root password baked in the image's build
    history on the table. The auto-pull must surface DB_PASS from the config history."""
    import json

    monkeypatch.setattr(ip, "_http_get", lambda *a, **k: (200, '{"repositories":["umbrella/timetracking"]}'))
    manifest = json.dumps({"config": {"digest": "sha256:abc"}, "layers": []}).encode()
    config = json.dumps({
        "config": {"Env": ["NODE_VERSION=19.3.0", "LOG_FILE=/logs/tt.log"]},
        "history": [
            {"created_by": "/bin/sh -c #(nop)  ENV DB_USER=root"},
            {"created_by": "/bin/sh -c #(nop)  ENV DB_PASS=Ng1-f3!Pe7-e5?Nf3xe5"},
            {"created_by": "/bin/sh -c #(nop)  ENV NODE_VERSION=19.3.0"},
        ],
    }).encode()
    ...
    def fake_urlopen(req, timeout=8):
        ...
        return _Resp(manifest if "/manifests/" in url else config)

    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    out = ip._check_docker_registry(_svc(5000))
    rules = [f.rule_id for f in out]
    assert "docker-registry-open" in rules
    secrets = [f.message for f in out if f.rule_id == "docker-registry-baked-secret"]
    assert any("DB_PASS=Ng1-f3!Pe7-e5?Nf3xe5" in s for s in secrets)
    assert any("DB_USER=root" in s for s in secrets)
    assert not any("NODE_VERSION" in s for s in secrets)  # version pin, not a secret
