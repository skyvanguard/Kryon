...
_DIG_AXFR_SUCCESS = """
; <<>> DiG 9.16.1 <<>> +time=4 +tries=1 @192.0.2.205 AXFR example.com
; (1 server found)
;; global options: +cmd
example.com.		3600	IN	SOA	dc01.example.com. admin.example.com. 2026051801 900 600 86400 3600
example.com.		3600	IN	NS	dc01.example.com.
example.com.		3600	IN	NS	dc02.example.com.
example.com.		3600	IN	MX	10 mail.example.com.
example.com.		3600	IN	TXT	"v=spf1 ip4:201.x.x.x ~all"
dc01.example.com.	1200	IN	A	192.0.2.205
dc02.example.com.	1200	IN	A	192.0.2.5
mail.example.com.	1200	IN	A	192.0.2.150
example.com.		3600	IN	SOA	dc01.example.com. admin.example.com. 2026051801 900 600 86400 3600
...
class TestAxfrSuccess:
    def test_example_zone_transfer_succeeds(self):
        """Simulates the worst-case .205 scenario: AXFR for example.com
...
class TestAxfrRestricted:
    ...
    def test_axfr_communications_error(self):
        comm_err = "; Transfer failed.\n;; communications error to 192.0.2.205#53: end of file\n"
        ptr_stdout = "Name:    dc.example.com\n"
...
class TestDeriveZoneCandidates:
    def test_ptr_yields_zone(self):
        ...
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_proc(ptr_stdout)):
            zones = _derive_dns_zone_candidates("192.0.2.205")
    ...
    def test_reverse_zone_always_added_for_ipv4(self):
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_proc("")):
            zones = _derive_dns_zone_candidates("192.0.2.205")
        assert "2.0.192.in-addr.arpa" in zones
...
class TestTryAxfr:
    def test_dig_success_first(self):
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_proc(_DIG_AXFR_SUCCESS)):
            ok, snippet = _try_axfr("192.0.2.205", "example.com")
        assert ok is True
