...
_NXDOMAIN_OUT = (
    "Server:  UnKnown\nAddress:  192.0.2.205\n\n*** UnKnown can't find 192.0.2.99: Non-existent domain\n"
)
...
class TestEnumMedium:
    def test_three_generic_hostnames_flag_medium(self):
        ...
        ptr_map = {
            "192.0.2.5": "host5.internal.example",
            "192.0.2.10": "host10.internal.example",
            "192.0.2.50": "bastion.internal.example",
        }
...
class TestEnumHigh:
    def test_banking_keyword_elevates_to_high(self):
        ptr_map = {
            "192.0.2.5": "dc02.example.com",
            "192.0.2.50": "bastion.example.com",
            "192.0.2.150": "core-banking-db.example.com",
            "192.0.2.200": "swift-gateway.example.com",
        }
    ...
    def test_postgres_keyword_elevates(self):
        ptr_map = {
            "192.0.2.10": "vmhost.example.local",
            "192.0.2.100": "app01.example.local",
            "192.0.2.150": "postgres-prod.example.local",
        }
    ...
    def test_vault_keyword_elevates(self):
        ptr_map = {
            "192.0.2.1": "gateway.example.local",
            "192.0.2.10": "app01.example.local",
            "192.0.2.100": "vault-prod.example.local",
        }
...
class TestNegative:
    def test_only_two_hits_below_threshold(self):
        ptr_map = {
            "192.0.2.5": "dc02.example.local",
            "192.0.2.50": "bastion.example.local",
        }
    ...
    def test_all_refused_no_flag(self):
        refused = "*** UnKnown can't find 192.0.2.10: Query refused\n"
        with patch(
            "kryon.cli.engage.subprocess.run",
    ...
    def test_all_generic_ip_derived_hostnames_no_flag(self):
        """Hostnames like `host-192-0-2-5.dyn.isp.net` are
...
class TestGate:
    ...
    def test_closed_port_skipped(self):
        svc = DiscoveredService(host="192.0.2.205", port=53, state="closed", service="domain", product="")
        with patch("kryon.cli.engage.subprocess.run", side_effect=AssertionError("must not call")):
            assert _check_reverse_dns_enum(svc) is None
...
class TestTryPtrQuery:
    def test_parses_name_line(self):
        out = _ptr_response("dc01.example.com", "192.0.2.205")
        with patch(
            "kryon.cli.engage.subprocess.run",
            side_effect=lambda cmd, **_kw: subprocess.CompletedProcess(cmd, 0, stdout=out, stderr=""),
        ):
            assert _try_ptr_query("192.0.2.205", "192.0.2.5") == "dc01.example.com"

    def test_refused_returns_none(self):
        out = "*** UnKnown can't find 192.0.2.5: Query refused\n"
        with patch(
            "kryon.cli.engage.subprocess.run",
            side_effect=lambda cmd, **_kw: subprocess.CompletedProcess(cmd, 0, stdout=out, stderr=""),
        ):
            assert _try_ptr_query("192.0.2.205", "192.0.2.5") is None
...
class TestIsGenericPtr:
    def test_in_addr_arpa_is_generic(self):
        assert _is_generic_ptr("5.2.0.192.in-addr.arpa", "192.0.2.5") is True

    def test_ip_derived_hostname_is_generic(self):
        assert _is_generic_ptr("host-192-0-2-5.dyn.isp.net", "192.0.2.5") is True

    def test_named_hostname_is_not_generic(self):
        assert _is_generic_ptr("dc01.example.com", "192.0.2.5") is False
        assert _is_generic_ptr("bastion-prod.example.local", "192.0.2.50") is False
