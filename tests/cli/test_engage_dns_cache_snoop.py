...
; <<>> DiG 9.16 <<>> +norecurse +cd @172.18.201.205 outlook.office365.com A
;; global options: +cmd
;; Got answer:
...
;; Query time: 4 msec
;; SERVER: 172.18.201.205#53
"""

_DIG_NOT_CACHED = """
; <<>> DiG 9.16 <<>> +norecurse +cd @172.18.201.205 stripe.com A
;; global options: +cmd
;; Got answer:
...
_DIG_REFUSED = """
; <<>> DiG 9.16 <<>> +norecurse +cd @172.18.201.205 instagram.com A
;; global options: +cmd
;; Got answer:
...
; <<>> DiG 9.16 <<>> +norecurse +cd @172.18.201.205 bcp.com.py A
;; ANSWER SECTION:
...
class TestCacheSnoopDetected:
    def test_two_cached_domains_flag_medium(self):
        """Britimp-realistic: outlook.office365.com (Microsoft 365) +
        bcp.com.py (banking) cached -> MEDIUM finding."""

        def _multi(cmd, **_kw):
            # cmd is like ["dig", ..., "@172.18.201.205", "<name>", "A"]
            name = cmd[-2]
