...
def test_run_one_survives_whitespace_only_stderr(monkeypatch):
    """A child that writes only whitespace to stderr must not crash _run_one with
    IndexError (the `if proc.stderr` guard checked the raw string, but `[-1]`
    indexed `.strip().splitlines()` == [] → uncaught IndexError aborted the batch)."""

    class _Proc:
        returncode = 0
        stdout = ""
        stderr = "\n   \n"  # truthy, but strips to empty → splitlines() == []

    monkeypatch.setattr(subprocess, "run", lambda *a, **k: _Proc())
    item_id, rc, tail = _run_one(["kryon", "engage", "x"], "item-1")
    assert item_id == "item-1"
    assert rc == 0
    assert tail == ""  # no crash, empty tail


def test_resolve_engage_bin_existing_path_with_spaces_is_single_token(tmp_path):
    r"""A bare existing path (Windows 'C:\Program Files\...' style) must stay ONE
    argv token — shlex.split would eat backslashes / break on the space."""
    exe = tmp_path / "Program Files" / "kryon.exe"
    exe.parent.mkdir(parents=True)
    exe.write_text("stub")
    assert _resolve_engage_bin(str(exe)) == [str(exe)]


def test_resolve_engage_bin_command_with_args_still_splits():
    assert _resolve_engage_bin("kryon --profile x") == ["kryon", "--profile", "x"]
