...
servicios ASIMETRICA. Surface ground truth: Example POC pilot
2026-05-18, .205 vs .5 — mismo dominio example.com pero distinto
...
class TestBelowThreshold:
    def test_single_dc_no_drift(self):
        host_findings = {
            "192.0.2.205": [_f("AD-1.1"), _f("dns-open-resolver")],
        }
...
# ---------------------------------------------------------------------------
# DNS rule drift — the Example .205 vs .5 case
# ---------------------------------------------------------------------------


class TestDnsRuleDrift:
    def test_example_dnssec_drift_scenario(self):
        ...
        """
        host_findings = {
            "192.0.2.205": [_f("AD-1.1"), _f("dns-open-resolver"), _f("dnssec-validation-disabled")],
            "192.0.2.5": [_f("AD-1.1"), _f("dns-open-resolver")],
        }
...
class TestServiceDrift:
    def test_iis_plaintext_drift(self):
        ...
        host_findings = {
            "192.0.2.205": [_f("AD-1.1")],
            "192.0.2.5": [_f("AD-1.1"), _f("http-plaintext", host="192.0.2.5:80", sev="HIGH")],
        }
        drift = diff_dc_dns_posture(host_findings)
        ...
        assert "192.0.2.5" in iis_drift[0].message
        assert "192.0.2.205" in iis_drift[0].message

    def test_ssh_drift(self):
        """The .5 case: SSH-for-Windows on .5 but not on .205."""
        host_findings = {
            "192.0.2.205": [_f("AD-1.1")],
            "192.0.2.5": [_f("AD-1.1"), _f("ssh-banner-visible", host="192.0.2.5:22")],
        }
        drift = diff_dc_dns_posture(host_findings)
