...
Surfaced POC Example 2026-05-19: cluster `example-cluster` con 3
nodos PVE en versiones DIFERENTES:
  - .115 proxmox2: PVE 9.1.4
  - .200 pve-example: PVE 8.4.16 (aislado del cluster!)
...
# ---------------------------------------------------------------------------
# Example POC scenario — 3 nodes with drift
# ---------------------------------------------------------------------------


class TestExampleClusterDrift:
    def test_example_python_simplehttp_drift(self):
        """POC Example: python http.server CRITICAL solo en .200,
        no en .115 ni .222 -> drift CRITICAL."""
        host_findings = {
            "192.0.2.115": [_f("PVE-1.2"), _f("PVE-2.1"), _f("PVE-3.1"), _f("sshd-permit-root-login")],
            "192.0.2.200": [
                _f("PVE-1.2"),
                _f("PVE-2.1"),
                _f("PVE-3.1"),
            ...
            "192.0.2.222": [_f("PVE-1.2"), _f("PVE-2.1"), _f("PVE-3.1"), _f("sshd-permit-root-login")],
        }
...
class TestPveVersionDrift:
    def test_three_versions_in_cluster_flags_high(self):
        """Example .115 = 9.1.4, .200 = 8.4.16, .222 = 9.1.8."""
        host_findings = {
            "192.0.2.115": [_f("PVE-1.2", evidence="pveversion: pve-manager/9.1.4/...")],
            "192.0.2.200": [_f("PVE-1.2", evidence="pveversion: pve-manager/8.4.16/...")],
            "192.0.2.222": [_f("PVE-1.2", evidence="pveversion: pve-manager/9.1.8/...")],
        }
        drift = diff_proxmox_cluster_posture(host_findings)
