...
Regression test for the gap detected in the Britimp POC pilot 2026-05-18
against .205 (DC britimp.com.py). Manual probe `nslookup google.com
172.18.201.205` resolved to public Google IPs (142.251.128.78 +
...
class TestOpenResolverDetected:
    def test_britimp_h205_scenario(self):
    ...
    def test_resolves_multiple_external(self):
        stdout = (
            "Server:  dns01.britimp\n"
            "Address:  172.18.201.5\n"
            "\n"
            "Name:    google.com\n"
            "Addresses:  142.251.128.78\n"
            "          142.251.128.100\n"
        ...
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_nslookup(stdout)):
            finding = _check_dns_open_resolver(_svc(host="172.18.201.5"))
        assert finding is not None
...
class TestSecureConfigurations:
    def test_recursion_refused(self):
        stdout = (
            "Server:  internal-dns.britimp\n"
            "Address:  172.18.201.205\n"
            "\n"
            "*** internal-dns.britimp can't find google.com: Query refused\n"
        )
        with patch("kryon.cli.engage.subprocess.run", side_effect=_fake_nslookup(stdout)):
...
class TestIsExternalIpv4:
    def test_public_google_ip_is_external(self):
        assert _is_external_ipv4("142.251.128.78", "172.18.201.205") is True

    def test_rfc1918_10_is_not_external(self):
        assert _is_external_ipv4("10.0.0.5", "172.18.201.205") is False

    def test_rfc1918_172_is_not_external(self):
        assert _is_external_ipv4("172.18.201.5", "172.18.201.205") is False
        assert _is_external_ipv4("172.16.0.1", "172.18.201.205") is False
        assert _is_external_ipv4("172.31.255.254", "172.18.201.205") is False
    ...
    def test_172_outside_rfc1918_range_is_external(self):
        ...
        assert _is_external_ipv4("172.15.0.1", "172.18.201.205") is True
        assert _is_external_ipv4("172.32.0.1", "172.18.201.205") is True

    def test_192_168_is_not_external(self):
        assert _is_external_ipv4("192.168.1.1", "172.18.201.205") is False

    def test_loopback_is_not_external(self):
        assert _is_external_ipv4("127.0.0.1", "172.18.201.205") is False

    def test_link_local_is_not_external(self):
        assert _is_external_ipv4("169.254.1.5", "172.18.201.205") is False
    ...
    def test_target_self_is_not_external(self):
        ...
        assert _is_external_ipv4("172.18.201.205", "172.18.201.205") is False
