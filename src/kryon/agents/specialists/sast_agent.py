...
_SAST_PROMPT = (
    ...
    "1. `run_command` con `grep -rn` para los sinks de riesgo del lenguaje del "
    "repo (donde input no confiable llega a una operación peligrosa).\n"
...
def create_sast_specialist(registry: dict[str, Any], *, model: Any = None):
    ...
    tools = [registry[name] for name in ("run_command",) if name in registry]
    kwargs: dict[str, Any] = {}
