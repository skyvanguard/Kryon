...
def discover_agent_factories() -> dict[str, Callable[[], Agent]]:
    ...
    for _importer, modname, ispkg in pkgutil.iter_modules(kryon.agents.__path__, kryon.agents.__name__ + "."):
        if ispkg:
    ...
    patterns_path = os.path.join(os.path.dirname(kryon.agents.__file__), "patterns")
    if os.path.exists(patterns_path):
        for _importer, modname, ispkg in pkgutil.iter_modules([patterns_path], kryon.agents.__name__ + ".patterns."):
            if ispkg:
                continue
...
def get_agent_factory(agent_name: str) -> Callable[[], Agent]:
    ...
    if agent_name_lower not in AGENT_FACTORIES:
        raise ValueError(f"Agent '{agent_name}' not found. Available agents: {list(AGENT_FACTORIES.keys())}")
