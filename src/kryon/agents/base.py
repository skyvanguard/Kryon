...
def is_cloud_model(model: str | None) -> bool:
    ...
    s = (model or "").lower()
    # Local aliases (served by our own llama-server) are never cloud, even if the
    # underlying GGUF name contains a cloud-ish token — e.g. the bench alias
    # `kryon-gpt-oss` carries "gpt-" but is a local model. Cloud models (deepseek,
    # gpt-4, claude…) are configured without the kryon-/local- prefix.
    if s.startswith("kryon-") or s.startswith("local"):
        return False
