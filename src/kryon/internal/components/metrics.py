...
def _load_component(name: str) -> Any:
    ...
    try:
        ...
        if sys.modules.get(f"kryon.internal.components.{name}"):
            return sys.modules[f"kryon.internal.components.{name}"]
        try:
            return importlib.import_module(f"kryon.internal.components.{name}")
        except ImportError:
