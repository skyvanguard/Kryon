...
def create_app(config: ServerConfig | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Wire the scheduler singleton + its scan callback, then restore jobs.
        # (Previously a throwaway ScanScheduler was created here — not the
        # singleton the API uses — and no callback was ever set, so scheduled
        # scans never executed.)
        try:
            from kryon.server.deps import get_scheduler
            from kryon.server.routes.scans import run_scheduled_scan

            scheduler = get_scheduler()
            scheduler.set_scan_callback(run_scheduled_scan)
    ...
    # Client-facing dashboard — served from the appliance itself.
    # Static assets (CSS/JS) are same-origin, so they satisfy the strict CSP
    # (script-src 'self') without weakening the security headers. All data is
    # fetched by the JS from the authenticated /api/v1 endpoints (X-API-Key).
    ui_dir = Path(__file__).parent / "ui"
    if (ui_dir / "index.html").is_file():
        app.mount("/app", StaticFiles(directory=str(ui_dir)), name="ui")
