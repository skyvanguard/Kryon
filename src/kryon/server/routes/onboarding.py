...
async def update_step(session_id: str, body: StepBody, user: User | None = Depends(get_current_user)) -> dict:
    ...
    # BOLA fix: a scoped user must own the session's client (sibling credential
    # endpoints already do this; these 4 skipped it).
    verify_client_access(user, session["client_id"], store)
...
async def get_session(session_id: str, user: User | None = Depends(get_current_user)) -> dict:
    ...
    if not session:
        logger.warning("Onboarding session not found: %s", session_id)
        raise not_found("OnboardingSession", session_id)
    verify_client_access(user, session["client_id"], store)  # BOLA fix
    return session


@router.post("/onboarding/{session_id}/complete")
async def complete_onboarding(session_id: str, user: User | None = Depends(get_current_user)) -> dict:
...
async def import_assets(body: ImportBody, user: User | None = Depends(get_current_user)) -> dict:
    """Import assets from CSV or JSON."""
    store = get_store()
    # BOLA fix: a scoped user can't inject assets under a client they don't own.
