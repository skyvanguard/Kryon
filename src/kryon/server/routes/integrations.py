...
@router.post("/integrations/siem", response_model=SIEMConfigResponse, dependencies=[Depends(require_permission("integrations:write"))])
async def create_siem_config(req: CreateSIEMConfigRequest) -> SIEMConfigResponse:
    ...
    store.create_siem_config({
        ...
        "endpoint": req.endpoint, "token": req.token,
...
async def list_siem_configs() -> list[SIEMConfigResponse]:
    ...
    return [SIEMConfigResponse(
        id=c["id"], name=c["name"], siem_type=c["siem_type"],
        endpoint=c["endpoint"], index_name=c.get("index_name", ""),
        enabled=bool(c.get("enabled", True)), created_at=c["created_at"],
    ) for c in configs]
