...
# Billing usage/tier/limits are cross-tenant admin data — the endpoints take
# tenant_id as a free query param with no membership check, so gate the whole
# router behind billing:read (only 'admin' holds it via '*'), stopping any
# analyst/viewer from enumerating another tenant's metering/tier/features.
router = APIRouter(
    tags=["billing"],
    dependencies=[Depends(require_api_key), Depends(require_permission("billing:read"))],
)
