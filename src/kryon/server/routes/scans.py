...
@router.post("/scans", dependencies=[Depends(require_permission("scans:write"))])
...
@router.delete("/scans/{job_id}", dependencies=[Depends(require_permission("scans:write"))])
...
def _persist_auto_scan(orch, client_id: str) -> None:
    """Persist a finished auto-scan to the store as a ScanRecord + FindingRecords.

    This is the linchpin that makes the dashboard come alive: the findings /
    drift / KPI views all read from the store, so without this the UI stays
    empty after a scan. Also creates a ScanRecord per run, which is exactly
    what the drift endpoint diffs against the previous run.

    Best-effort: a persistence failure never breaks the scan itself.
    """
    ...
    # create_scan has a FK to clients — ensure the client row exists first.
    try:
        if store.get_client(cid) is None:
            store.create_client(Client(id=cid, name=cid.capitalize()))
    except Exception:
        logger.debug("Could not ensure client %s exists", cid, exc_info=True)

    findings = list(getattr(orch, "findings", []) or [])
    try:
        scan = ScanRecord(
            client_id=cid,
            agent_key="kryon",
        ...
        logger.warning("Failed to persist ScanRecord for client %s", cid, exc_info=True)
        return

    saved = 0
    for f in findings:
        try:
            store.save_finding(FindingRecord(scan_id=scan.id, client_id=cid, finding_json=f.model_dump_json()))
            saved += 1
        except Exception:
            logger.debug("Skipped persisting a finding", exc_info=True)
    logger.info("Persisted auto-scan: client=%s scan=%s findings=%d/%d", cid, scan.id, saved, len(findings))


async def run_scheduled_scan(job) -> None:
    """Scan callback invoked by the scheduler when a scheduled job fires.

    Runs an auto-scan over the job's targets and persists the results, so a
    nightly scheduled scan feeds the same dashboard (findings/drift/KPIs) as
    a manual one. Wired to the scheduler in the app lifespan.
    """
    targets = list(getattr(job, "targets", None) or [])
    if not targets:
        logger.warning("Scheduled job %s has no targets — skipping", getattr(job, "id", "?"))
        return
    ...
    orch = EnterpriseOrchestrator(
        ...
        output_format="pdf",
        compliance_frameworks=list(getattr(job, "frameworks", None) or []),
    )
    logger.info("Scheduled scan starting: job=%s client=%s targets=%d", job.id, job.client_id, len(targets))
    await orch.run()
    _persist_auto_scan(orch, job.client_id)
...
@router.post(
    ...
    dependencies=[Depends(require_permission("scans:write"))],
)
async def start_auto_scan(req: AutoScanRequest, user: User | None = Depends(get_current_user)) -> AutoScanResponse:
    ...
    async def _run_scan():
        try:
            await orch.run()
            _persist_auto_scan(orch, req.client_id)
        except Exception:
...
@router.delete("/scans/auto/{scan_id}", dependencies=[Depends(require_permission("scans:write"))])
