...
class RunRequest(BaseModel):
    ...
    rich_events: bool = Field(
        False,
        description=(
            "Stream the FULL Kryon turn (determinism + pre_hooks + reflective loop) as "
            "rich AgentEvents (tool_started/tool_output/finding/assistant/done) instead of "
            "the raw agent stream. Opt-in; requires stream=True. Powers the Charm/TUI client."
        ),
    )
    free_run: bool = Field(
        False,
        description="Skip the deterministic engine + pre_hooks (KRYON_NO_DETERMINISM experiment). Rich path only.",
    )
...
class SessionCreateRequest(BaseModel):
    ...
    model: str | None = Field(
        None,
        max_length=200,
        description="Per-session model override (model_override). None → the server's configured model.",
    )
...
class ScheduleScanRequest(BaseModel):
    ...
    agent_key: str = Field("kryon", min_length=1, max_length=100)
    ...
    targets: list[str] = Field(default_factory=list, max_length=200)  # IPs/CIDRs/hosts to scan
    frameworks: list[str] = Field(default_factory=list, max_length=20)  # compliance frameworks
    start_hour: int | None = Field(None, ge=0, le=23)  # nightly window hour-of-day
