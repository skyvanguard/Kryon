...
class ScheduledJob(BaseModel):
    ...
    targets: list[str] = Field(default_factory=list)  # what to scan (IPs/CIDRs/hosts)
    frameworks: list[str] = Field(default_factory=list)  # compliance frameworks to assess
    cron: str = ""  # Cron expression (display only in the asyncio version)
    interval_seconds: int = 0  # Actual scheduling interval
    start_hour: int | None = None  # Hour-of-day (0-23) to run — the nightly window
...
def seconds_until_hour(start_hour: int, now: datetime) -> float:
    """Seconds from ``now`` until the next occurrence of ``start_hour``:00.

    Pure + testable. If it's already past that hour today, targets tomorrow.
    Returns 0 when start_hour is the current hour (run right away).
    """
...
class ScanScheduler:
    ...
    async def _run_loop(self, job: ScheduledJob, *, skip_first: bool = False) -> None:
        ...
        try:
            # Align the first run to the nightly window if a start_hour is set;
            # otherwise a restored interval job waits one interval before firing.
            if job.start_hour is not None:
                delay = seconds_until_hour(job.start_hour, datetime.now(timezone.utc))
                if delay > 0:
                    await asyncio.sleep(delay)
            elif skip_first and job.interval_seconds > 0:
