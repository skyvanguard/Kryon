        ...
        if role == "assistant" and msg.get("tool_calls"):
            for tc in msg["tool_calls"]:
                ...
                if tool_msg is None:
                    ...
                    # This is a PLACEHOLDER for a strict provider (assistant had a
                    # tool_call with no matching tool result). It is NOT the tool's
                    # output. The old "Auto-generated response for X" text read like a
                    # real (empty) result, so the model concluded the tool "failed"
                    # and re-issued the same call in a loop. Make it explicit so the
                    # model advances instead of retrying.
                    tool_msg = {
                        "role": "tool",
                        "tool_call_id": tool_id,
                        "content": (
                            f"[sin salida capturada para {tool_name}. Este marcador NO es la "
                            "respuesta del tool ni un error del target — el resultado no se "
                            "registró. No reintentes la misma llamada: usá la evidencia que ya "
                            "tenés o probá otra táctica.]"
                        ),
                    }
    ...
    for msg in final:
        ...
        if msg.get("role") == "tool":
            if msg.get("content") is None or msg.get("content") == "":
                # Same rationale as the synthesized placeholder above: mark it as a
                # non-result so the model doesn't read an empty tool message as "the
                # tool returned nothing useful" and retry.
                msg["content"] = (
                    f"[sin salida registrada para {msg.get('tool_call_id', 'unknown')}; "
                    "no es la respuesta del tool, no reintentes la misma llamada]"
                )
            continue
...
# Internal agents whose turns must NEVER render to the user's console — their
# output is consumed programmatically (e.g. the injection guardrail's verdict is
# read via result.final_output). Without this, the guardrail streams a raw JSON
# panel ("[1] Agent: Prompt Injection Detector … {contains_injection:…}") on top
# of the real run output. Match by agent name.
_SILENT_INTERNAL_AGENTS = frozenset({"Prompt Injection Detector"})
...
def cli_print_agent_messages(
    ...
    # Internal guardrail/utility agents are never shown to the user.
    if agent_name in _SILENT_INTERNAL_AGENTS:
        return
