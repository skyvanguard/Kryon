...
def update_json_locked(path: Path, mutator, *, default: Any, strict: bool = False) -> Any:
    ...
    On lock timeout: best-effort lock-free read-modify-write (correct for a SINGLE
    writer). With ``strict=True`` it re-raises ``Timeout`` instead — degrading to a
    lock-free write for a genuine claim/transition (``--concurrency N>1``) would
    reintroduce the exact TOCTOU race the lock exists to close (two workers both see
    'pending', both write 'running'). Strict callers must handle ``Timeout``."""
    ...
    try:
        ...
        if strict:
            logger.warning("state lock busy for %s — strict mode, refusing lock-free write", path)
            raise
