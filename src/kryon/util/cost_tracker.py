...
def get_model_name(model) -> str:
    ...
    return os.environ.get("KRYON_MODEL", "kryon-local")


def get_model_input_tokens(model) -> int:
    """Get max context window tokens for a model.

    ``KRYON_MODEL_MAX_TOKENS`` wins — it is how an aliased local model (e.g.
    ``qwen-unc`` served at 262k) declares its real window. Without it, an
    unrecognised alias fell through to the hard 32k floor, so a 38k-token turn
    read **121%** in the footer and tripped spurious compaction / truncation
    (the model then saw synthesized "Auto-generated response" tool messages and
    re-planned in a loop). After the override: the known-model substring map,
    then the 32k floor.
    """
    override = os.environ.get("KRYON_MODEL_MAX_TOKENS", "").strip()
    if override:
        try:
            v = int(override)
            if v > 0:
