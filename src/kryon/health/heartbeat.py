...
def _check_cve_cache() -> HealthCheckResult:
    ...
    The cache path comes from :func:`cve_validator._default_cache_path`
    — the SAME resolver the updater and the strict-mode validator use
    (``KRYON_CVE_CACHE_PATH`` override, else home-anchored
    ``~/.kryon/nvd_cache/cves.txt``). Deriving it here independently
    (a cwd-relative default) made ``doctor`` report "missing" even when
    the cache was populated, because the updater writes home-anchored.
    When the operator sets ``KRYON_CVE_CACHE_REQUIRED=true`` (F151 strict
    mode), an empty cache silently rejects every CVE-shaped finding —
    this check catches that misconfiguration up front.
    ...
    """
    from kryon.validation.cve_validator import _default_cache_path
