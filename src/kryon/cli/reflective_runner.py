...
def _planner_autoexec_enabled() -> bool:
    ...
    G7: also OFF for a capable model — auto-executing a recon directive without the
    model choosing it is the strongest form of "the planner imposes" (the live nmap in
    turn 1). A capable model decides its own actions; determinism stays the safety net."""
    from kryon.util.env import env_bool, is_capable_model, is_red_team  # noqa: PLC0415

    return env_bool("KRYON_PLANNER_AUTOEXEC") and is_red_team() and not is_capable_model()
...
def _recommendation_signature(rec: Any) -> str:
    ...
    # 200 (not 2000) so benign tail drift — a longer payload, an extra flag — doesn't read as a
    # new recommendation; the stall detector keys on the stable head (tool + first 200 args chars).
...
# Web-app foothold signals — a web engagement rarely yields a Linux/Windows shell
# prompt, so the shell-only regexes missed a legitimate web foothold and the
# premature-gate then rejected a correct close (observed on web labs won by SQLi/IDOR).
_WEB_FOOTHOLD_MARKERS = ("webshell", "flag{", "rce-confirmed", "sqli-confirmed", "auth-bypass")


def _has_foothold(facts: ExtractedFacts) -> bool:
    """True when the agent has materially cracked past recon.

    Foothold = creds confirmed, hashes extracted, a shell-prompt marker, a Linux
    privesc primitive (``privesc:*`` from _extract_privesc — sudo/SUID/caps/ssh-key),
    or a web-foothold marker (webshell/flag/confirmed-exploit). Knowing usernames or
    open ports alone is NOT foothold (that's recon).
    ...
    for hint in facts.hints:
        if hint.startswith("privesc:"):
            return True
        low = hint.lower()
        if any(m in low for m in _WEB_FOOTHOLD_MARKERS):
            return True
...
def _render_graph_state(attack_graph: Any, accumulated_facts: Any, findings: list | None) -> str:
    """Render the PROVEN attack-graph state (held capabilities, demonstrated
    chains, goal-directed pursuit) for injection into the reflection turn of the
    MAIN run.

    This is the same rich state ``_reasoning_next_action`` feeds its one-shot
    sub-planner — but handed to the model that actually DRIVES the kill-chain,
    so a capable model composes over the full graph (chains + residual gap)
    instead of only the single distilled ``NextActionRecommendation`` line. The
    harness gap this closes: the map of proven paths went to a one-pass planner
    while the model doing the digging got just one arrow.

    Returns '' when nothing is proven yet (no chains, no pursuable route), so a
    barren graph never adds noise. Fully guarded — graph rendering must never
    break the run.
    """
    try:
        from kryon.intelligence.attack_path import (  # noqa: PLC0415
            build_attack_graph,
            format_attack_paths,
            plan_path_pursuit,
        ...
        paths = format_attack_paths(graph)
        pursuit = plan_path_pursuit(graph, target=os.getenv("KRYON_ATTACK_TARGET") or None)
        if not paths and not pursuit:
            return ""
        block = [
            "\n## 🕸️ Attack-graph — proven state (compose FROM this, do NOT re-recon)\n",
...
def _build_reflection_prompt(
    *,
    ...
    # Capable-model gate, resolved once: a frontier model that DRIVES the
    # kill-chain gets nudges it can override; the 4B-local regime gets hard
    # directives (it otherwise improvises past them and loops). Banca-safe
    # contract — the downgrade only applies under is_capable_model().
    ...
    if next_action is not None:
        ...
        # Fase 1 — Adviser escalation (PentAGI Mentor). A stalling/stuck loop
        # means the model choosing freely has FAILED, so promote the concrete
        # recommendation to the imperative top slot regardless of its confidence:
        # it reads a specific next step before re-anchoring on its looping chain.
        # A high-conf rec is already top-slotted, so don't double-wrap it.
        _loop_now = stall_detected or (stuck_record is not None)
        if next_action.confidence >= 0.85 and not _capable:
            # 4B regime: hard directive top-slotted so it reads before its own
            # chain can re-anchor its reasoning. A capable model instead reads
            # the attack-graph + facts first and DECIDES — its distilled rec
            # drops to next_action_block (below), never top-slotted.
            next_action_top = rendered
        elif _loop_now:
            # Gated tone: for the 4B regime it's an order; for a capable model
            # it's a strong nudge it can still override (banca-safe contract).
            verb = (
                "Considerá ejecutar esta acción antes de continuar tu cadena"
                if _capable
                else "EJECUTÁ ESTA ACCIÓN AHORA en vez de repetir lo anterior"
            )
            next_action_top = f"\n🔁 **ESTÁS REPITIENDO SIN PROGRESO.** {verb}:\n{rendered}"
        else:
    ...
    # Fase 1 — hackingBuddyGPT anti-repeat. Surface the ACTUAL (tool, args)
    # already executed with an explicit "do not repeat" — dynamic and accurate,
    # unlike the degen block's hardcoded examples ("no nmap otra vez"). Universal:
    # helps any model avoid re-issuing a call whose result it already has.
    # Deduped, most-recent-first, capped.
    executed_block = ""
    if tool_history:
        seen_exec: list[str] = []
        for r in reversed(tool_history[-10:]):
            entry = f"  • `{r.tool_name}` {(r.args_preview or '').strip()[:90]}"
            if entry not in seen_exec:
                seen_exec.append(entry)
        if seen_exec:
            executed_block = (
                "\n## ⛔ Ya ejecutado — NO repitas estos (mismos args = loop, "
                "el resultado ya lo tenés arriba)\n" + "\n".join(seen_exec) + "\n"
            )
    ...
    stall_block = ""
    if stall_detected and _capable:
        # Capable model: the PLANNER has repeated the same rec while facts stayed
        # flat — but a frontier model may legitimately be varying its own
        # hypothesis. Don't order a letter-for-letter copy (that treats
        # exploration as a 4B loop). Frame it as a decision it owns.
        stall_block = (
            "\n🛑 **STALL — planner repeating, facts flat** 🛑\n\n"
            f"For {_DEFAULT_STALL_THRESHOLD}+ reflection turns the planner has "
            "proposed the same next link and no new structured intel landed in "
            "ExtractedFacts. Step back and decide:\n"
            "  (A) If your current chain isn't actually advancing, break the "
            "repetition: integrate the planner's recommendation above, or a "
            "concretely stronger move you can justify — but a DIFFERENT probe "
            "than the one that isn't moving the facts.\n"
            "  (B) If this path is genuinely exhausted, emit the final summary "
            "now: what you tried, what each attempt returned, and the specific "
            "reason it can't reach impact. The operator decides next.\n"
        )
    elif stall_detected:
        stall_block = (
    ...
    premature_block = ""
    if premature_summary_detected and _capable:
        # Capable model: the close looks premature (no foothold + few calls), but
        # a frontier model may have a real reason to stop. Keep the brake, but
        # give it the justified-exit path instead of an absolute "NO TERMINES".
        premature_block = (
            "\n🛑 **CIERRE POSIBLEMENTE PREMATURO** 🛑\n\n"
            "Emitiste un resumen / conclusión con pocos tool calls y SIN "
            "foothold confirmado (no hay creds, hashes, uid=, ni shell en los "
            "facts). El operator pidió pentest activo, no recon pasivo.\n\n"
            "Antes de cerrar, decidí:\n"
            "  (A) Si todavía hay superficie sin probar, formulá UNA hipótesis "
            "concreta y ejecutá su tool call ahora — no cierres con superficie "
            "sin tocar.\n"
            "  (B) Si genuinamente agotaste los vectores explotables, cerrá — "
            "pero justificá en el resumen POR QUÉ no hay foothold (qué vectores "
            "probaste y qué devolvió cada uno).\n"
        )
    elif premature_summary_detected:
        premature_block = (
    ...
    # Q5 "¿PARAR? → resumen" es un off-ramp recurrente hacia narrar/reportar. Para un
    # capable (que debe CONDUCIR hacia impacto) lo reencuadramos: la pregunta de cierre
    # pasa a "¿lograste impacto / agotaste vectores?" en vez de "¿deberías parar?". El 4B
    # conserva las preguntas originales (necesita el off-ramp para no loopear).
    if _capable:
        reflection_questions = (
            "**Antes de continuar**, respondé internamente (guialo en tu razonamiento):\n\n"
            "1. ¿Qué **aprendí** que NO sabía antes?\n"
            "2. ¿Qué **hipótesis** de explotación sigue sin probar?\n"
            "3. ¿Cuál es el **próximo eslabón** hacia impacto (dump→creds, cred→login, "
            "upload→RCE, IDOR→escritura)? Ejecutá ESE, no lo narres.\n"
            "4. ¿Necesito una **skill o tool** que no tengo? (invocá `request_skill`/`tool_search`)\n"
            "5. ¿Ya **lograste impacto real** (RCE/creds/dump/admin) o **agotaste** los vectores "
            "explotables? Solo entonces emití el resumen. Si no, continuá encadenando.\n"
        )
    else:
        reflection_questions = (
            "**Antes de continuar**, respondé internamente (no necesitás emitir mensaje "
            "separado — guialo en tu razonamiento del próximo turn):\n\n"
            "1. ¿Qué **aprendí** en estos últimos turns que NO sabía antes?\n"
            "2. ¿Qué **hipótesis** sigue sin verificar?\n"
            "3. ¿Estoy **progresando** hacia el objetivo? (sí / no / atascado)\n"
            "4. ¿Necesito una **skill o tool** que no tengo? "
            "(si sí, invocá `request_skill(topic='...')` para obtenerla)\n"
            "5. ¿Debería **PARAR** ahora? (sí / no / por qué)\n\n"
            "Si decidís parar → emití el resumen final del objetivo del operador.\n"
            "Si no → continuá con el tool call que aporte MÁS signal nuevo, "
            "NO repitas tools ya invocadas con los mismos args.\n"
        )
    ...
    return (
        ...
        f"Última observación (preview):\n```\n{last_output_summary[: resolve_context_budget(500)]}\n```\n"
        f"{stuck_block}\n"
        f"{reflection_questions}"
    )
...
def _chunk_text_from_capture(capture_hooks: Any) -> str:
    ...
    for item in items:
        ...
        # Prefer the larger fact slice (16KB) over the 500-char display preview so
        # the extractor sees the privesc/hash/port lines that live past char 500.
        output_preview = item.get("output_facts") or item.get("output_preview", "")
...
class ItemCaptureHooks:
    ...
    def __init__(self, event_sink: Any = None, attack_graph: Any = None) -> None:
        ...
        # Optional EventSink (services.agent_events). When set, tool_started /
        # tool_output events are emitted LIVE for a front-end (SSE stream / Go TUI).
        # None (the default) = no-op, so the REPL path is byte-for-byte unchanged.
        self._sink = event_sink
        self._start_ts: dict[str, float] = {}
        self._step = 0
        # v2 confirm-then-add-edge: the persistent AttackGraph grown LIVE — when a
        # validate_* tool CONFIRMS here, the proven edge is recorded the instant it
        # happens (see on_tool_end), not inferred post-hoc from finished findings.
        self._attack_graph = attack_graph

    def _emit(self, event: Any) -> None:
        """Best-effort emit — event delivery must NEVER break the agent run."""
    ...
    async def on_agent_end(self, context: Any, agent: Any, output: Any) -> None:
        ...
        self.captured_items.append(
            {
                "type": "agent_end",
                "output_preview": str(output)[: resolve_context_budget(500)] if output else "",
                "timestamp": time.time(),
            }
        )
    ...
    async def on_tool_start(self, context: Any, agent: Any, tool: Any) -> None:
        ...
        self._start_ts[tool_name] = entry["timestamp"]
        if self._sink is not None:
            from kryon.services.agent_events import tool_started

            self._step += 1
            self._emit(tool_started(tool_name, step_id=self._step))
    ...
    async def on_tool_end(self, context: Any, agent: Any, tool: Any, result: Any) -> None:
        ...
        if idx is not None and idx < len(self.captured_items):
            _out = str(result)
            self.captured_items[idx]["output_preview"] = _out[: resolve_context_budget(500)]
            # Fact synthesis needs far more than the 500-char preview: the privesc
            # line (`sudo -l` NOPASSWD), the hashes (secretsdump), and later nmap
            # ports live past char 500 and were never reaching accumulated_facts —
            # the model then "forgot" what it had discovered. Keep a large slice for
            # the fact extractor; the preview stays 500 for chain/display/writeback.
            self.captured_items[idx]["output_facts"] = _out[:16000]

            # v2 confirm-then-add-edge — the "validate each link" moment: a
            # validate_* tool that just CONFIRMED an exploit grows the persistent
            # attack graph HERE, live, so the next reasoning turn sees a proven
            # path edge instead of waiting for the post-hoc findings pass.
            if self._attack_graph is not None:
                try:
                    from kryon.intelligence.attack_path import add_confirmed_validation

                    add_confirmed_validation(self._attack_graph, tool_name, _out)
                except Exception:  # noqa: BLE001 — graph growth must never break the run
                    pass

            if self._sink is not None:
                from kryon.services.agent_events import tool_output

                _dur = max(0.0, time.time() - self._start_ts.get(tool_name, time.time()))
                _low = _out[:200].lower()
                _status = "error" if ("error" in _low or "exit code" in _low or "failed" in _low) else "ok"
                _lines = _out.count("\n") + 1 if _out.strip() else 0
                # Collapse large output from the stream body (front-end fetches it
                # on demand by step_id) — mirrors the REPL's /show threshold.
                self._emit(
                    tool_output(
                        tool_name,
                        status=_status,
                        duration_s=_dur,
                        summary=f"{_lines} lines" if _lines > 1 else "",
                        output=_out[:4000],
                        step_id=self._last_call_idx.get(tool_name),
                        collapsed=_lines > 8,
                    )
                )
...
def _derive_intent(initial_input: str | list[Any]) -> str:
    """Pull the user's objective text out of the agent's initial input (a prompt string or an SDK
    message list) so the planner can gate keyword rules on it. Best-effort; ``""`` when nothing readable."""
    if isinstance(initial_input, str):
        return initial_input
    if isinstance(initial_input, list):
        parts: list[str] = []
        for msg in initial_input:
            content = msg.get("content") if isinstance(msg, dict) else msg
            if isinstance(content, str):
                parts.append(content)
            elif isinstance(content, list):  # content blocks [{type:text, text:...}, …]
                parts.extend(b.get("text", "") for b in content if isinstance(b, dict))
        return " ".join(p for p in parts if p)
    return ""


# FASE — forced closing synthesis. A capable model explores well but rarely
# self-terminates: it keeps calling tools until the reflective loop cuts it off at
# max_total_turns / wall budget, so final_output stays "" and the report shows
# "(el agente no produjo salida final)" despite 18 coherent tool calls. Verified
# live (Qwen3.5-122B vs example.com: crawled the whole site + searched, never
# summarized). This forces ONE closing turn — toolless, so it can only narrate —
# asking for the final report over everything it observed.
_FINAL_SYNTHESIS_PROMPT = (
    "El presupuesto de exploración se agotó. Con TODO lo que observaste en esta "
    "sesión (los detectores deterministas ya confirmados + los resultados de tus "
    "tool calls), producí AHORA el reporte final. NO intentes llamar más "
    "herramientas — no tenés ninguna disponible en este turno.\n\n"
    "Usá EXACTAMENTE esta estructura, en este orden (veredicto PRIMERO):\n\n"
    "## Veredicto\n"
    "Una línea: riesgo global (CRÍTICO / ALTO / MEDIO / BAJO / LIMPIO) + severidad "
    "pico. ¿Hay algo explotable de impacto real, o el target quedó limpio?\n\n"
    "## Hallazgos (N)\n"
    "Ordenados de mayor a menor severidad. Uno por bloque:\n"
    "▸ SEVERIDAD · <título corto>\n"
    "  Evidencia: <la prueba concreta que lo confirma>\n"
    "  Fix: <remediación en una línea>\n"
    "Si no hay ninguno: escribí '## Hallazgos (0)' y 'Ninguno de impacto.'\n\n"
    "## Alcance probado\n"
    "Qué se ejecutó realmente (discovery, headers, email records, paths, tools).\n\n"
    "## Pendiente de verificar\n"
    "Hipótesis no confirmadas: qué faltó probar y por qué.\n\n"
    "Reglas: NO repitas el reporte. NO inventes hallazgos — si algo no está "
    "confirmado va en 'Pendiente', nunca en 'Hallazgos'. Sé honesto: si el target "
    "quedó limpio más allá de lo determinista, decilo."
)


async def _force_final_synthesis(agent: Any, base_history: Any, run_config: Any) -> str:
    """Force a closing synthesis turn when the reflective loop exhausted its turn/wall
    budget without the agent ever producing a final summary. Clones the agent WITHOUT
    tools (synthesis physically can't call anything) and asks for the report over the
    accumulated history. Returns the synthesized text, or "" on any failure (never
    raises — it's a best-effort safety net). Disable with KRYON_FINAL_SYNTHESIS=false."""
    if os.environ.get("KRYON_FINAL_SYNTHESIS", "true").strip().lower() == "false":
        return ""
    try:
        import dataclasses
        ...
        if isinstance(base_history, list) and base_history:
            synth_input: Any = [*base_history, {"role": "user", "content": _FINAL_SYNTHESIS_PROMPT}]
        else:
            synth_input = _FINAL_SYNTHESIS_PROMPT

        # Belt-and-suspenders: tool_choice="none" on top of the toolless clone.
        ...
        try:
            ...
            synth_cfg = (
                dataclasses.replace(run_config, model_settings=ms) if run_config else RunConfig(model_settings=ms)
            )
        except Exception:  # noqa: BLE001 — settings copy is optional, fall back to original cfg
            synth_cfg = run_config

        result = await Runner.run(toolless, input=synth_input, max_turns=1, run_config=synth_cfg)
        fo = getattr(result, "final_output", None)
        text = str(fo).strip() if fo else ""
        if not text:
            # Capable thinking-model may have left the summary in reasoning_content;
            # _run_impl promotes it, but if the clone path missed it, salvage here.
            text = _extract_chunk_text(result).strip()
        if text and os.environ.get("KRYON_REFLECT_DEBUG", "").lower() in ("1", "true", "yes"):
            print(f"\n🧾 [reflective-runner] forced final synthesis produced {len(text)} chars")
        return text
    except Exception as e:  # noqa: BLE001 — synthesis is best-effort, must never break the run
        logger.debug("forced final synthesis failed: %s", e)
        return ""


async def _reasoning_next_action(
    agent: Any,
    run_config: Any,
    accumulated_facts: Any,
    prior_args: list[str],
    findings: list | None = None,
    attack_graph: Any = None,
) -> NextActionRecommendation | None:
    """#5 — reasoning-driven next action from the live attack-graph state.

    Only fires for a capable model (KRYON_CAPABLE_MODEL); a small local model
    keeps the deterministic rules planner. Reuses the loop's own model via a
    tool-less one-shot (same pattern as _force_final_synthesis) — no separate
    plumbing. Returns None on disabled / failure / decline, so the caller falls
    back to the rules planner. Strictly additive: default behavior unchanged.
    """
    from kryon.util.env import is_capable_model  # noqa: PLC0415

    if not is_capable_model():
        return None
    _ = (agent, run_config)  # kept for signature compatibility with the call sites
    try:
        ...
        from kryon.intelligence.reasoning_planner import (  # noqa: PLC0415
            _build_prompt,
            _parse_action,
            summarize_findings,
        )

        # Prefer the PERSISTENT graph (v2): it already holds the edges the
        # validation hook confirmed LIVE this run. Fold in the latest facts +
        # confirmed findings (idempotent — add_edge dedups) so it reflects the
        # full proven state, then let the model chain toward the residual gap.
        # Falls back to a fresh build when no persistent graph is threaded in.
        if attack_graph is not None:
            graph = populate_attack_graph(attack_graph, accumulated_facts, findings or [])
        else:
            graph = build_attack_graph(accumulated_facts, findings or [])
        ...
        # v4: KRYON_ATTACK_TARGET lets the operator pin a specific impact goal
        # (rce / admin / db_takeover / account_takeover / root / data_exfil).
        ...
        rec = _parse_action(text)
        if rec is not None and os.environ.get("KRYON_REFLECT_DEBUG", "").lower() in ("1", "true", "yes"):
            print(f"\n🧠 [reasoning-planner] proposed {rec.tool}({(rec.args or '')[:60]}) c={rec.confidence:.2f}")
        return rec
    except Exception as e:  # noqa: BLE001 — reasoning proposal is best-effort
        logger.debug("reasoning planner proposal failed: %s", e)
        return None
...
async def run_with_reflection(
    ...
    *,
    ...
    # Record the run's objective so execute_planner_directive passes it to plan_next_action (the wpscan
    # and other keyword-gated rules fire on intent OR facts; without it they only fire when the markers
    # land in facts, which a web_fetch JSON envelope doesn't surface — the planner then skips the WP chain).
    _run_intent = _derive_intent(initial_input)
    _set_planner_intent(_run_intent)
    turns_used = 0
    tool_history: list[_ToolCallRecord] = []
    ...
    # v2 confirm-then-add-edge — a persistent AttackGraph the capture hook grows
    # LIVE as validate_* tools confirm exploits. Shared object; grown in
    # on_tool_end, seeded below, re-folded + consumed in _reasoning_next_action.
    from kryon.intelligence.attack_graph import AttackGraph as _AttackGraph  # noqa: PLC0415
    ...
    # Seed the persistent attack graph from the deterministic-phase facts +
    # confirmed findings, so it starts with the post-hoc proven edges and then
    # only GROWS from there as live validations land.
    try:
        from kryon.intelligence.attack_path import populate_attack_graph as _populate_ag  # noqa: PLC0415

        _populate_ag(attack_graph, accumulated_facts, findings)
    except Exception as _seed_exc:  # noqa: BLE001 — never break the run on graph seeding
        logger.debug("attack-graph seed failed, starting empty: %s", _seed_exc)
    ...
    from kryon.util.env import is_capable_model, is_local_llm

    # Reasoning-max (capable) + local MoE are SLOW per chunk (large prefill + long
    # thinking); the 180s remote default killed such runs before ANY output (observed
    # 2× on Juice Shop, even at the local 900s). Give capable OR local a generous
    # per-chunk timeout so a slow reasoning turn isn't cut mid-thought. Wall budget
    # (if set) still bounds the whole run; operators override via KRYON_CHUNK_TIMEOUT_S.
    _run_capable = is_capable_model()
    _default_chunk_timeout = 1800.0 if (is_local_llm() or _run_capable) else 180.0
    # Capable-aware hard stuck-loop abort. _is_stuck is RESULT-INDEPENDENT (it matches
    # only tool+args_hash), so it fires even when a capable model re-issues a probe with
    # intent while chaining. Give capable more consecutive-chunk grace before the hard
    # abort (the 4B — which truly loops one URL ~48× — keeps the tight trigger).
    _stuck_abort_trigger = (_DEFAULT_STUCK_ABORT_TRIGGER + 2) if _run_capable else _DEFAULT_STUCK_ABORT_TRIGGER
    try:
    ...
    # A stuck/repeated malformed tool_call used to fall through to `raise`, ending the whole run
    # (THM Internal: Ornith re-emitted a 291-username inline list → llama.cpp 500 → run died at turn 20
    # with all the brute-force progress lost). Instead, hard-reset the model off the stuck approach and
    # CONTINUE, bounded; only after this many resets do we end gracefully with a partial report.
    _malformed_recoveries = 0
    _MAX_MALFORMED_RECOVERIES = 2

    # ONE StuckDetector for the whole chunked run — else Runner.run builds a
    # fresh one per chunk (~4 turns) and its window resets before any multi-turn
    # loop (alternation / re-derivation) can trip abort. Passed to every chunk.
    try:
        from kryon.sdk.agents.run import _build_stuck_detector

        _persistent_stuck = _build_stuck_detector()
    except Exception:  # noqa: BLE001 — never break the run over the loop guard
        _persistent_stuck = None

    # Best-effort accumulated conversation, kept fresh for the forced-synthesis
    # fallback at loop exit (the max_turns break path skips the in-loop assignments).
    base_history: Any = None
    ...
    while turns_used < max_total_turns:
        ...
        try:
            _chunk_coro = Runner.run(
                ...
                stuck_detector=_persistent_stuck,  # L1 — persist across chunks
            )
            # Cap this chunk's timeout at the wall budget REMAINING, so a chunk
            # that starts just under the boundary can't run a further full
            # _chunk_timeout_s and overshoot the wall budget ~2x. Floor at 5s so
            # a nearly-exhausted budget still lets the chunk start + unwind.
            _chunk_deadline = _chunk_timeout_s
            if _wall_budget_s > 0:
                _remaining = _wall_budget_s - (time.monotonic() - _loop_start)
                _chunk_deadline = max(5.0, min(_chunk_timeout_s, _remaining))
            if _chunk_deadline > 0:
                result = await asyncio.wait_for(_chunk_coro, timeout=_chunk_deadline)
            else:
            ...
            if "MaxTurns" in ename:
                ...
                try:
                    ...
                    # #5 — reasoning-driven first (capable model), rules fallback.
                    next_action_mt = await _reasoning_next_action(
                        agent, run_config, accumulated_facts, prior_args_mt, findings, attack_graph
                    )
                    if next_action_mt is None:
                ...
                _autoexec_block_mt = await _maybe_autoexec(
                    next_action_mt,
                    _host_mt,
                    enabled=_planner_autoexec_enabled(),
                ...
                if next_action_mt is not None:
                    ...
                    # Fix ② (MaxTurns parity): a capable model reads the graph +
                    # facts first, then decides — its distilled rec is NOT hoisted
                    # to the imperative top slot. The 4B regime keeps the top slot.
                    if next_action_mt.confidence >= 0.85 and not is_capable_model():
                        next_action_top_mt = rendered_mt
                    else:
                        next_action_block_mt = rendered_mt

                # Fix ① (MaxTurns parity): hand the driving model the proven
                # attack-graph state on the budget-exhausted path too.
                graph_block_mt = _render_graph_state(attack_graph, accumulated_facts, findings)
                reflection_msg = (
            ...
            if _is_tool_json_500 and not _repeated_500 and _server_errors + 1 < _MAX_SERVER_ERRORS:
                ...
                continue
            # Unrecoverable malformed tool_call: the model re-emitted the SAME one, or we hit
            # _MAX_SERVER_ERRORS distinct ones. Don't crash the run — the enum/brute-force so far is
            # real progress. Hard-reset the model off the stuck approach and CONTINUE (bounded); only
            # after _MAX_MALFORMED_RECOVERIES do we end gracefully with a partial report (never raise).
            if _is_tool_json_500:
                # Salvage what the chunk ran before the 500 (tools that succeeded are real intel).
                accumulated_facts = _salvage_chunk_intel(capture_hooks, accumulated_facts, tool_history)
                _malformed_recoveries += 1
                if _malformed_recoveries <= _MAX_MALFORMED_RECOVERIES:
                    # A genuinely-different next approach should get fresh retries, so clear the
                    # per-error fingerprint + counter; bound the whole thing via _malformed_recoveries.
                    _last_500_fingerprint = None
                    _server_errors = 0
                    turns_used += 1  # count the wasted turn so the run always progresses
                    base_history = (
                        current_input
                    ...
                    reset_nudge = (
                        "🛑 ABANDONÁ por completo ese enfoque — tu tool_call se rechazó por JSON inválido "
                        "(inlineaste un payload/lista enorme dentro del argumento). NO lo repitas.\n"
                        "Tomá UNA acción DISTINTA y MÍNIMA: un solo comando corto en run_command, JSON "
                        "estrictamente válido, sin listas largas ni contenido multilínea inline. Dejá que "
                        "la HERRAMIENTA itere, no generes la lista vos. Ej. brute-force en UNA línea:\n"
                        "  hydra -l <user> -P /usr/share/wordlists/rockyou.txt <host> http-post-form "
                        "'<path>:<form-params>:<fail-string>'\n"
                        "  wpscan --url <url>/blog --usernames <user> --passwords /usr/share/wordlists/rockyou.txt\n"
                        "Mantené cada tool_call chico y con JSON válido."
                    )
                    current_input = base_history + [{"role": "user", "content": reset_nudge}]
                    logger.warning(
                        "reflective runner: recovering from stuck malformed tool_call "
                        "(recovery %d/%d) at turn %d — hard reset + continue",
                        _malformed_recoveries,
                        _MAX_MALFORMED_RECOVERIES,
                        turns_used,
                    )
                    continue
                # Resets exhausted → end gracefully with a partial report (no crash, progress kept).
                logger.warning(
                    "reflective runner: malformed tool_call unrecoverable after %d resets at turn %d "
                    "— ending with a partial report instead of raising",
                    _malformed_recoveries,
                    turns_used,
                )
                _partial_note = (
                    "⚠️ El agente quedó atascado emitiendo tool_calls con JSON inválido (payload inline "
                    "demasiado grande). Los hallazgos abajo son PARCIALES y requieren verificación."
                )
                if last_result is not None:
                    try:
                        prior = getattr(last_result, "final_output", "") or ""
                        last_result.final_output = (prior + "\n\n" + _partial_note).strip()  # type: ignore[attr-defined]
                    except Exception:  # noqa: BLE001
                        pass
                else:
                    from types import SimpleNamespace

                    last_result = SimpleNamespace(final_output=_partial_note, new_items=[])
                break
        ...
        if not _has_pending_tool_calls(result):
            ...
            if not final_output_text.strip() and empty_output_count < _MAX_EMPTY_OUTPUT_FALLBACKS:
                ...
                try:
                    # #5 — reasoning-driven first (capable model), rules fallback.
                    _next_eo = await _reasoning_next_action(
                        agent, run_config, accumulated_facts, _prior_eo, findings, attack_graph
                    )
                    if _next_eo is None:
                        _next_eo = plan_next_action(accumulated_facts, prior_tool_args=_prior_eo, intent=_run_intent)
        ...
        if stuck is not None:
            ...
            if consecutive_stuck_count >= _stuck_abort_trigger:
                logger.warning(
        ...
        # #5 — reasoning-driven proposal first (capable model only); the rules
        # planner is the fallback. The model proposes a hypothesis from the live
        # attack-graph state; it is still executed and its edge confirmed by the
        # validators before it counts (the model never asserts an edge is real).
        try:
            next_action = await _reasoning_next_action(
                agent, run_config, accumulated_facts, prior_args, findings, attack_graph
            )
        except Exception as e:  # noqa: BLE001 — reasoning proposal is best-effort
            logger.debug("reasoning next-action failed: %s", e)
        ...
        try:
            ...
            if next_action is not None and next_action.confidence >= 0.92:
                _adherence.record_injection(turn=turns_used, tool=next_action.tool, confidence=next_action.confidence)
        ...
        # Fix ① — hand the model that DRIVES the kill-chain the same rich
        # attack-graph state (proven chains + goal-directed pursuit) that until
        # now only fed the one-shot sub-planner. A capable model composes over
        # the full graph instead of a single distilled line.
        graph_block = _render_graph_state(attack_graph, accumulated_facts, findings)
    ...
    if last_result is not None:
        ...
        if operator_input_requested:
            try:
                ...
                pass
        # Forced closing synthesis — the loop exhausted its budget (max_turns / wall)
        # without the agent ever summarizing (it kept calling tools until cut off), so
        # final_output is empty. Ask for ONE toolless closing turn so the report shows
        # the agent's analysis instead of "(el agente no produjo salida final)". Skipped
        # when the operator-input fallback already populated final_output.
        elif not str(getattr(last_result, "final_output", "") or "").strip() and (
            capture_hooks.to_chain() or accumulated_items
        ):
            _synth = await _force_final_synthesis(agent, base_history, run_config)
            if _synth:
                try:
                    last_result.final_output = _synth  # type: ignore[attr-defined]
                except (AttributeError, Exception):  # noqa: BLE001
                    pass
