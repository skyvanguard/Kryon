...
Usage:
    ...
    kryon engage 127.0.0.1 \\
        ...
        --client example \\
        --engagement-id example-demo-2026-04-15
...
class Finding:
    ...
    # F210 — how directly the emitter observed the flagged condition. Drives
    # the confidence band in scoring.confidence so "deterministic" stops
    # meaning "infallible 1.0" for banner/version-inferred checks:
    #   "confirmed" — probed the actual condition (default; 1.0)
    #   "heuristic" — inferred from a banner / config string / version match (0.6, needs review)
    #   "inferred"  — version→CVE mapping without confirming the vulnerable path (0.4, needs review)
    verification_level: str = "confirmed"
    # Originating engine ("openvas" | "cinc" | "lynis" | "kryon-native" | ...).
    # Empty by default; the dedup layer falls back to rule_id-prefix inference.
    # Enables accurate cross-engine corroboration (esp. for CVE findings whose
    # rule_id alone can't distinguish the source).
    source: str = ""
    # T3-A12: the exact vulnerable endpoint/param (e.g. "/download?file="). Web probe
    # ids are static per class, so without this the model gets "LFI on host X" without
    # WHICH endpoint to exploit, and the dedup key can't tell two instances apart.
    url: str = ""


# Canonical rank (was a byte-copy — a lowercase severity fed make_finding rank 99).
from kryon.util.severity import (
    SEVERITY_RANK as _SEV_RANK,  # noqa: E402
    severity_rank as _severity_rank,  # noqa: E402
)
...
def make_finding(
    ...
    *,
    ...
    return Finding(
        ...
        severity_rank=_severity_rank(severity),  # case-normalizing (lowercase → correct rank)
...
def _nmap_host_port(target: str) -> tuple[str, str]:
    """Extract a bare host/IP (+ optional explicit port) from a target that
    may be a URL.

    nmap fails to resolve ``http://host:port/path`` ("Failed to resolve",
    0 hosts scanned) — it needs the bare host. Any explicit port in the target
    is returned so the caller can fold it into the scan, so non-canonical
    service ports (e.g. Juice Shop on ``:3000``) are covered even when
    ``--top-ports`` might miss them.
    """
    ...
    if "://" in t:
        p = urlparse(t)
        return (p.hostname or t, str(p.port) if p.port else "")
    # bare host[:port] — skip IPv6 (bracketed or with multiple colons)
...
def _build_engage_nmap_cmd(target: str, ports: str = "") -> list[str]:
    ...
    # nmap needs a bare host/IP, not a URL — extract it and fold any explicit
    # URL port into the scan (fixes "0 puertos" on http://host:port targets).
    host, _url_port = _nmap_host_port(target)
    if _url_port and _url_port not in ports:
        ports = f"{ports},{_url_port}".strip(",") if ports.strip() else _url_port
    ...
    # F202.T: --ports overrides the default. Default is top-1000 (T3-A3: top-100
    # missed redis/mongo/winrm/elastic and high app-ports where the THM foothold
    # lives, so the service never materialized and its probes never ran).
    # KRYON_NMAP_TOP_PORTS overrides the count; KRYON_NMAP_FULL=1 → -p- (all 65535).
    _nmap_full = os.environ.get("KRYON_NMAP_FULL", "").strip().lower() in ("1", "true", "yes", "on")
    _top_ports = os.environ.get("KRYON_NMAP_TOP_PORTS", "1000").strip() or "1000"
    if ports.strip():
        ...
        if sanitized:
            ...
            cmd.extend(["--top-ports", _top_ports])  # fallback
    elif _nmap_full:
        cmd.append("-p-")
    else:
        cmd.extend(["--top-ports", _top_ports])
...
def _extend_timeout_for_throttle(timeout_s: int) -> int:
    ...
    Surfaced by the internal testing pilot 2026-05-18 against .106:
...
def _serves_http(svc: DiscoveredService) -> bool:
    """True when the service speaks HTTP — robust to nmap misclassifying a
    non-canonical web port.

    nmap ``-sV`` labels Juice Shop on ``:3000`` as ``ppp`` (the port's legacy
    IANA name), so a label/port gate alone silently skips ALL web checks
    (plaintext, cookies, the F57 webexploit sweep). This checks the label, a
    broadened web-port list, then falls back to a cheap HTTP probe so ANY web
    port is covered.
    """
    if (svc.service or "").lower() in ("http", "http-proxy", "https", "http-alt", "ssl/http"):
        return True
    if svc.port in (80, 443, 8080, 8443, 3000, 3001, 4200, 5000, 8000, 8081, 8888, 9000, 9200):
        return True
    for scheme in ("http", "https"):
        try:
            out = subprocess.run(
                ["curl", "-skSI", "--max-time", "4", f"{scheme}://{svc.host}:{svc.port}/"],
                capture_output=True,
                text=True,
                timeout=6,
                check=False,
            ).stdout
            if out.strip().upper().startswith("HTTP/"):
                return True
        except Exception:
            continue
    return False
...
def _check_password_manager(svc: DiscoveredService) -> Finding | None:
    ...
    Surfaced by the internal testing pilot 2026-05-18 against .99, where the
...
def _http_get(url: str, *, timeout_s: int = 5) -> tuple[int, str]:
    ...
    F202.M (internal testing .106 2026-05-19): use `--compressed` so curl
...
def _check_python_simplehttp_exposed(svc: DiscoveredService) -> Finding | None:
    ...
    Surfaced by the internal testing pilot on 2026-05-18 against TORRE_SVR.200,
...
# Surfaced by internal testing .205 (Domain Controller example.com): the
...
def _derive_dns_zone_candidates(host: str) -> list[str]:
    ...
    Strategy:
      1. PTR query: ask the target for its own PTR record. If it
         returns `dc01.example.local.` we extract `example.com`.
      2. Reverse zone: build the in-addr.arpa zone from the target IP
         (e.g. 192.0.2.205 -> `2.0.192.in-addr.arpa`).
...
_DNS_SNOOP_PROBES: tuple[str, ...] = (
    ...
    "dropbox.com",
    "github.com",
    # Banking Paraguay (example context)
    "bcp.com.py",
    "bancard.com.py",
    "mercadopago.com.py",
    # Payments global
...
#   nslookup 192.0.2.5 <dns>  -> dc02.example.local
#   nslookup 192.0.2.50 <dns> -> bastion-ubuntu.example.local
#   nslookup 192.0.2.150 <dns> -> postgres-prod.example.local
...
def _is_generic_ptr(hostname: str, ip: str) -> bool:
    ...
    some encoding (e.g. `1-2-3-4.dyn.isp.net`, `host-192-0-2-5.x`,
    `5.2.0.192.in-addr.arpa`). These don't expose internal
...
#   - inject `evil.example.com A <attacker_ip>` -> phishing infra
...
def _check_dns_dynamic_update(svc: DiscoveredService) -> Finding | None:
    ...
    return Finding(
        ...
        remediation=(
            ...
            "permite reescribir el MX de example.com hacia atacante -> "
...
# Surfaced by the internal testing pilot 2026-05-18 against example.com:
...
# Surfaced internal testing 2026-05-19: cluster `example-cluster` con 3
# nodos Proxmox VE (.115 proxmox2, .200 pve-example, .222 pve-torre-
            ...
            drift_findings.append(
                Finding(
                    ...
                    remediation=(
                        ...
                        "Trampa comun internal testing: VM-Wazuh STOPPED en "
...
def _check_webserver_eol(svc: DiscoveredService, headers: str) -> Finding | None:
    ...
    Surfaced by the internal testing pilot 2026-05-18 against .18 (nginx
...
def _fetch_response_headers(url: str, *, timeout_s: int = 5) -> dict[str, str]:
    """HEAD/GET the URL and return response headers as a lowercased dict (last value wins)."""
    try:
        proc = subprocess.run(
            ["curl", "-sSI", "-k", "--max-time", str(timeout_s), url],
            capture_output=True,
            text=True,
            timeout=timeout_s + 3,
            check=False,
...
def _check_header_quality(svc: DiscoveredService) -> list[Finding]:
    """Weak VALUES inside present security headers (CWE-693/1021).

    ``_check_security_headers`` only flags MISSING headers — a hardened site like a brochure PHP app
    passes it while still shipping a CSP with ``'unsafe-inline'``/``'unsafe-eval'`` (XSS protection
    largely defeated) or an HSTS without ``includeSubDomains`` (subdomains downgradeable). This is the
    deterministic 'header quality' pass the LLM was being relied on for.
    """
    if svc.state != "open" or svc.port not in (80, 443, 8080, 8443, 8000, 8888):
        return []
    is_tls = svc.port in (443, 8443) or svc.service == "https"
    scheme = "https" if is_tls else "http"
    headers = _fetch_response_headers(f"{scheme}://{svc.host}:{svc.port}/")
    if not headers:
        return []
    host = f"{svc.host}:{svc.port}"
    findings: list[Finding] = []
    ...
    csp_l = csp.lower()
    # only script-src/default-src unsafe-* meaningfully weakens XSS protection
    if csp and ("'unsafe-inline'" in csp_l or "'unsafe-eval'" in csp_l):
        weak = [d for d in ("'unsafe-inline'", "'unsafe-eval'") if d in csp_l]
        findings.append(
            Finding(
                cwe="CWE-1021",
                severity="LOW",
                host=host,
                rule_id="csp-unsafe-directives",
                message=f"CSP permite {', '.join(weak)} — debilita la protección contra XSS",
                evidence=f"Content-Security-Policy: {csp[:400]}",
                remediation=(
                    "Reemplazar 'unsafe-inline' por nonces/hashes por-request en script-src y eliminar "
                    "'unsafe-eval'. Mover el JS inline a archivos servidos desde 'self'."
                ),
                severity_rank=_SEV_RANK["LOW"],
            )
        )
    if csp and "object-src" not in csp_l and "default-src 'none'" not in csp_l:
        findings.append(
            Finding(
                cwe="CWE-693",
                severity="INFO",
                host=host,
                rule_id="csp-no-object-src",
                message="CSP sin object-src 'none' (Flash/plugin legacy injection surface)",
                evidence=f"Content-Security-Policy: {csp[:300]}",
                remediation="Agregar object-src 'none' y base-uri 'self' a la CSP.",
                severity_rank=_SEV_RANK["INFO"],
            )
        )

    # Referrer-Policy that leaks the full URL (path+query) cross-origin: 'unsafe-url' always, and
    # 'no-referrer-when-downgrade' (the legacy default) on same-or-higher-TLS navigations.
    refpol = headers.get("referrer-policy", "").lower()
    if refpol and any(weak in refpol for weak in ("unsafe-url", "no-referrer-when-downgrade")):
        findings.append(
            Finding(
                cwe="CWE-200",
                severity="INFO",
                host=host,
                rule_id="weak-referrer-policy",
                message=f"Referrer-Policy débil ('{headers.get('referrer-policy', '')}') — filtra la URL completa (path+query) a orígenes externos vía Referer",
                evidence=f"Referrer-Policy: {headers.get('referrer-policy', '')}",
                remediation="Usar Referrer-Policy: strict-origin-when-cross-origin (o no-referrer/same-origin).",
                severity_rank=_SEV_RANK["INFO"],
            )
        )
    ...
    if hsts and "includesubdomains" not in hsts.lower():
        findings.append(
            Finding(
                ...
                severity="LOW",
                host=host,
                rule_id="hsts-no-includesubdomains",
                message="HSTS sin includeSubDomains — los subdominios siguen downgradeables a HTTP",
...
# Hidden-input name fragments that indicate an anti-CSRF token in a form.
_CSRF_TOKEN_HINTS = ("csrf", "xsrf", "_token", "authenticity_token", "nonce", "anti-forgery", "__requestverification")


def _check_form_csrf(svc: DiscoveredService) -> list[Finding]:
    """POST forms served without an anti-CSRF token (CWE-352).

    A passive but high-signal web finding the LLM kept fetching past: crawl the common pages, parse
    each ``method=post`` form, and flag the ones with no hidden CSRF-token field. Deterministic — no
    state change, just GET + HTML parse.
    """
    if svc.state != "open" or svc.port not in (80, 443, 8080, 8443, 8000, 8888):
        return []
    is_tls = svc.port in (443, 8443) or svc.service == "https"
    scheme = "https" if is_tls else "http"
    base = f"{scheme}://{svc.host}:{svc.port}"
    pages = [
        "/",
        "/index.php",
        "/contacto.php",
    ...
    seen: set[str] = set()
    for p in pages:
        code, body = _http_get(f"{base}{p}")
        if code != 200 or not body:
            continue
        for m in re.finditer(r"<form\b([^>]*)>(.*?)</form>", body, re.IGNORECASE | re.DOTALL):
            attrs, inner = m.group(1).lower(), m.group(2)
            inner_l = inner.lower()
            explicit_post = "method" in attrs and "post" in attrs
            # data-submission forms whose method is set by JS (no method attr → HTML default GET, but
            # the field set shows it really submits data): password fields, or email + a message body.
            sensitive = (
                'type="password"' in inner_l
                or "type='password'" in inner_l
                or ("email" in inner_l and ("<textarea" in inner_l or "mensaje" in inner_l or "message" in inner_l))
            )
            if not (explicit_post or sensitive):
                continue  # GET/search form with no sensitive data → not CSRF-relevant
            am = re.search(r"action\s*=\s*[\"']([^\"']*)", attrs)
            action = am.group(1) if am else p
            key = f"{p}|{action}"
            if key in seen:
                continue
            seen.add(key)
            if any(h in inner_l for h in _CSRF_TOKEN_HINTS):
                continue  # has a token field
            sev = "MEDIUM" if explicit_post else "LOW"
            how = "POST" if explicit_post else "data-submission (método vía JS)"
            findings.append(
                Finding(
                    cwe="CWE-352",
                    severity=sev,
                    host=f"{svc.host}:{svc.port}",
                    rule_id="form-no-csrf-token",
                    message=f"Form {how} en {p} (action={action or p}) sin token anti-CSRF",
                    evidence=f"GET {base}{p}\n<form {m.group(1)[:160]}> ... sin campo csrf/_token/nonce",
                    remediation=(
                        "Emitir un token CSRF por-sesión (synchronizer token) como hidden input y "
                        "validarlo server-side en cada POST. En PHP: bin2hex(random_bytes(32)) en "
                        "$_SESSION + hash_equals() al recibir."
                    ),
                    severity_rank=_SEV_RANK[sev],
                )
            )
            if len(findings) >= 6:  # cap noise
                return findings
    return findings
...
def _check_ssh(svc: DiscoveredService, ssh_target: str | None, ssh_password: str | None) -> list[Finding]:
    ...
    def _remote(cmd: str) -> str:
        base = [
            "ssh",
            # F202.S security hardening (internal testing audit): accept-new
...
# Surfaced internal testing 2026-05-19: VM-Ubuntu-Wazuh (VM 101 en cluster
...
def _check_siem_activity(
    ...
    def _remote(cmd: str) -> str:
        base = [
            "ssh",
            # F202.S security hardening (internal testing audit review):
...
# Surfaced internal testing .200.26 mediavault: smbclient -L -N anonymous
# login successful + share name visible (`rpa-example`). Aunque el tree-
# connect quedo denied (file access protected), la disclosure del
# nombre del share revelo el nombre del cliente. Banking-relevant:
...
# Surfaced by internal testing BASE .203.1: router edge con TCP/179 abierto al
...
_DATABASE_ENGINES: dict[tuple[str, int], dict[str, str]] = {
    ...
    # example .15: ms-sql-s en :1433 expuesto al segmento sin flag —
...
def _parse_agent_findings(text: str, *, target_host: str) -> list[Finding]:
    ...
    out: list[Finding] = []
    # Under KRYON_CAPABLE_MODEL, applicability gates DEGRADE a finding to a
    # low-confidence needs_verification item instead of dropping it — a capable model
    # finds real CVEs/findings the heuristic gates (product-keyword-in-prose, stale
    # fingerprint, cache-miss) wrongly reject. Same pattern as validation/grounding.
    from kryon.util.env import is_capable_model  # noqa: PLC0415

    _capable = is_capable_model()
    for item in items:
        if not isinstance(item, dict):
            continue
        _gate_flagged = False  # a gate rejected it, but capable keeps it for review
        # Normalize severity instead of dropping the finding: a real CVE emitted as
        # "informational"/"9.8"/"Critical (CVSS 9.8)" used to fail `not in _SEV_RANK`
        # and vanish. normalize_severity maps aliases + CVSS numbers, default MEDIUM.
        from kryon.util.severity import normalize_severity

        sev = normalize_severity(item.get("severity"))
        # is_finding_shape accepts `title`, so read it (and `description`) here too —
        # a finding structured under title/description used to pass the shape check
        # and then get dropped by `if not msg`.
        msg = str(
            item.get("message") or item.get("finding") or item.get("title") or item.get("description") or ""
        ).strip()
        if not msg:
            continue
        ...
        if not cve_ok:
            if _capable:
                logger.info("F151 flagged (capable → needs_verification): %s", cve_reason)
                _gate_flagged = True
            else:
        ...
        if not app_ok:
            if _capable:
                logger.info("F173 flagged (capable → needs_verification): %s", app_reason)
                _gate_flagged = True
            else:
        ...
        if not gen_ok:
            if _capable:
                logger.info("F183 flagged (capable → needs_verification): %s", gen_reason)
                _gate_flagged = True
            else:
        ...
        out.append(
            Finding(
                ...
                # them if a deterministic finding corroborates. A
                # gate-flagged (capable) finding starts lower — kept for
                # operator review, not asserted.
                confidence=0.3 if _gate_flagged else 0.5,
...
def _invoke_agent_deepening(
    ...
    *,
    ...
    try:
        ...
        # C — un modelo capable necesita más pasos para encadenar (dump→creds→SSH);
        # el default 4 está tuneado para el deepening rápido del 4B. Env explícito gana.
        from kryon.util.env import is_capable_model  # noqa: PLC0415

        _agent_default = "8" if is_capable_model() else "4"
        _agent_max = int(os.environ.get("KRYON_AGENT_MAX_TURNS", _agent_default))
...
def _phase_preamble(phase_name: str, *, target: str, scope: str, families: list[str], findings: list[Finding]) -> str:
    ...
    _capable = is_capable_model()
    # Fallback template covers phases without a dedicated key — notably the
    # exploitation / post_exploit phases (profiles deep/enterprise_deep). For the 4B
    # the passive "investigate and emit findings" is fine; for the capable that fallback
    # is exactly the phase it exists to DRIVE, so give it the offensive chaining stance.
    if _capable:
        _fallback = (
            "Phase: {phase}. Target {target}. Current findings: {findings_count}. "
            "Esta es tu kill-chain para CONDUCIR: explotá cada finding confirmado, "
            "encadená (dump→creds→SSH/lateral, foothold→privesc), re-escaneá con otros "
            "parámetros si aporta, y extendé hacia acceso/impacto. Emit structured JSON "
            "findings for everything you verify."
        )
    else:
        _fallback = (
    ...
    template = _PHASE_PREAMBLES.get(phase_name, _fallback)
    ...
    if _capable:
        # Same output shape, but DON'T frame the turn as "emit array → stop / []".
        # A capable model chains — register findings as it verifies AND keeps going
        # while a lead is open (mirrors the eje-9 head-start stance in investigate).
        _contract = (
            "\n\nIMPORTANT — registrá cada finding que verifiques en un JSON array con "
            'esta forma: [{"cwe": "CWE-...", "severity": "CRITICAL|HIGH|MEDIUM|LOW|INFO", '
            '"host": "...", "rule_id": "...", "message": "...", "evidence": "...", '
            '"remediation": "..."}]. NO cierres el turno con [] ni con el resumen mientras '
            "haya lead abierta — explotá / encadená / pivotá primero. Tool-call JSON does "
            "NOT count as a finding. <think>...</think> blocks are accepted; the parser strips them.\n"
        )
    else:
        _contract = (
...
def _invoke_orchestrated_engagement(
    ...
    *,
    ...
    try:
        ...
        console.print(f"  [yellow]agent load failed: {exc}[/yellow]")
        # Must be a 3-tuple like every other exit — the caller unpacks 3 values;
        # returning 2 turned this graceful-degradation path into a ValueError.
        return [], [], None
...
_DEVICE_FAMILIES: list[tuple[str, list[str], tuple[str, ...], str]] = [
    ...
    # example .200.249: Kyocera MFP en data center segment con banner
...
_DVR_BODY_MARKERS = (
    ...
    # F201.A.B — Hikvision modern firmware login URLs (internal testing
...
_DVR_PORT_COMBOS: tuple[frozenset[int], ...] = (
    ...
    # F201.A.B — internal testing TORRE_USR .2: 80 + 8000 (HTTP + Hikvision
...
# Surfaced by internal testing .200.249 — `Server: KM-MFP-http` + path
...
_PRINTER_BANNER_MARKERS = (
    "km-mfp",  # Kyocera Mita MFP (internal testing .200.249)
...
_PRINTER_BODY_MARKERS = (
    "/wlmesp/",  # Kyocera/Olivetti (internal testing .200.249)
...
def _detect_device_families(services: list[DiscoveredService]) -> list[str]:
    ...
    for s in services:
        ...
        # F199.K refined this after the internal testing pilot showed both
        ...
        # F202.I (internal testing 2026-05-18 .101): 135 + 139 + 445 were
        ...
        # F202.I.B (internal testing 2026-05-18 .10 PBX): the host had
...
# F210 Fase 4 — device-compliance control_ids whose verdict is INFERRED
# from a (spoofable / backport-prone) version or banner rather than a
# direct probe of the flagged condition. Downgraded to
# verification_level="heuristic" so they land in the report's "requiere
# verificación" band instead of ground truth. Extend ONLY after per-check
# review — add a control here only when it infers from a version→CVE
# mapping or a REMOTE banner, NOT when it reads an on-box config/version
# (pveversion, `get system status`) which is a reliable direct read.
_BANNER_INFERRED_CONTROLS = frozenset(
    {
        "TOMCAT-1.1",  # Tomcat major EOL inferred from HTTP banner / error-page version (spoofable)
        "FGT-5.3",  # FortiOS version → known-CVE exposure mapping (backport-prone)
    }
)


def _compliance_verification_level(verdict: str, control_id: str) -> str:
    """F210 — classify a device-compliance finding's verification level.

    ``ERROR`` verdicts (the check could not determine the state — SSH,
    command or parse failure) and banner/version-inferred controls are
    ``heuristic`` (routed to "requiere verificación"); a directly-probed
    ``FAIL`` is ``confirmed`` ground truth.
    """
    if verdict == "ERROR" or (control_id or "").upper() in _BANNER_INFERRED_CONTROLS:
        return "heuristic"
    return "confirmed"
...
def _run_device_compliance(
    ...
    *,
    ...
    # F210 — band lookup for the inferred/ERROR downgrade. Local import
    # matches the existing pattern (annotate_confidence is imported lazily
    # too) and avoids a top-level dependency on the scoring subpackage.
    ...
    for r in family_results:
        ...
        # F210 — an ERROR (check couldn't determine state) or a banner/
        # version-inferred control is not ground truth → needs review.
        level = _compliance_verification_level(r.verdict, r.control_id)
        conf = _VERIFICATION_BANDS[level]
        findings.append(
            make_finding(
...
def _run_openvas_engage_phase(console, *, target: str, client=None) -> list:
    """OpenVAS/Greenbone active scan → engage.Finding list.

    Opt-in (``--openvas``) and double-gated by KRYON_OPENVAS_FIRE (the caller
    checks ``is_openvas_enabled``). Drives a stock Greenbone over GMP, then
    normalizes results (one Finding per CVE, QoD→confidence) through the
    applicability gates. Skips silently on any error — never breaks the
    engagement. ``client`` is injectable for testing.
    """
    try:
        ...
        return results_to_findings(xml)
    except Exception as exc:  # noqa: BLE001 — never break the engagement
        console.print(f"  [yellow]openvas warning: {exc}[/yellow]")
        return []


def _run_cinc_engage_phase(
    console,
    *,
    target: str,
    ssh_user: str = "",
    ...
    Opt-in (``--cinc``) and gated by KRYON_CINC_FIRE (checked by the caller).
    Runs each configured profile against the target over Cinc's own SSH
    transport, normalizing FAILED controls to Findings via the applicability
    gates. Per-profile errors are logged and skipped — never break the
    engagement. ``runner`` is injectable for testing.
    """
    try:
    ...
    for profile in profiles_from_env():
        try:
            ...
            console.print(f"  [yellow]cinc warning ({profile}): {exc}[/yellow]")
    return findings


def _parse_ssh_user_port(ssh: str) -> tuple[str, int]:
    """Extract (user, port) from an 'user@host[:port]' SSH target string."""
    ssh = ssh or ""
...
def _run_lynis_engage_phase(
    ...
    *,
    ...
    Opt-in (``--lynis``) and gated by KRYON_LYNIS_FIRE (checked by the caller).
    Lynis is host-local, so we run it ON the target over SSH (reusing the tested
    compliance run_cmd transport) unless a ``runner`` is injected (tests).
    report.dat → Findings via the applicability gates. Skips silently on any
    Lynis error — never breaks the engagement.
    """
    try:
    ...
    except LynisError as exc:
        console.print(f"  [yellow]lynis warning: {exc}[/yellow]")
        return []
...
def run_engage(args: argparse.Namespace) -> int:
    ...
    _baseline_existed: bool = False  # R3 warm-up: distinguish "no baseline" from "empty baseline"
    ...
    try:
        from kryon.state.baseline_diff import baseline_exists, load_previous_findings
        from kryon.state.engagement_state import read_state as _read_state_for_diff

        prev_state = _read_state_for_diff(target)
        if prev_state is not None:
            _baseline_existed = baseline_exists(prev_state.findings_path)
    ...
    # F136 fix: bind `services` unconditionally. On --resume the `else` branch
    # (which assigns it from nmap) is skipped, but Phase 2b' at ~line 5880 calls
    # _detect_device_families(services) regardless — leaving it unbound crashed
    # the whole resume path with NameError.
    ...
    for svc in open_svcs:
        ...
        # F202.A — DNS open resolver. Surfaced by .205 (DC example.com
    ...
    # SSH creds. Surfaced internal testing: Wazuh VM apagado -> agents
    ...
    # --- Phase 2b'' — OpenVAS/Greenbone active vuln scan (opt-in --openvas,
    # double-gated by KRYON_OPENVAS_FIRE). Stock Greenbone over GMP; findings
    # normalized + applicability-filtered like every other deterministic source.
    if getattr(args, "openvas", False):
        ...
        if not is_openvas_enabled():
            ...
            _banner(console, "Fase 2b'' — OpenVAS/Greenbone vuln scan")
            ov_findings = _run_openvas_engage_phase(console, target=target)
            if ov_findings:
                findings.extend(ov_findings)
                findings.sort(key=lambda f: (f.severity_rank, f.host, f.rule_id))

    # --- Phase 2b''' — Cinc Auditor compliance-as-code (opt-in --cinc,
    # double-gated by KRYON_CINC_FIRE). dev-sec/CIS profiles over SSH; FAILED
    # controls → findings, applicability-filtered.
    if getattr(args, "cinc", False):
        ...
        if not is_cinc_enabled():
            ...
            _banner(console, "Fase 2b''' — Cinc Auditor (dev-sec/CIS profiles)")
            _cu, _cp = _parse_ssh_user_port(args.ssh or "")
            cinc_findings = _run_cinc_engage_phase(
                console,
                target=target,
                ssh_user=_cu,
                ssh_key=args.ssh_key,
            ...
            if cinc_findings:
                findings.extend(cinc_findings)
                findings.sort(key=lambda f: (f.severity_rank, f.host, f.rule_id))

    # --- Phase 2b'''' — Lynis host audit (opt-in --lynis, double-gated by
    # KRYON_LYNIS_FIRE). Runs lynis ON the target over SSH; warnings/suggestions
    # → findings, applicability-filtered.
    if getattr(args, "lynis", False):
        ...
        if not is_lynis_enabled():
            ...
            _banner(console, "Fase 2b'''' — Lynis host audit")
            _lu, _lp = _parse_ssh_user_port(args.ssh or "")
            lynis_findings = _run_lynis_engage_phase(
                console, target=target, ssh_user=_lu, ssh_key=args.ssh_key, ssh_port=_lp
            )
            if lynis_findings:
    ...
    # WIRING FIX — DNS / email-security posture (SPF/DMARC/DKIM/CAA/MTA-STS/
    # TLS-RPT + subdomain-takeover). domain-keyed, so it lives outside the
    # per-service probe loop (run_dns_probes skips IP literals and bare hosts).
    # Was wired into `kryon investigate` but never into `kryon engage`, so every
    # banking audit silently missed email-spoofing posture — the recurring gap
    # surfaced in the POC. Read-only DNS lookups, never raises → no gate.
    try:
        from urllib.parse import urlparse

        _t = (target or "").strip()
        _domain = (urlparse(_t).hostname or "") if "://" in _t else _t.split("/", 1)[0].rsplit(":", 1)[0]
        if _domain:
            from kryon.cli.dns_probes import run_dns_probes

            _dns_findings = run_dns_probes(_domain)
            if _dns_findings:
                findings.extend(_dns_findings)
                console.print(f"  [green]{len(_dns_findings)}[/green] hallazgo(s) DNS/email-security en {_domain}")
    except Exception as exc:  # noqa: BLE001 — never break the engagement
        console.print(f"  [dim]DNS posture probes skipped: {exc}[/dim]")

    # F133 / R3 — Baseline diff + warm-up drift alert. The whole decision
    # lives in run_drift_check (testable, network-injectable): a first-ever
    # run (no baseline file) stays silent instead of alerting on every finding
    # as "NEW"; a present-but-empty baseline (previous run was clean) correctly
    # treats this run's findings as real drift. Alerting is opt-in via
    # --notify-drift OR KRYON_NOTIFY_DRIFT=true (the appliance/cron path, where
    # there's no CLI flag to pass).
    baseline_diff = None
    try:
        from kryon.notifications.drift_alert import run_drift_check
        from kryon.state.baseline_diff import format_diff_summary

        _notify_enabled = getattr(args, "notify_drift", False) or (
            os.environ.get("KRYON_NOTIFY_DRIFT", "").strip().lower() in {"1", "true", "yes", "on"}
        )
        baseline_diff, _alerted = run_drift_check(
            previous_findings_for_diff,
            findings,
            baseline_existed=_baseline_existed,
    ...
    # F210 Fase 5 — split confirmed (ground truth) from inferred/heuristic
    # (needs_verification) so the operator sees what to confirm before
    # handoff instead of one flat table that reads as all-confirmed.
    confirmed = [f for f in findings if not f.needs_verification]
    review = [f for f in findings if f.needs_verification]
    ...
    if review:
        rtbl = Table(
            ...
            title="⚠ Requiere verificación (inferido/heurístico — no ground truth)",
        )
        rtbl.add_column("#", style="dim", width=3)
        rtbl.add_column("Severity", width=10)
        rtbl.add_column("CWE", width=10)
        rtbl.add_column("Host", width=30)
        rtbl.add_column("Rule")
        rtbl.add_column("Nivel", width=10)
        for i, f in enumerate(review, 1):
            rtbl.add_row(str(i), f.severity, f.cwe, f.host, f.rule_id, getattr(f, "verification_level", "heuristic"))
        console.print(rtbl)
        console.print(
            f"  [yellow]⚠ {len(review)} finding(s) inferido(s) — confirmá manualmente antes de entregar al cliente[/yellow]"
        )
    ...
    # Dedup + cross-engine corroboration (default on): collapse the same
    # CVE/control reported by multiple engines (OpenVAS ∪ Cinc ∪ Lynis ∪ Nuclei
    # ∪ native checks) into one finding with merged provenance + boosted
    # confidence. KRYON_FINDING_DEDUP=false disables.
    if os.environ.get("KRYON_FINDING_DEDUP", "true").strip().lower() not in {"0", "false", "no", "off"}:
        from kryon.services.finding_dedup import dedupe_findings

        _n_before = len(findings)
        findings = dedupe_findings(findings)
        if len(findings) < _n_before:
            console.print(f"  [dim]dedup:[/dim] {_n_before} → {len(findings)} findings (corroboración cruzada)")
    ...
    ctx = {
        ...
        "engagement_id": engagement_id,
        "target_scope": scope,
        "auditor": args.auditor or "Your Security Firm / Kryon",
    ...
    # WIRING FIX — appliance loop: persist the diff as a machine-readable
    # artifact, forward findings to the SIEM, and notify the operator.
    # Previously only the FastAPI audit middleware ever forwarded to a SIEM,
    # so cron/CLI engagements were invisible to the SOC, and
    # notify_engagement_complete (F140) was implemented but never called.

    # (a) delta.json — the baseline diff was only printed to console; persist
    # it next to the report so a SIEM/agent can ingest only what changed.
    try:
        if baseline_diff is not None and out_dir:
            import json as _json
            from dataclasses import asdict as _asdict

            delta_path = Path(out_dir) / f"kryon-{engagement_id}-delta.json"
            delta_path.write_text(
                _json.dumps(_asdict(baseline_diff), ensure_ascii=False, indent=2, default=str),
                encoding="utf-8",
            )
            console.print(f"  [green]delta[/green] → {delta_path}")
    except Exception as exc:  # pragma: no cover
        console.print(f"  [dim]delta.json skipped: {exc}[/dim]")

    # (a-bis) informe de novedades — the client-facing artifact ("qué cambió
    # anoche"). delta.json feeds the SIEM (machine); this feeds the human the
    # operator forwards to the customer, in plain business language. Same gate.
    try:
        if baseline_diff is not None and out_dir:
            from datetime import date as _date

            from kryon.reporting.drift_report import build_drift_report

            report_md = build_drift_report(
                baseline_diff,
                target=target,
                client=args.client or "",
                date=_date.today().isoformat(),
            )
            report_path = Path(out_dir) / f"kryon-{engagement_id}-novedades.md"
            report_path.write_text(report_md, encoding="utf-8")
            console.print(f"  [green]informe[/green] → {report_path}")
    except Exception as exc:  # pragma: no cover
        console.print(f"  [dim]informe de novedades skipped: {exc}[/dim]")

    # (a-ter) PDF con marca del informe — the formal deliverable. Reuses the
    # HTML renderer + apply_branding + weasyprint. Best-effort: degrades
    # silently when weasyprint/GTK3 is missing (common on Windows) — the .md
    # is always there. In the appliance (Docker/Linux) the PDF renders fine.
    try:
        if baseline_diff is not None and out_dir:
            ...
            from kryon.reporting.drift_report import build_drift_report_html
            from kryon.reporting.pdf import html_to_pdf

            _html = apply_branding(
                build_drift_report_html(
                    baseline_diff,
                    target=target,
                    client=args.client or "",
                    date=_date.today().isoformat(),
                ),
                BrandingConfig(footer_text="Confidencial — Solo para destinatarios autorizados"),
            )
            pdf_path = Path(out_dir) / f"kryon-{engagement_id}-novedades.pdf"
            pdf_path.write_bytes(_asyncio.run(html_to_pdf(_html)))
            console.print(f"  [green]informe PDF[/green] → {pdf_path}")
    except Exception as exc:  # pragma: no cover
        console.print(f"  [dim]informe PDF skipped (weasyprint/GTK3?): {exc}[/dim]")

    # (b) SIEM forward — best-effort, no-op unless KRYON_SIEM_TYPE (cron) or a
    # server-store config is present. Events are delta-tagged new/changed/existing.
    try:
        from kryon.integrations.finding_event import emit_findings_to_siem

        n_siem = emit_findings_to_siem(findings, baseline_diff, engagement_id=engagement_id, client=args.client or "")
        if n_siem:
            console.print(f"  [green]SIEM[/green] forwarded {n_siem} event(s)")
    except Exception as exc:  # pragma: no cover
        console.print(f"  [dim]SIEM forward skipped: {exc}[/dim]")

    # (c) end-of-run notification — gated by notify.should_notify
    # (KRYON_NOTIFY=on-critical default: only NOT_MET or CRITICAL/HIGH present).
    try:
        from kryon.notifications.notify import notify_engagement_complete

        _verdict = engagement_verdict_info.get("verdict") if isinstance(engagement_verdict_info, dict) else None
        res = notify_engagement_complete(
            engagement_id=engagement_id, target=target, verdict=_verdict, findings=findings
        )
        if res is not None and getattr(res, "ok", False) and not getattr(res, "dry_run", False):
            console.print(f"  [cyan]notify[/cyan] {res.provider}: {res.detail}")
    except Exception as exc:  # pragma: no cover
        console.print(f"  [dim]notification skipped: {exc}[/dim]")
...
def add_engage_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    p.add_argument("--auditor", default="", help="auditor name (default: Your Security Firm / Kryon)")
    ...
    p.add_argument(
        "--openvas",
        action="store_true",
        help="run an OpenVAS/Greenbone active vuln scan (Fase 2b''). Double-gated: "
        "also requires KRYON_OPENVAS_FIRE=true (active scanning needs written "
        "authorization). Findings are normalized + applicability-filtered.",
    )
    p.add_argument(
        "--cinc",
        action="store_true",
        help="run Cinc Auditor (FOSS InSpec) with the dev-sec/CIS profiles (Fase "
        "2b'''). Double-gated: also requires KRYON_CINC_FIRE=true. Read-only "
        "compliance-as-code over SSH; findings applicability-filtered.",
    )
    p.add_argument(
        "--lynis",
        action="store_true",
        help="run a Lynis host audit on the target over SSH (Fase 2b''''). "
        "Double-gated: also requires KRYON_LYNIS_FIRE=true. Needs lynis on the "
        "target + key-based SSH. Warnings→MEDIUM, suggestions→LOW.",
    )
