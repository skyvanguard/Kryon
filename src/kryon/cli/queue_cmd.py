...
def default_item_timeout() -> float | None:
    """Per-item child wall timeout (seconds), shared by queue/sweep/retest so a
    single wedged `kryon engage` child can't hang a worker forever.
    ``KRYON_QUEUE_ITEM_TIMEOUT_S`` overrides; 0 disables. Default 3600s."""
    return float(os.environ.get("KRYON_QUEUE_ITEM_TIMEOUT_S", "3600")) or None
...
def _resolve_engage_bin(explicit: str) -> list[str]:
    ...
    def _split(s: str) -> list[str]:
        ...
        if not s:
            return []
        # A bare existing path (possibly with spaces, e.g. Windows
        # 'C:\\Program Files\\Kryon\\kryon.exe') must NOT be shlex-split: POSIX
        # shlex eats the backslashes and the space splits it into garbage argv.
        # Treat an existing file path as a single token; otherwise parse as a
        # command line (posix=False on Windows so backslashes survive).
        if Path(s).exists():
            return [s]
...
def _run_one(argv: list[str], item_id: str, *, timeout: float | None = None) -> tuple[str, int, str]:
    ...
    try:
        ...
        # Guard the INDEXED value, not the raw string: whitespace-only stderr is
        # truthy but `.strip().splitlines()` is [], so `[-1]` raised IndexError and
        # (uncaught here) aborted the whole batch instead of just this item.
        _err_lines = (proc.stderr or "").strip().splitlines()
        stderr_tail = _err_lines[-1] if _err_lines else ""
...
def _process_queue(args) -> int:
    ...
    _item_timeout: float | None = default_item_timeout()
    ...
    if concurrency == 1:
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as pool:
            futures = {pool.submit(_run_one, argv, iid, timeout=_item_timeout): iid for iid, argv in argvs.items()}
