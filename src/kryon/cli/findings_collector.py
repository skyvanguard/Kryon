...
class CLIFindingsCollector:
    ...
    def __init__(self, store: object | None = None):
        ...
        # Content signatures already persisted this session — stops the same
        # deterministic finding being saved again when an analysis is re-run.
        self._saved_sigs: set[str] = set()
    ...
    def extract_from_message_history(self, messages: list[dict]) -> list[Finding]:
        ...
        for msg in messages:
            ...
            for finding in self._try_parse_findings(content):
                ...
                dedup_key = (
                    f"{finding.title}|{finding.affected_asset}|{(getattr(finding, 'description', '') or '')[:200]}"
                )
    ...
    def finding_signature(f: Finding) -> str:
        """Content signature for dedup — title + asset + severity, case-folded.
        IDs are random per parse, so they can't be used to dedup re-runs."""
        sev = getattr(getattr(f, "severity", ""), "value", "") or str(getattr(f, "severity", ""))
        return f"{(f.title or '').strip().lower()}|{(f.affected_asset or '').strip().lower()}|{sev.lower()}"

    def save_findings(self, findings: list[Finding]) -> int:
        """Persist findings to SQLite with cross-scan historical tracking
        (first_seen/last_seen/occurrences via FindingTracker), skipping
        content-duplicates already saved this session. Returns count saved."""
        fresh: list[Finding] = []
        for f in findings:
            sig = self.finding_signature(f)
            if sig in self._saved_sigs:
                continue  # same finding already persisted this session
            self._saved_sigs.add(sig)
            fresh.append(f)
        if not fresh:
            return 0
        # FindingTracker dedups against prior scans for this client: an already-seen
        # finding gets occurrences+1 / last_seen bumped; a new one is inserted with
        # first_seen — the persisted history that feeds the report's trend and
        # attack-path sections (previously findings were saved with no history).
        from kryon.memory.finding_history import FindingTracker

        FindingTracker(self.store).record_findings(fresh, self._scan_id, _CLI_CLIENT_ID)
        return len(fresh)
