...
class EngagementQueue:
    ...
    def claim_atomic(self, item_id: str) -> bool:
        ...
        # strict=True: if the lock can't be acquired, DO NOT fall back to a
        # lock-free write — that would let two concurrent workers both claim the
        # same item. Treat a lock timeout as "not claimed" so the item stays
        # pending for another attempt (safe: better un-run than double-run).
        from filelock import Timeout

        try:
            won = update_json_locked(self.state_path, _mutate, default={"items": []}, strict=True)
        except Timeout:
            logger.warning("claim_atomic: state lock busy for %s — item %s left pending", self.state_path, item_id)
            return False
        if won:
