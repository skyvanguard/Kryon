...
Perform thorough {phase.phase_type.value} and report all findings in detail."""
    ...
    # T3-A2: don't hard-code 5 turns — that ignored KRYON_MAX_TURNS/PHASE_TURNS and,
    # with a capable/reasoning model's CoT, left ~3 effective turns (can't finish a
    # kill-chain). resolve_phase_turn_cap() respects the env override + reasoning/
    # capability class (8 for capable, 5 for instruct).
    from kryon.tools.autonomous.pentest_planner import resolve_phase_turn_cap  # noqa: PLC0415

    _max_turns = resolve_phase_turn_cap()
    await Runner.run(agent, input=prompt, max_turns=_max_turns, run_config=get_run_config())
