...
class EngagementManager:
    ...
    async def pause_engagement(self, engagement_id: str):
        ...
        self._store.update_engagement(engagement_id, status=EngagementStatus.PAUSED.value, paused_at=now)
    ...
    async def resume_engagement(self, engagement_id: str):
        ...
        self._store.update_engagement(engagement_id, status=EngagementStatus.ACTIVE.value, paused_at=None)
    ...
    async def cancel_engagement(self, engagement_id: str):
        ...
        self._store.update_engagement(engagement_id, status=EngagementStatus.CANCELLED.value)
            ...
            if eng.id not in self._active_tasks:
                ...
                for phase in phases:
                    if phase.status == PhaseStatus.RUNNING:
                        self._store.update_engagement_phase(phase.id, status="pending", progress=0.0)
    ...
    async def _run_engagement(self, engagement_id: str):
        ...
        try:
            ...
            self._store.update_engagement(engagement_id, plan_json=json.dumps(plan, default=str))
            ...
            now = datetime.now(timezone.utc).isoformat()
            self._store.update_engagement(engagement_id, status=EngagementStatus.ACTIVE.value, started_at=now)
