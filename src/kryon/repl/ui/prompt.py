...
from kryon.repl.commands import FuzzyCommandCompleter
...
def get_command_shadow(text):
    ...
    if current_time - shadow_cache["last_update"] < shadow_cache["update_interval"] and shadow_cache["result"]:
        return shadow_cache["result"]
...
def get_user_input(command_completer, key_bindings, history_file, toolbar_func, current_text):
    ...
    def get_message():
        """Crystalline prompt marker: the live Ghost eye (breathes when idle,
        throbs in the finding's colour after a hit) + a bold-cyan KRYON and a
        steel chevron. Re-evaluated each refresh tick (refresh_interval below)."""
        from kryon.repl.ui.ghost_state import eye_pt_markup

        return HTML(f'{eye_pt_markup()} <style fg="#45e0ef"><b>KRYON</b></style><style fg="#2f6ea6">❯</style> ')
    ...
    return prompt(
        ...
        refresh_interval=0.75,  # tick so the marker + toolbar eye pulse (C3: ↑ from
        # 0.5 → fewer idle wakeups; the quantized pulse keeps it smooth)
