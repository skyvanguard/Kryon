"""
Animated "thinking" spinner for KRYON agent processing — crystalline identity.

Structure ported from Claude Code's React/Ink spinner, then restyled to Kryon:

- Crystal-pulse glyph (·◦◇◈◆ ping-pong) shimmering steel-blue → electric-cyan
  (the same materialization as the boot-up eye)
- Offensive-security spinner verbs first ("Reconnoitering", "Enumerating",
  "Exploiting" …), then the classic set
...
# Glyph frames — the Kryon crystal materializing: a spark grows into a full
# faceted diamond and back (echoes the boot-up eye). All widely supported.
_GLYPHS = ["·", "◦", "◇", "◈", "◆"]
# Ping-pong: forward then reverse → a breathing crystal pulse.
...
# Colors (RGB tuples) — Kryon crystalline identity (matches assets/kryon-mark.svg).
_COLOR_ACTIVE = (47, 110, 166)  # steel-blue #2f6ea6 — the base "thinking" color
_COLOR_SHIMMER_PEAK = (69, 224, 239)  # electric-cyan #45e0ef — the light-seam peak
_COLOR_STALLED = (239, 68, 68)  # red-500 — kept: universal "stuck" signal
_COLOR_DIM = (120, 120, 120)
_COLOR_TOOL = (34, 197, 94)  # green-500
_COLOR_HANDOFF = (250, 204, 21)  # yellow-400

# Spinner verbs — offensive-security flavored first, then the classic set.
_VERBS = [
    # Kryon offensive verbs — surface Kryon's identity while it works.
    "Reconnoitering",
    "Enumerating",
    "Fingerprinting",
    "Probing",
...
class AgentSpinner:
    ...
    def _render_frame(self) -> Text:
        ...
        # If the Ghost just found something, the spinner reacts in real time:
        # the crystal takes the severity colour + glyph, and the verb becomes a
        # ⚠ SEVERITY marker while the fresh-throb window is open.
        react_sev: str | None = None
        react_glyph = glyph
        react_color: str | None = None
        try:
            from kryon.repl.ui.ghost_state import eye_style, fresh_reaction

            react_sev = fresh_reaction()
            if react_sev:
                react_glyph, react_color = eye_style()
        except Exception:
            react_sev = None

        result = Text()

        # ── Glyph — reactive severity colour, else the crystal shimmer ──
        if react_sev and react_color:
            result.append(f" {react_glyph} ", style=f"bold {react_color}")
        else:
        ...
        # ── Verb, or a ⚠ SEVERITY alert marker while reacting ──
        if react_sev and react_color:
            result.append(f"⚠ {react_sev}", style=f"bold {react_color}")
        else:
    ...
    def start(self) -> None:
        if self._thread is not None:
            return
        try:
            from kryon.repl.ui.ghost_state import set_mood
    ...
    def create_hooks(self):
        ...
        class SpinnerRunHooks(RunHooks):
            ...
            async def on_tool_end(self, context, agent, tool, result):
                ...
                # Let the always-on Ghost eye react to any finding in the
                # output, and flash each finding inline (stays in scrollback).
                try:
                    from kryon.repl.ui.ghost_state import note_tool_output
                    from kryon.repl.ui.tool_call_renderer import render_finding_flash

                    findings = note_tool_output(name, result_str)
                    for sev, detail in findings[:5]:
                        render_finding_flash(severity=sev, detail=detail, console=spinner._console)
                    if len(findings) > 5:
                        spinner._console.print(f"[dim]  … +{len(findings) - 5} more findings[/dim]")
                except Exception:
                    pass
                with spinner._lock:
            ...
            async def on_handoff(self, context, from_agent, to_agent):
                ...
                try:
                    spinner._console.print(f"[#45e0ef]⤳ handoff →[/#45e0ef] [bold #45e0ef]{to_name}[/]")
                except Exception:
                    pass
