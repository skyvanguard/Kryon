...
class ParallelCommand(Command):
    ...
    def __init__(self):
        ...
        self.add_subcommand("merge", "Merge message histories from multiple agents", self.handle_merge)
        self.add_subcommand("prompt", "Set a custom prompt for a specific parallel agent", self.handle_prompt)
            ...
            try:
                with open(config_path) as f:
                    ...
                    if data and isinstance(data, dict) and "parallel_agents" in data:
                        ...
                        self._sync_to_env()
                        console.print(f"[green]Loaded {len(PARALLEL_CONFIGS)} agents from agents.yml[/green]")
    ...
    def handle_no_args(self) -> bool:
        ...
        if PARALLEL_CONFIGS:
            ...
            console.print(
                "\n[dim]Note: You can use agent IDs (P1, P2, etc.) in commands instead of long agent names[/dim]"
            )
    ...
    def handle_add(self, args: list[str] | None = None) -> bool:
        ...
        if instance_count > 1:
            console.print(f"[green]Added {display_name} #{instance_count} to parallel configuration[/green]")
        else:
            console.print(f"[green]Added {display_name} to parallel configuration[/green]")

        if len(PARALLEL_CONFIGS) >= 2:
            console.print(f"[bold green]Parallel mode AUTO-ENABLED with {len(PARALLEL_CONFIGS)} agents[/bold green]")
        else:
    ...
    def handle_list(self, args: list[str] | None = None) -> bool:
        ...
        for idx, config in enumerate(PARALLEL_CONFIGS, 1):
            ...
            prompt_display = (
                (config.prompt[:37] + "...") if config.prompt and len(config.prompt) > 40 else config.prompt or "-"
            )
            table.add_row(
                str(idx),
                config.id or "-",
    ...
    def handle_remove(self, args: list[str] | None = None) -> bool:
        ...
        if removed:
            ...
            # Show status
            if len(PARALLEL_CONFIGS) >= 2:
                console.print(f"[green]Parallel mode still ENABLED with {len(PARALLEL_CONFIGS)} agents[/green]")
            ...
            with open(config_path) as f:
                ...
                for agent_config in data["parallel_agents"]:
                    if "name" not in agent_config:
                        console.print("[yellow]Warning: Skipping agent without 'name' field[/yellow]")
                        continue
                ...
                self._sync_to_env()
                console.print(f"[green]Loaded {len(PARALLEL_CONFIGS)} agents from {filename}[/green]")
    ...
    def handle_override_models(self, args: list[str] | None = None) -> bool:
        ...
        global_model = os.getenv("KRYON_MODEL", "kryon-devstral-24b")
        count = 0
        ...
        if count > 0:
            console.print(f"[green]Override {count} agent(s) to use global model: {global_model}[/green]")
    ...
    def handle_merge(self, args: list[str] | None = None) -> bool:
        ...
        if strategy not in valid_strategies:
            console.print(
                f"[red]Error: Invalid strategy '{strategy}'. Must be one of: {', '.join(valid_strategies)}[/red]"
            )
            return False
    ...
    def _merge_chronological(self, all_histories: dict[str, list], agents_to_merge: list[str]) -> list[dict[str, Any]]:
        ...
        indices = dict.fromkeys(agents_to_merge, 0)
        ...
        for msg in all_messages:
            ...
            if should_add and msg.get("role") == "user":
                ...
                if merged and merged[-1].get("role") == "user" and merged[-1].get("content") == msg.get("content"):
                    should_add = False
    ...
    def _merge_by_agent(self, all_histories: dict[str, list], agents_to_merge: list[str]) -> list[dict[str, Any]]:
        ...
        merged.append(
            {
                "role": "system",
                "content": (f"This is a merged conversation history from agents: {', '.join(agents_to_merge)}"),
            }
        )
            ...
            if history:
                ...
                for msg in history:
                    ...
                    if msg.get("role") == "system" and any(
                        existing.get("role") == "system" and existing.get("content") == msg.get("content")
    ...
    def _merge_interleaved(self, all_histories: dict[str, list], agents_to_merge: list[str]) -> list[dict[str, Any]]:
            ...
            if msg.get("role") == "assistant" and msg.get("tool_calls"):
                ...
                if tool_call_id not in seen_tool_calls:
                    ...
                    for search_agent in agents_to_merge:
                        ...
                        while search_idx < len(histories[search_agent]):
                            ...
                            if search_msg.get("role") == "tool" and search_msg.get("tool_call_id") == tool_call_id:
                ...
                if msg.get("role") in ["user", "system"]:
                    ...
                    for recent_msg in reversed(merged[-5:]):  # Check last 5 messages
                        if recent_msg.get("role") == msg.get("role") and recent_msg.get("content") == msg.get(
                            "content"
                        ):
                            is_duplicate = True
                            break
    ...
    def _save_merged_history(
        ...
        if not target_exists_in_configs:
            ...
            existing_model = None
            for (agent_name, _instance_id), model_ref in ACTIVE_MODEL_INSTANCES.items():
        ...
        console.print(f"[dim]Note: The merged agent '{target_agent}' is now available with the combined history[/dim]")
    ...
    def _save_merged_history_to_sources(
        ...
        for agent_name in source_agents:
            ...
            # First check if there's an active instance
            existing_model = None
            for (model_agent_name, _instance_id), model_ref in ACTIVE_MODEL_INSTANCES.items():
        ...
        while i < len(args):
            ...
            if args[i].upper().startswith("P"):
                ...
                for config in PARALLEL_CONFIGS:
                    if config.id and config.id.upper() == target_id:
                        ...
                        for _idx, c in enumerate(PARALLEL_CONFIGS):
                            if c.agent_name == config.agent_name:
                                instance_num += 1
                                if c.id == config.id:
                                    break
            ...
            if not found_match:
                ...
                if not args[i].startswith("--"):
                    console.print(f"[yellow]Warning: Agent '{args[i]}' not found in histories[/yellow]")
                i += 1
