...
def get_compact_model():
    try:
        from kryon.repl.commands.compact import get_compact_model as _get_compact_model
...
class MemoryCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(name="/memory", description="Manage memory storage for agents", aliases=["/mem"])
...
"""
    ...
    def handle_apply(self, args: list[str] | None = None) -> bool:
        ...
        if len(args) > 1:
            ...
            if agent_identifier.lower() == "all":
                ...
                for agent_name, _agent_id in active_agents.items():
    ...
    def handle_merge(self, args: list[str] | None = None) -> bool:
        ...
        if not output_name:
            output_name = (
                f"merged_{len(memory_identifiers)}_memories_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
...
"""
            ...
            # Reload the agent with the new memory (preserve history by default)
            self._reload_agent_with_memory(agent_name, preserve_history=True)
    ...
    def _clear_agent_history(self, agent_name: str):
        ...
        model_instance = None
        for (name, _inst_id), model_ref in ACTIVE_MODEL_INSTANCES.items():
            if name == agent_name:
                model = model_ref() if model_ref else None
                if model:
    ...
    async def _ai_summarize_history(self, agent_name: str | None = None) -> str | None:
        ...
        model_name = get_compact_model() or os.environ.get("KRYON_MODEL", "kryon-devstral-24b")
...
This session is being continued from a previous conversation that ran out of context. The conversation is summarized below:"""
        ...
        for msg in history:
            ...
            if role == "user":
                ...
                if len(str(content)) < 500:  # Only include short outputs
                    ...
                    formatted_parts.append("TOOL OUTPUT: [Long output truncated]")
    ...
    def _reload_agent_with_memory(self, agent_name: str, preserve_history: bool = True):
        ...
        try:
            ...
            from kryon.agents import get_agent_by_name, get_available_agents
            from kryon.sdk.agents.simple_agent_manager import AGENT_MANAGER
            ...
            if agent_id == "P1":
                ...
                try:
                    ...
                    if hasattr(kryon.cli, "agent"):
                        kryon.cli.agent = new_agent
                except Exception:
                    pass
