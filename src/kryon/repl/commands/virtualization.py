        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if "Mounts denied" in error_msg or try_mount:
                    console.print("[yellow]Mount failed. Trying without workspace mount...[/yellow]")
                    ...
                    # Try again
                    retry_process = subprocess.run(new_cmd, capture_output=True, text=True, check=False)
                    ...
                    if retry_process.returncode == 0:
                        ...
                        return True, (
                            f"Successfully started container with ID: {container_id} (without workspace mount)"
                        )
                    else:
                        ...
                        if container.get("Names", "") == f"/{container_name}":
                            ...
                            if container_image == normalized_image_name:
                                ...
                                if "up" in container_status:
                                    return True, (f"Successfully started existing container: {container_id}")
                                # If stopped, start it
                    ...
                    for container in DockerManager.get_container_list():
                        if container.get("Names", "") == f"/{container_name}":
                            if container.get("Image", "") == image_name:
                                ...
                                # Start it if needed
                                start_cmd = ["docker", "start", container_id]
                                start_process = subprocess.run(start_cmd, capture_output=True, text=True, check=False)
                                    ...
                                    if needs_entrypoint_override:
                                        ...
                                        if fixed_result.returncode == 0:
                                            ...
                                            if subprocess.run(
                                                ...
                                                return True, (f"Successfully started fixed container: {container_id}")
    ...
    def set_active_container(container_id: str) -> None:
        ...
        try:
            ...
            if check_process.returncode == 0 and "false" in check_process.stdout.lower():
                ...
                if start_process.returncode != 0:
                    console.print("[yellow]Normal start failed. Trying with custom keep-alive...[/yellow]")
                    ...
                    try:
                        ...
                        if image_info.returncode == 0:
                            ...
                            if new_container.returncode == 0:
                                ...
                                console.print(f"[green]Created and started new container {container_id[:12]}[/green]")
                            else:
                                console.print(f"[red]Failed to create new container: {new_container.stderr}[/red]")
                    ...
                    if not verify.stdout.strip():
                        console.print(
                            "[yellow]Container started but exited immediately. Recreating with keep-alive...[/yellow]"
                        )
                        ...
                        if image_info.returncode == 0:
                            ...
                            if new_container.returncode == 0:
                                # Update container_id to the new one
                                container_id = new_container.stdout.strip()
                                console.print(f"[green]Created persistent container {container_id[:12]}[/green]")
                            else:
                                console.print(
...
class VirtualizationCommand(Command):
    ...
    def show_useful_images_table(self, show_all: bool = False) -> None:
        ...
        for image_name in essential_images:
            if image_name in DEFAULT_IMAGES:
                ...
                # Add row to table
                image_table.add_row(f"[bold]{image_id}[/bold]", display_name, info["description"], status)
    ...
    def _show_all_images_by_category(self, available_images, active_image):
        ...
        for category in category_order:
            ...
            for name, info in images:
                ...
                image_table.add_row(f"[bold]{image_id}[/bold]", image_name_display, description_display, status)
    ...
    def show_usage_info(self) -> None:
        ...
        console.print("  [bold]/virtualization[/bold]                      - Show environment selection menu")
        console.print("  [bold]/virtualization <image_name>[/bold]         - Switch to environment by name")
        console.print("  [bold]/virtualization <image_id>[/bold]           - Switch by ID (e.g.: sec1, pen1)")

        # Common commands with examples
        console.print("\n[bold yellow]Common Commands:[/bold yellow]")
        console.print(
            "  [bold]/virtualization kalilinux/kali-rolling[/bold] - Kali Linux with security tools [ID: pen1]"
        )
        console.print("  [bold]/virtualization parrotsec/security[/bold]   - Parrot Security OS with tools [ID: pen2]")
    ...
    def handle_run_subcommand(self, args: Optional[list[str]] = None) -> bool:
        ...
        if success:
            ...
            console.print(f"[green]Container {container_id[:12]} set as active environment.[/green]")
    ...
    def handle_activate_image(self, image_identifier: str) -> bool:
            ...
            if container_image == normalize_image_name(image_name):
                ...
                if "exited" in container_status or "created" in container_status:
                    ...
                    try:
                        ...
                        # Check if start was successful
                        if start_process.returncode != 0:
                            console.print(f"[yellow]Normal start failed: {start_process.stderr.strip()}[/yellow]")
                            console.print(
        ...
        if not image_available:
            console.print(f"[yellow]Image {image_name} not found locally. Pulling from Docker Hub...[/yellow]")
        ...
        if not fixed_image_exists:
            ...
            if not original_exists:
                ...
                with console.status(f"Pulling {image_name}..."):
                    result = subprocess.run(["docker", "pull", image_name], capture_output=True, text=True, check=False)
            ...
            with console.status("Creating temporary container..."):
                ...
                if create_result.returncode != 0:
                    console.print(f"[red]Failed to create temporary container: {create_result.stderr}[/red]")
                    return False
...
"""
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".dockerfile") as f:
                ...
                with console.status(f"Building fixed {fixed_image_name}..."):
                    ...
                    if build_result.returncode != 0:
                        console.print(f"[red]Failed to build fixed image: {build_result.stderr}[/red]")
                        return False

            # 2.5: Clean up the temporary container
            subprocess.run(["docker", "rm", temp_name], capture_output=True, text=True, check=False)

            console.print(f"[green]Successfully created fixed Parrot Security image: {fixed_image_name}[/green]")
        ...
        if existing_container_id:
            ...
            if check_result.returncode == 0 and "true" in check_result.stdout.lower():
                ...
                console.print(f"[green]Container {existing_container_id[:12]} is already running.[/green]")
            else:
                # Start it
                ...
                if start_result.returncode != 0:
                    ...
                    console.print("[yellow]Failed to start container. Removing and creating a new one.[/yellow]")
                    ...
                    console.print(f"[green]Successfully started container {existing_container_id[:12]}.[/green]")
        ...
        if not existing_container_id:
            ...
            with console.status("Creating container..."):
                ...
                if not check_result.stdout.strip():
                    console.print("[red]Container created but not running. Setting with fallback handling.[/red]")
                else:
                    console.print(f"[green]Container {existing_container_id[:12]} created and running.[/green]")
    ...
    def _fallback_to_best_environment(self) -> bool:
        ...
        for container in self.cached_containers:
            ...
            if image in DEFAULT_IMAGES:
                ...
                console.print(
                    f"[yellow]Falling back to existing container {container_id[:12]} from image {image}.[/yellow]"
                )
                # Refresh Docker info
                self.refresh_docker_info()
