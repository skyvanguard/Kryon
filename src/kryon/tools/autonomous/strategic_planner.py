...
class StrategicPlanner:
    ...
    def _order_objectives_by_dependencies(self, objectives: list[str]) -> list[str]:
        ...
        in_degree = dict.fromkeys(objectives, 0)
    ...
    def _generate_single_plan(
        ...
        constraints.get("time_limit", float("inf"))
        constraints.get("stealth_level", "medium")
        ...
        stealth_scores = {"low": 1, "medium": 2, "high": 3, "very_high": 4}
        avg_stealth = sum(stealth_scores.get(s["path"]["stealth_level"], 2) for s in plan["stages"]) / len(
            plan["stages"]
        )
    ...
    def dynamic_plan_adjustment(
        ...
        expected_time = sum(current_plan["stages"][i]["estimated_time"] for i in range(current_stage_idx))
        ...
        estimated_remaining = sum(
            current_plan["stages"][i]["estimated_time"] for i in range(current_stage_idx, len(current_plan["stages"]))
        )
        ...
        if estimated_remaining > time_remaining:
            ...
            for i in range(current_stage_idx, len(adjustments["new_plan"]["stages"])):
                ...
                if paths:
                    ...
                    adjustments["new_plan"]["stages"][i]["estimated_time"] = fastest_path["estimated_time"]
