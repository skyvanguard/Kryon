...
class AdaptiveStrategy:
    ...
    def adaptive_exploit_execution(
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            adapted_strategy = self._adapt_strategy(strategy, failure_reason, attempt_result, attempt)
    ...
    def _detect_failure_reason(self, attempt_result: dict[str, Any]) -> FailureReason:
        ...
        cache_key = hashlib.sha256(f"{error_msg}:{response}:{status_code}".encode()).hexdigest()
        ...
        # IPS detection
        elif any(indicator in error_msg for indicator in ["intrusion", "ips", "ids", "snort", "suricata"]):
        ...
        # Payload detected
            ...
            indicator in response for indicator in ["malicious", "attack detected", "suspicious", "security violation"]
        ):
        ...
        # Network errors
        elif any(indicator in error_msg for indicator in ["network", "dns", "unreachable", "connection"]):
    ...
    def _adapt_for_waf(self, strategy: dict, result: dict, attempt: int) -> dict[str, Any]:
        ...
        try:
            from kryon.tools.autonomous.evasion_autonomy import get_evasion_recommendations
    ...
    def _adapt_for_payload_detection(self, strategy: dict, result: dict, attempt: int) -> dict[str, Any]:
    ...
    def _adapt_for_permission_denied(self, strategy: dict, result: dict, attempt: int) -> dict[str, Any]:
    ...
    def _execute_exploit_attempt(self, target_ip: str, exploit: dict, service: dict, strategy: dict) -> dict[str, Any]:
        """Execute a single exploit attempt via the real autonomous executor.

        Was a placeholder that always returned failure (so the adaptive engine —
        and thus autonomous_ctf_solver — could never actually compromise a host).
        Now delegates to ``execute_exploit_autonomous`` (nuclei/sqlmap/RCE
        payloads). Imported lazily to avoid an import cycle with the orchestrator.
        ``strategy`` (timing/evasion adaptations) is applied by the retry loop
        around this call; the executor itself is strategy-agnostic for now."""
        from kryon.tools.autonomous.orchestrator import execute_exploit_autonomous

        try:
            return execute_exploit_autonomous(target_ip, exploit, service)
        except Exception as e:  # noqa: BLE001 — one attempt's failure feeds the retry loop
            return {
                "success": False,
