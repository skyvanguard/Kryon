...
def fuzz_api_endpoint(
    ...
    for ptype in active_types:
        ...
        for payload in payloads[ptype]:
            ...
            # T4-M12: curl -w appends "\n<http_code>". The old code did output.strip()
            # FIRST, which for an empty body ("\n204") stripped the leading newline to
            # "204" → the status code was misread as the body and status collapsed to
            # "0". rstrip only the trailing newline, then split off the last line.
            raw = output.rstrip("\n")
            if "\n" in raw:
                body, status = raw.rsplit("\n", 1)
            else:
                # No separator: a bare status code (empty body) vs a bodiless failure.
                body, status = ("", raw) if raw.strip().isdigit() else (raw, "0")
