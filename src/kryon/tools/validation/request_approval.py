...
# 3A — action destructiveness must reflect what the COMMAND does, not the
# severity of the finding that motivated it. The LLM routinely tags a benign
# "generate a report" action with the finding's "critical" severity, which the
# alias above turns into "destructive". Ground the label in the command text:
# destructive markers win first (safety), then modify, then read.
...
def _command_severity(text: str) -> str | None:
    """Derive action severity from the command/purpose text. Returns one of
    destructive/modify/read, or None when the text is opaque (caller then
    falls back to the LLM-supplied hint)."""
    low = (text or "").lower()
    if any(m in low for m in _DESTRUCTIVE_MARKERS):
        return "destructive"
    if any(m in low for m in _MODIFY_MARKERS):
        return "modify"
...
def request_approval(
    ...
    flow that MODIFIES REMOTE INFRASTRUCTURE (config edits, service
    restarts, firewall changes, remediation commands). The agent emits a
    list of actions; this tool shows them in the rich approval UI (F12.1)
    and returns the operator's verdict.

    Do NOT use this tool for benign, read-only, or internal operations —
    generating a report, persisting/recording findings, or re-scanning the
    target do not modify infrastructure and must NOT be routed through the
    approval gate. Just do them directly.

    The `severity` you pass should describe what the COMMAND does
    (destructive/modify/read/neutral), NOT how severe the finding is — a
    report about a CRITICAL vuln is still a `read` action.
    ...
    for a in actions or []:
        ...
        # Prefer the command-grounded severity; fall back to the LLM hint only
        # when the command text is opaque. Stops benign report/persist/rescan
        # actions from rendering as "⚠ destructive" just because they act on a
        # critical finding.
        derived = _command_severity(f"{command} {purpose}")
        severity_key = derived or _normalise_severity(str(a.get("severity", "modify")))
        built.append(
            ProposedAction(
                command=command,
                purpose=purpose,
