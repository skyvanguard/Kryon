...
_FRAMEWORK_PREFIX = {
    "pci-dss": ("1.", "2.", "4.", "5.", "6.", "7.", "8.", "10.", "11."),  # F15.1 numeric PCI sections
    "pci": ("1.", "2.", "4.", "5.", "6.", "7.", "8.", "10.", "11."),
    "proxmox": ("PVE-",),  # F23
    "pve": ("PVE-",),
    "ad": ("AD-",),  # F24
    "active-directory": ("AD-",),
    ...
    "coyote": ("TOMCAT-",),
    "vmware": ("ESX-",),  # VMware ESXi / vSphere hardening (CIS ESXi Benchmark)
    "esxi": ("ESX-",),
    "vsphere": ("ESX-",),
    "kvm": ("KVM-",),  # KVM / libvirt / QEMU host hardening
    "libvirt": ("KVM-",),
    "qemu": ("KVM-",),
    "hyper-v": ("HV-",),  # Microsoft Hyper-V hardening (CIS Hyper-V Benchmark, via WinRM)
    "hyperv": ("HV-",),
    "xen": ("XEN-",),  # Xen — XCP-ng / XenServer / Citrix Hypervisor (dom0 + xe CLI over SSH)
    "xcp-ng": ("XEN-",),
    "xenserver": ("XEN-",),
    "mikrotik": ("MTK-",),  # MikroTik RouterOS hardening (RouterOS CLI over SSH)
    "routeros": ("MTK-",),
    "postgresql": ("PG-",),  # PostgreSQL hardening (CIS PostgreSQL Benchmark, via psql/SSH)
    "postgres": ("PG-",),
    "psql": ("PG-",),
    "mysql": ("MYSQL-",),  # MySQL / MariaDB hardening (CIS Benchmark, via mysql client/SSH)
    "mariadb": ("MYSQL-",),
    "nginx": ("NGX-",),  # nginx hardening (CIS nginx Benchmark, via `nginx -T` over SSH)
    "apache": ("APACHE-",),  # Apache HTTPD hardening (CIS Apache Benchmark, via config grep/SSH)
    "httpd": ("APACHE-",),
    "linux": ("LNX-",),  # Linux OS baseline (CIS Distribution-Independent Linux, via SSH)
    "cis-linux": ("LNX-",),
    "cisco": ("IOS-",),  # Cisco IOS / IOS-XE hardening (CIS Cisco Benchmark, via SSH show run)
    "ios": ("IOS-",),
    "ios-xe": ("IOS-",),
    "haproxy": ("HAP-",),  # HAProxy reverse-proxy / LB hardening (config over SSH)
    "iis": ("IIS-",),  # Microsoft IIS hardening (CIS IIS Benchmark, via WinRM/PowerShell)
    "caddy": ("CADDY-",),  # Caddy web server hardening (Caddyfile over SSH)
...
def run_compliance_audit(
    ...
    """
    # Fail-fast: a host-CONFIG compliance audit needs access to the target
    # (SSH creds, or the target IS the local container for a self-audit).
    # Against a REMOTE host with NO creds the checks each try to connect and
    # accumulate timeouts far past the pre_hook budget — a plain
    # "audita <web-domain>" would hang ~180s. The network-only slice
    # (TLS/HTTP headers) is already covered by the read-only web battery, so
    # skipping here loses no coverage. Return N/A immediately instead.
    _local_hosts = ("localhost", "127.0.0.1", "::1", "", "0.0.0.0")
    if host not in _local_hosts and not ssh_user and not ssh_key_path:
        ...
        return _json.dumps(
            {
                "framework": framework,
                "host": host,
                "summary": {"status": "N/A", "reason": "no host access (SSH creds required)"},
                "findings": [],
                "note": (
                    f"Compliance audit de '{host}' necesita acceso al host (ssh_user/ssh_key_path). "
                    "Para un target web sin credenciales usá el audit web/vuln — el motor determinista "
                    "ya corre los checks de red (TLS/headers). El compliance de host se salta acá."
                ),
            }
        )
