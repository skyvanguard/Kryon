...
ALL_DISCLOSURE_RULES: frozenset[str] = frozenset(
    {
        "INFO-001", "INFO-002", "INFO-003", "INFO-004", "INFO-005",
        "INFO-007", "INFO-008",
        "INFO-010", "INFO-011", "INFO-012", "INFO-013", "INFO-014",
        "INFO-015", "INFO-016",
        "INFO-020", "INFO-021",
        "INFO-022", "INFO-023", "INFO-024", "INFO-025",
        "INFO-030", "INFO-031",
        "INFO-040", "INFO-041", "INFO-050",
    }
)
...
_PROBE_TABLE: dict[str, tuple[str, str, str, re.Pattern]] = {
    ...
    "/.git/index": ("INFO-001", "CRITICAL", "Git index file exposed",
                    re.compile(r"DIRC", re.IGNORECASE)),
    "/.git/logs/HEAD": ("INFO-001", "CRITICAL", "Git HEAD reflog exposed",
                       re.compile(r"^[0-9a-f]{40}\s+[0-9a-f]{40}", re.MULTILINE)),
    "/.gitignore": ("INFO-001", "LOW", "Gitignore file exposed (info-tier)",
                    re.compile(r"^[\w/.*-]", re.MULTILINE)),
    "/.svn/entries": ("INFO-001", "HIGH", "SVN entries file exposed",
                      re.compile(r"^\d+\s*$", re.MULTILINE)),
    "/.svn/wc.db": ("INFO-001", "HIGH", "SVN working copy DB exposed",
                    re.compile(r"SQLite format 3", re.IGNORECASE)),
    "/.hg/store": ("INFO-001", "HIGH", "Mercurial store directory exposed",
                   re.compile(r"00manifest|index", re.IGNORECASE)),
    "/.bzr/branch-format": ("INFO-001", "MEDIUM", "Bazaar branch-format exposed",
                            re.compile(r"Bazaar", re.IGNORECASE)),
    # INFO-002: env files
    ...
    "/.env.local": ("INFO-002", "CRITICAL", ".env.local file exposed",
                    re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    "/.env.development": ("INFO-002", "CRITICAL", ".env.development file exposed",
                          re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    "/.env.dev": ("INFO-002", "CRITICAL", ".env.dev file exposed",
                  re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    "/.env.staging": ("INFO-002", "CRITICAL", ".env.staging file exposed",
                      re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    "/.env.test": ("INFO-002", "CRITICAL", ".env.test file exposed",
                   re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    "/.env.backup": ("INFO-002", "CRITICAL", ".env.backup file exposed",
                     re.compile(r"=|^[A-Z_]+=", re.MULTILINE)),
    # INFO-003: db dumps
    ...
    "/database.sql": ("INFO-003", "CRITICAL", "Database SQL exposed",
                      re.compile(r"INSERT INTO|CREATE TABLE|DROP TABLE", re.IGNORECASE)),
    "/db.sql": ("INFO-003", "CRITICAL", "db.sql file exposed",
                re.compile(r"INSERT INTO|CREATE TABLE", re.IGNORECASE)),
    "/mysql.sql": ("INFO-003", "CRITICAL", "mysql.sql file exposed",
                   re.compile(r"INSERT INTO|CREATE TABLE", re.IGNORECASE)),
    "/db_backup.sql": ("INFO-003", "CRITICAL", "db_backup.sql exposed",
                       re.compile(r"INSERT INTO|CREATE TABLE", re.IGNORECASE)),
    # INFO-004: SSL/TLS private keys + SSH keys
    "/id_rsa": ("INFO-004", "CRITICAL", "SSH private key (id_rsa) exposed",
                re.compile(r"BEGIN (?:RSA |OPENSSH |DSA |EC )?PRIVATE KEY", re.IGNORECASE)),
    "/id_dsa": ("INFO-004", "CRITICAL", "SSH DSA private key exposed",
                re.compile(r"BEGIN DSA PRIVATE KEY", re.IGNORECASE)),
    "/id_ecdsa": ("INFO-004", "CRITICAL", "SSH ECDSA private key exposed",
                  re.compile(r"BEGIN EC PRIVATE KEY", re.IGNORECASE)),
    "/server.key": ("INFO-004", "CRITICAL", "TLS server private key exposed",
                    re.compile(r"BEGIN (?:RSA |EC |ENCRYPTED )?PRIVATE KEY", re.IGNORECASE)),
    "/privkey.pem": ("INFO-004", "CRITICAL", "TLS private key (privkey.pem) exposed",
                     re.compile(r"BEGIN (?:RSA |EC |ENCRYPTED )?PRIVATE KEY", re.IGNORECASE)),
    "/server.pem": ("INFO-004", "CRITICAL", "TLS server.pem exposed",
                    re.compile(r"BEGIN (?:RSA |EC )?PRIVATE KEY", re.IGNORECASE)),
    # INFO-005: cloud / package credentials
    "/.aws/credentials": ("INFO-005", "CRITICAL", "AWS credentials file exposed",
                          re.compile(r"\[default\]|aws_access_key_id|aws_secret", re.IGNORECASE)),
    "/.aws/config": ("INFO-005", "HIGH", "AWS config file exposed",
                     re.compile(r"\[(default|profile)|region\s*=", re.IGNORECASE)),
    "/.docker/config.json": ("INFO-005", "CRITICAL", "Docker auth config exposed",
                             re.compile(r'"auths"|"credHelpers"|"credsStore"', re.IGNORECASE)),
    "/.npmrc": ("INFO-005", "CRITICAL", ".npmrc with auth token exposed",
                re.compile(r"_authToken|//registry", re.IGNORECASE)),
    "/.pypirc": ("INFO-005", "CRITICAL", ".pypirc with credentials exposed",
                 re.compile(r"\[pypi|username\s*=|password\s*=", re.IGNORECASE)),
    "/.netrc": ("INFO-005", "CRITICAL", ".netrc with credentials exposed",
                re.compile(r"machine\s+\S+|login\s+|password\s+", re.IGNORECASE)),
    "/.boto": ("INFO-005", "HIGH", "Boto credentials file exposed",
               re.compile(r"\[Credentials\]|aws_access_key_id", re.IGNORECASE)),
    # INFO-007: Terraform state (gold mine for secrets)
    "/terraform.tfstate": ("INFO-007", "CRITICAL", "Terraform state file exposed",
                           re.compile(r'"terraform_version"|"resources"', re.IGNORECASE)),
    "/terraform.tfstate.backup": ("INFO-007", "CRITICAL", "Terraform state backup exposed",
                                  re.compile(r'"terraform_version"|"resources"', re.IGNORECASE)),
    "/.terraform/terraform.tfstate": ("INFO-007", "CRITICAL", "Terraform local state exposed",
                                       re.compile(r'"terraform_version"', re.IGNORECASE)),
    # INFO-008: application secrets configs
    "/config/database.yml": ("INFO-008", "CRITICAL", "Rails database.yml exposed",
                             re.compile(r"adapter:|username:|password:", re.IGNORECASE)),
    "/config/secrets.yml": ("INFO-008", "CRITICAL", "Rails secrets.yml exposed",
                            re.compile(r"secret_key_base|production:", re.IGNORECASE)),
    "/config/master.key": ("INFO-008", "CRITICAL", "Rails master.key exposed",
                           re.compile(r"^[0-9a-f]{32}", re.IGNORECASE)),
    "/config/credentials.yml.enc": ("INFO-008", "HIGH",
                                    "Rails encrypted credentials file exposed",
                                    re.compile(r"^[A-Za-z0-9+/=]", re.MULTILINE)),
    "/appsettings.json": ("INFO-008", "HIGH", ".NET appsettings.json exposed",
                          re.compile(r'"ConnectionStrings"|"Logging"', re.IGNORECASE)),
    "/appsettings.Development.json": ("INFO-008", "HIGH",
                                      ".NET dev appsettings exposed",
                                      re.compile(r'"ConnectionStrings"|"Logging"', re.IGNORECASE)),
    "/application.properties": ("INFO-008", "HIGH",
                                "Spring application.properties exposed",
                                re.compile(r"spring\.|datasource\.url|server\.port", re.IGNORECASE)),
    "/application.yml": ("INFO-008", "HIGH", "Spring application.yml exposed",
                        re.compile(r"spring:|datasource:|server:", re.IGNORECASE)),
    "/config.php": ("INFO-008", "HIGH", "config.php exposed",
                    re.compile(r"<\?php|define\(", re.IGNORECASE)),
    "/wp-config.php.bak": ("INFO-022", "CRITICAL", "WordPress wp-config.php backup exposed",
                           re.compile(r"DB_PASSWORD|DB_USER|AUTH_KEY", re.IGNORECASE)),
    "/wp-config.php.old": ("INFO-022", "CRITICAL", "WordPress wp-config.php.old exposed",
                           re.compile(r"DB_PASSWORD|DB_USER", re.IGNORECASE)),
    "/wp-config.php.swp": ("INFO-022", "CRITICAL", "WordPress wp-config.php.swp exposed",
                           re.compile(r"DB_PASSWORD|DB_USER", re.IGNORECASE)),
    # INFO-013: IDE
    ...
    "/.project": ("INFO-013", "LOW", "Eclipse project file exposed",
                  re.compile(r"<projectDescription|<name>", re.IGNORECASE)),
    "/.classpath": ("INFO-013", "LOW", "Eclipse classpath file exposed",
                    re.compile(r"<classpath", re.IGNORECASE)),
    ...
    "/docker-compose.override.yml": ("INFO-014", "MEDIUM",
    ...
    "/package-lock.json": ("INFO-014", "LOW", "package-lock.json exposed",
                           re.compile(r'"lockfileVersion"|"dependencies"', re.IGNORECASE)),
    "/yarn.lock": ("INFO-014", "LOW", "yarn.lock exposed",
                   re.compile(r"^# yarn lockfile|integrity sha", re.IGNORECASE | re.MULTILINE)),
    "/pnpm-lock.yaml": ("INFO-014", "LOW", "pnpm-lock.yaml exposed",
    ...
    "/composer.lock": ("INFO-014", "LOW", "Composer lock file exposed",
    ...
    "/Pipfile": ("INFO-014", "LOW", "Python Pipfile exposed",
                 re.compile(r"\[packages\]|\[\[source\]\]", re.IGNORECASE)),
    "/Pipfile.lock": ("INFO-014", "LOW", "Pipfile.lock exposed",
                      re.compile(r'"_meta"|"default"', re.IGNORECASE)),
    "/pyproject.toml": ("INFO-014", "LOW", "pyproject.toml exposed",
    ...
    "/Gemfile.lock": ("INFO-014", "LOW", "Gemfile.lock exposed",
                     re.compile(r"GEM\b|PLATFORMS\b", re.IGNORECASE)),
    "/Cargo.toml": ("INFO-014", "LOW", "Rust Cargo.toml exposed",
                    re.compile(r"\[package\]|\[dependencies\]", re.IGNORECASE)),
    "/go.mod": ("INFO-014", "LOW", "Go modules file exposed",
                re.compile(r"^module\s+|^go\s+\d", re.IGNORECASE | re.MULTILINE)),
    # INFO-015: CI/CD
    "/.gitlab-ci.yml": ("INFO-015", "MEDIUM", "GitLab CI config exposed",
                        re.compile(r"^stages:|^image:|^script:", re.IGNORECASE | re.MULTILINE)),
    "/.travis.yml": ("INFO-015", "MEDIUM", "Travis CI config exposed",
                     re.compile(r"^language:|^script:", re.IGNORECASE | re.MULTILINE)),
    "/.circleci/config.yml": ("INFO-015", "MEDIUM", "CircleCI config exposed",
                              re.compile(r"^version:|^jobs:|^workflows:", re.IGNORECASE | re.MULTILINE)),
    "/Jenkinsfile": ("INFO-015", "MEDIUM", "Jenkinsfile exposed",
                     re.compile(r"pipeline\s*\{|node\s*\{|agent\s+", re.IGNORECASE)),
    "/bitbucket-pipelines.yml": ("INFO-015", "MEDIUM",
                                 "Bitbucket Pipelines config exposed",
                                 re.compile(r"^pipelines:|^image:", re.IGNORECASE | re.MULTILINE)),
    "/.github/workflows/main.yml": ("INFO-015", "MEDIUM",
                                    "GitHub Actions workflow exposed",
                                    re.compile(r"^on:|^jobs:|^name:", re.IGNORECASE | re.MULTILINE)),
    "/azure-pipelines.yml": ("INFO-015", "MEDIUM",
                             "Azure Pipelines config exposed",
                             re.compile(r"^trigger:|^pool:|^stages:", re.IGNORECASE | re.MULTILINE)),
    # INFO-016: Java app config
    "/WEB-INF/web.xml": ("INFO-016", "HIGH", "Java web.xml descriptor exposed",
                         re.compile(r"<web-app|<servlet|<security", re.IGNORECASE)),
    "/web.xml": ("INFO-016", "HIGH", "web.xml exposed at root",
                 re.compile(r"<web-app|<servlet", re.IGNORECASE)),
    "/struts.xml": ("INFO-016", "HIGH", "Struts config exposed",
                    re.compile(r"<struts|<package", re.IGNORECASE)),
    "/hibernate.cfg.xml": ("INFO-016", "HIGH", "Hibernate config exposed",
    ...
    "/nginx_status": ("INFO-020", "HIGH", "nginx stub_status exposed",
                      re.compile(r"Active connections|server accepts handled", re.IGNORECASE)),
    "/status": ("INFO-020", "MEDIUM", "Generic /status endpoint responds",
    ...
    "/info.php": ("INFO-021", "HIGH", "PHP info page exposed",
                  re.compile(r"PHP Version|phpinfo", re.IGNORECASE)),
    "/test.php": ("INFO-021", "MEDIUM", "test.php exposed (often phpinfo)",
                  re.compile(r"PHP Version|phpinfo", re.IGNORECASE)),
    # INFO-023: Java app server admin
    "/manager/html": ("INFO-023", "HIGH", "Tomcat Manager UI exposed",
                      re.compile(r"Tomcat|Manager App", re.IGNORECASE)),
    "/manager/text": ("INFO-023", "HIGH", "Tomcat Manager text API exposed",
                      re.compile(r"OK -|Tomcat", re.IGNORECASE)),
    "/host-manager/html": ("INFO-023", "HIGH", "Tomcat Host-Manager exposed",
                           re.compile(r"Host Manager|Tomcat", re.IGNORECASE)),
    "/jmx-console/": ("INFO-023", "HIGH", "JBoss JMX console exposed",
                      re.compile(r"JMX|JBoss", re.IGNORECASE)),
    "/web-console/": ("INFO-023", "HIGH", "JBoss web-console exposed",
                      re.compile(r"JBoss|Web Console", re.IGNORECASE)),
    "/admin/console": ("INFO-023", "HIGH", "WebSphere/Oracle admin console exposed",
                       re.compile(r"WebSphere|WebLogic", re.IGNORECASE)),
    # INFO-024: Go pprof
    "/debug/pprof/": ("INFO-024", "MEDIUM", "Go pprof debug endpoint exposed",
                      re.compile(r"goroutine|heap|profile|cmdline", re.IGNORECASE)),
    "/debug/pprof/heap": ("INFO-024", "MEDIUM", "Go pprof heap exposed",
                          re.compile(r"goroutine|heap|profile", re.IGNORECASE)),
    "/debug/pprof/goroutine": ("INFO-024", "MEDIUM", "Go pprof goroutine exposed",
                               re.compile(r"goroutine|profile", re.IGNORECASE)),
    # INFO-025: Spring Boot actuator
    "/actuator": ("INFO-025", "MEDIUM", "Spring Boot actuator root exposed",
                  re.compile(r'"_links"|"href"|/actuator', re.IGNORECASE)),
    "/actuator/env": ("INFO-025", "HIGH", "Spring Boot actuator /env exposed",
                     re.compile(r'"activeProfiles"|"propertySources"', re.IGNORECASE)),
    "/actuator/heapdump": ("INFO-025", "CRITICAL",
                          "Spring Boot heapdump endpoint exposed",
                          re.compile(r"HPROF|JAVA PROFILE", re.IGNORECASE)),
    "/actuator/mappings": ("INFO-025", "MEDIUM",
                          "Spring Boot actuator /mappings exposed",
                          re.compile(r'"contexts"|"mappings"', re.IGNORECASE)),
    "/actuator/configprops": ("INFO-025", "HIGH",
                              "Spring Boot actuator /configprops exposed",
                              re.compile(r'"contexts"|"beans"', re.IGNORECASE)),
    "/actuator/beans": ("INFO-025", "MEDIUM",
                       "Spring Boot actuator /beans exposed",
                       re.compile(r'"contexts"|"beans"', re.IGNORECASE)),
    "/actuator/threaddump": ("INFO-025", "MEDIUM",
                            "Spring Boot actuator /threaddump exposed",
                            re.compile(r'"threads"|"threadName"', re.IGNORECASE)),
    "/actuator/loggers": ("INFO-025", "MEDIUM",
                         "Spring Boot actuator /loggers exposed",
                         re.compile(r'"levels"|"loggers"', re.IGNORECASE)),
    "/actuator/health": ("INFO-025", "LOW",
    ...
    "/wp-login.php": ("INFO-030", "MEDIUM", "WordPress login page responds",
    ...
    "/admin.php": ("INFO-030", "MEDIUM", "admin.php responds",
                   re.compile(r"admin|login", re.IGNORECASE)),
    "/admin/login": ("INFO-030", "MEDIUM", "/admin/login responds",
                     re.compile(r"login|admin", re.IGNORECASE)),
    "/cpanel/": ("INFO-030", "MEDIUM", "cPanel responds",
                 re.compile(r"cPanel", re.IGNORECASE)),
    "/webmail/": ("INFO-030", "MEDIUM", "webmail responds",
                  re.compile(r"webmail|RoundCube|Horde", re.IGNORECASE)),
    # INFO-031: DB management
    "/phpmyadmin/": ("INFO-031", "MEDIUM", "phpMyAdmin exposed",
                     re.compile(r"phpMyAdmin", re.IGNORECASE)),
    "/pma/": ("INFO-031", "MEDIUM", "phpMyAdmin (alt path /pma/) exposed",
              re.compile(r"phpMyAdmin", re.IGNORECASE)),
    "/myadmin/": ("INFO-031", "MEDIUM", "myadmin path exposed",
                  re.compile(r"phpMyAdmin", re.IGNORECASE)),
    "/adminer.php": ("INFO-031", "MEDIUM", "Adminer DB tool exposed",
                     re.compile(r"Adminer", re.IGNORECASE)),
    "/sql/": ("INFO-031", "MEDIUM", "/sql/ path exposed",
              re.compile(r"phpMyAdmin|Adminer|sql", re.IGNORECASE)),
    # INFO-040: API docs
    ...
    "/swagger.json": ("INFO-040", "LOW", "swagger.json exposed",
    ...
    "/openapi.yaml": ("INFO-040", "LOW", "OpenAPI YAML spec exposed",
                      re.compile(r"^openapi:|^swagger:|^info:", re.IGNORECASE | re.MULTILINE)),
    "/v2/api-docs": ("INFO-040", "LOW", "Springfox API docs exposed",
                     re.compile(r'"swagger"', re.IGNORECASE)),
    "/v3/api-docs": ("INFO-040", "LOW", "Springdoc API docs exposed",
                     re.compile(r'"openapi"', re.IGNORECASE)),
    "/redoc": ("INFO-040", "LOW", "Redoc API docs exposed",
               re.compile(r"redoc|openapi", re.IGNORECASE)),
    # INFO-041: GraphQL
    "/graphql": ("INFO-041", "LOW", "GraphQL endpoint responds",
                 re.compile(r'"data"|"errors"|"__schema"', re.IGNORECASE)),
    "/graphiql": ("INFO-041", "MEDIUM", "GraphiQL UI exposed",
                  re.compile(r"GraphiQL|graphql-ui", re.IGNORECASE)),
    "/altair": ("INFO-041", "MEDIUM", "Altair GraphQL UI exposed",
                re.compile(r"Altair|altair-graphql", re.IGNORECASE)),
    "/playground": ("INFO-041", "MEDIUM", "GraphQL Playground exposed",
                    re.compile(r"GraphQL Playground", re.IGNORECASE)),
    "/__graphql": ("INFO-041", "MEDIUM", "GraphQL alternate endpoint exposed",
                   re.compile(r'"data"|"errors"|"__schema"', re.IGNORECASE)),
...
def _default_remediation(rule_id: str, path: str) -> str:
    ...
    if rule_id == "INFO-008":
        return (
            "Move config files outside docroot. Treat secrets like the "
            "rest of the codebase: never commit, never serve."
        )
    if rule_id == "INFO-015":
        return (
            "Deny CI/CD config files at the web layer. Audit for "
            "inlined secrets + migrate them to a secret manager."
        )
    if rule_id == "INFO-016":
        return (
            "Block /WEB-INF/ + /META-INF/ + standalone *.xml descriptors "
            "at the web tier. These should never be served."
        )
    if rule_id == "INFO-020":
        return (
