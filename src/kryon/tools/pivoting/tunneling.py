...
# T4-M3: these were plain functions, never @function_tool, so the tool registry
# (which indexes objects with .name + .params_json_schema) never saw them — the
# agent had NO way to set up SOCKS / port-forward / chisel transport for lateral
# movement. Decorated + registered under KRYON_RED_TEAM in skills/tool_budget.py.
# Internal callers use the raw callable via `<fn>._raw_fn(...)`.
