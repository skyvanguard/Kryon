...
# T4-A1: upper bound for how long a single interactive-session send blocks while
# collecting output. The caller's real tool timeout can be 300/900s, but a session
# send must not hold the turn that long — the adaptive wait breaks on quiet output
# well before this, so this only caps genuinely long-streaming session commands.
_SESSION_MAX_WAIT = float(os.environ.get("KRYON_SESSION_MAX_WAIT", "60"))
...
_LONG_TOOLS = (
    ...
    "masscan",
    "subfinder",
    # T3-M3: credential-bruteforce tools — a rockyou brute easily exceeds 300s.
    "hydra",
    "medusa",
    "crackmapexec",
    "nxc",
...
_QUICK_NETWORK_TOOLS = {
    "nc", "ncat", "netcat", "telnet", "socat",
    "ssh", "sftp", "ftp", "mysql", "psql", "python", "python3", "ncftp",
}  # fmt: skip


def resolve_tool_timeout(command: str, default: int = 300) -> int:
    """Heuristic per-tool subprocess timeout. Long-running offensive tools
    (nuclei/sqlmap/hydra/ffuf/…) get 900s so they aren't killed mid-run;
    interactive/stdin binaries get 60s so they fail fast instead of hanging near the
    cap. T3-A7: the dedicated tool wrappers (nuclei_scan/sqlmap_scan/hydra_attack) used
    the 300s default and killed the tool at 5 min, before the model saw any finding."""
    low = (command or "").lower()
    if any(t in low for t in _LONG_TOOLS):
        return 900
    stripped = low.strip()
    first = stripped.split(" ", 1)[0].split("/")[-1] if stripped else ""
    if first in _QUICK_NETWORK_TOOLS:
        return 60
    return default
...
def run_command(
    ...
    Returns:
        str: Command output, status message, or session ID.
    """
    # T3-A7: apply the per-tool timeout heuristic when the caller didn't specify one.
    # The dedicated tool wrappers call this without a timeout, and the old 300s default
    # killed nuclei/sqlmap/hydra mid-run. A long-running tool now gets 900s.
    if timeout is None:
        timeout = resolve_tool_timeout(str(command))
    # Defense-in-depth: a literal "null"/"none"/"" session_id from an LLM means
    # "no session". Without this, a truthy "null" routes into session-send below
    # and returns "Session null not found" (which made the model loop). Normalize
    # to None so the command runs stateless. See run_command tool for the primary
    # normalization at the LLM boundary.
    if session_id is not None and str(session_id).strip().lower() in {
        "",
        "null",
        "none",
        "nil",
    ...
    try:
        ...
        if session_id:
            ...
            # Wait for the command to execute and capture output. Adaptive wait
            # (T4-A1): the old code capped at a hardcoded 3.0s, so a session command
            # that takes >3s to emit (a scanner run over an SSH shell, `sleep 5; cat
            # flag`, a slow reverse-shell prompt) returned EMPTY and the model read it
            # as a failure. Now:
            #   * silent commands (cd/export) still return fast — a short no-output cap,
            #   * commands that ARE streaming keep going until output goes quiet OR we
            #     hit the caller's real timeout (capped so a session send never blocks
            #     for the full 300/900s tool timeout).
            check_interval = 0.5
            quiet_period = 0.5  # output considered complete after this much silence
            no_output_cap = 3.0  # give up early if nothing ever arrives
            absolute_cap = min(float(timeout or no_output_cap), _SESSION_MAX_WAIT)
            ...
            while elapsed < absolute_cap:
                time.sleep(check_interval)
                elapsed += check_interval

                # Non-consuming peek: returns output accumulated SINCE the mark above,
                # so "still producing" is detected by the buffer GROWING, not by a
                # non-empty read (a finished command keeps returning the same text).
                current_new_output = session.get_new_output(mark_position=False)
                cur_len = len(current_new_output.strip())

                if cur_len > prev_len:
                    new_output_detected = True
                    last_growth = elapsed
                    prev_len = cur_len
                elif new_output_detected and (elapsed - last_growth) >= quiet_period:
                    # Output arrived and has stopped growing — command finished.
                    break
                elif not new_output_detected and elapsed >= no_output_cap:
                    # Nothing ever came back — a silent command; stop waiting.
                    break
