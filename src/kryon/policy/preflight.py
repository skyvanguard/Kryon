...
def resolve_policy() -> EngagementPolicy:
    ...
    model = os.environ.get("KRYON_MODEL", "kryon-local").strip()
    ...
    # A capable model needs sampling headroom to vary hypotheses — greedy 0.0
    # collapses attack-path exploration. Mirror sdk.agents.run._default_temperature
    # (0.4 capable / 0.0 4B); without this, engage's preflight setdefault pins the
    # env to 0.0 and pre-empts that capable-aware default. Reasoning-mode 0.6 wins.
    from kryon.util.env import is_capable_model  # noqa: PLC0415

    if reasoning_active:
        temperature_default = 0.6
    elif is_capable_model():
