...
# Original aggressive default — only trim outputs larger than this many chars.
# Right for a tight-window local model where every token counts.
_DEFAULT_MICRO_COMPACT_BUDGET = 1000
# Windows at/above this size are "large" and don't need the aggressive second
# pass: on a big window layer-1 (tool_output_cap) preserves each result whole up
# to its own large cap, so re-trimming to head500/tail200 here would just discard
# evidence a capable model chains from. Keep this EQUAL to
# tool_output_cap._LARGE_WINDOW_TOOL_RESULT_CHARS so layer-2 never re-trims what
# layer-1 preserved (both 50k on a large window).
_LARGE_WINDOW_TOKENS = 500_000
_LARGE_WINDOW_BUDGET = 50_000


def resolve_micro_compact_budget(
    model_max_tokens: int | None = None,
    override: str | None = None,
) -> int:
    """Minimum tool-output size (chars) before micro-compact trims it.

    Precedence: explicit ``override`` (``KRYON_MICRO_COMPACT_BUDGET``) >
    window-relative default. Small/medium windows keep the aggressive 1k
    default; a large window (>=500k, e.g. V4 Flash 1M) relaxes it so the
    already-capped outputs aren't re-trimmed. A malformed/non-positive
    override is ignored.
    """
    ...
    if model_max_tokens is None:
        try:
            ...
            model_max_tokens = 0
    # Kept CONSISTENT with resolve_tool_result_cap (same window//4 scale) so layer-2
    # never re-trims what layer-1 preserved. Capable OR ≥500K → scale to window; a weak
    # 4B (even with a 128K window) stays at the aggressive 1000.
    ...
    if capable or model_max_tokens >= _LARGE_WINDOW_TOKENS:
        return max(_DEFAULT_MICRO_COMPACT_BUDGET, min(_LARGE_WINDOW_BUDGET, model_max_tokens // 4))
    return _DEFAULT_MICRO_COMPACT_BUDGET
...
def _should_compact(content: object, budget: int) -> TypeGuard[str]:
    """Return True if the content exceeds the budget and is worth trimming.

    A ``TypeGuard`` so callers can narrow the ``msg.get("content")`` value
    (``Any | None``) to ``str`` after the check.
    """
...
def micro_compact_history(
    ...
    Args:
        ...
        budget: Minimum content length (chars) before trimming applies. When
            ``None`` (the default), it is derived from the model's context
            window (large windows relax it — see resolve_micro_compact_budget).
    ...
    if budget is None:
        budget = resolve_micro_compact_budget(override=os.getenv("KRYON_MICRO_COMPACT_BUDGET"))
