...
def host_in_scope(ip: str, entries: Iterable[str]) -> bool:
    """True if ``ip`` falls inside any scope entry (CIDR or exact host).

    A flat-list counterpart to ``is_in_scope`` for callers that hold plain
    ``list[str]`` scope (e.g. the autonomous orchestrator). Fail-closed: an
    empty/blank ``ip`` or no entries yields False — an active exploiter must
    never touch a host it can't prove is authorized."""
    ip = (ip or "").strip()
    if not ip:
        return False
    return any(_matches(ip, e) for e in entries if e and e.strip())
