...
class MemoryStore:
    ...
    def list_scans(self, client_id: str | None = None, offset: int = 0, limit: int = 0) -> list[ScanRecord]:
        conn = self._get_conn()
        if client_id:
            sql = "SELECT * FROM scans WHERE client_id = ? ORDER BY started_at DESC, rowid DESC"
            params: list = [client_id]
        else:
            sql = "SELECT * FROM scans ORDER BY started_at DESC, rowid DESC"
            params = []
        if limit > 0:
            sql += " LIMIT ? OFFSET ?"
    ...
    def save_scheduled_job(self, job: dict) -> None:
        ...
        conn.execute(
            ...
            "(id, client_id, agent_key, profile, targets_json, frameworks_json, cron, "
            "interval_seconds, start_hour, webhook_url, status, next_run, last_run, created_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (
                job["id"],
                job["client_id"],
                ...
                json.dumps(job.get("frameworks", []) or []),
                job.get("cron", ""),
    ...
    def list_scheduled_jobs(self, status: str | None = None) -> list[dict]:
        ...
        for r in rows:
            d = dict(r)
            # Deserialize JSON columns back to lists under the model field names.
            d["targets"] = json.loads(d.pop("targets_json", "[]") or "[]")
            d["frameworks"] = json.loads(d.pop("frameworks_json", "[]") or "[]")
            out.append(d)
        return out
