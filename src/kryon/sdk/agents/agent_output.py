    ...
    def validate_json(self, json_str: str, partial: bool = False) -> Any:
        ...
        if self._is_wrapped:
            if not isinstance(validated, dict):
                ...
                raise ModelBehaviorError(f"Expected a dict, got {type(validated)} for JSON: {json_str}")
            ...
            if _WRAPPER_DICT_KEY not in validated:
                ...
                raise ModelBehaviorError(f"Could not find key {_WRAPPER_DICT_KEY} in JSON: {json_str}")
