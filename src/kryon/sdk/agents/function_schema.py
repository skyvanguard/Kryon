...
def generate_func_documentation(func: Callable[..., Any], style: DocstringStyle | None = None) -> FuncDocumentation:
    """
...
def function_schema(
    ...
    for name, param in params[1:]:
        ...
        if ann != inspect._empty:
            origin = get_origin(ann) or ann
            if origin is RunContextWrapper:
                raise UserError(f"RunContextWrapper param found at non-first position in function {func.__name__}")
    ...
    filtered_params_no_ctf = [(name, param) for name, param in filtered_params if name.lower() != "ctf"]
