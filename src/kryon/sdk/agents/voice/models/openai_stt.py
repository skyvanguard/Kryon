...
async def _wait_for_event(event_queue: asyncio.Queue[dict[str, Any]], expected_types: list[str], timeout: float):
    """
...
class OpenAISTTTranscriptionSession(StreamedTranscriptionSession):
    ...
    def __init__(
        ...
        self._output_queue: asyncio.Queue[str | ErrorSentinel | SessionCompleteSentinel] = asyncio.Queue()
    ...
    async def _setup_connection(self, ws: websockets.ClientConnection) -> None:
        ...
        try:
            ...
            wrapped_err = STTWebsocketConnectionError("Timeout waiting for transcription_session.created event")
        ...
        try:
            ...
            wrapped_err = STTWebsocketConnectionError("Timeout waiting for transcription_session.updated event")
            await self._output_queue.put(ErrorSentinel(wrapped_err))
            raise wrapped_err from e
        except Exception as e:
            await self._output_queue.put(ErrorSentinel(e))
    ...
    async def _stream_audio(self, audio_queue: asyncio.Queue[npt.NDArray[np.int16 | np.float32]]) -> None:
        assert self._websocket is not None, "Websocket not initialized"
        self._start_turn()
        while True:
    ...
    async def transcribe_turns(self) -> AsyncIterator[str]:
        ...
        while True:
            ...
            if turn is None or isinstance(turn, ErrorSentinel) or isinstance(turn, SessionCompleteSentinel):
