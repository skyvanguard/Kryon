...
class StreamedAudioResult:
    ...
    def __init__(
        ...
        self._dispatcher_task: asyncio.Task[Any] | None = None  # Task to dispatch audio chunks in order
                ...
                tts_span.set_error(
                    {
                        "message": str(e),
                        "data": {
                            "text": text if self._voice_pipeline_config.trace_include_sensitive_data else "",
                        },
                    }
                )
    ...
    async def _add_text(self, text: str):
        ...
        if len(combined_sentences) >= 20:
            ...
            self._tasks.append(asyncio.create_task(self._stream_audio(combined_sentences, local_queue)))
    ...
    async def _turn_done(self):
        if self._text_buffer:
            ...
            self._tasks.append(
                asyncio.create_task(self._stream_audio(self._text_buffer, local_queue, finish_turn=True))
            )
            self._text_buffer = ""
