...
def truncate_output(output: Any, max_length: int | None = None) -> str:
    ...
    Keeps the first and last half of ``max_length`` with TRUNCATED in between.
    This SDK-level cap sits on the tool-result the model sees in history. A
    capable model chains from mid-output evidence (paths in a gobuster dump,
    creds in a sqlmap dump, ports in an nmap -A) — with the fixed 10K it was
    re-truncating what the services-layer output cap already preserved for the
    capable (50K). Give capable a larger window; the 4B keeps the tight 10K.
    An explicit ``max_length`` (callers that pass one) always wins.
    """
    if max_length is None:
        try:
            from kryon.util.env import is_capable_model  # noqa: PLC0415

            max_length = 40000 if is_capable_model() else 10000
        except Exception:  # noqa: BLE001 — env must never break the tool path
            max_length = 10000
    ...
    half = max_length // 2
    first_part = output_str[:half]
    last_part = output_str[-half:]
    return f"{first_part}\n\n... TRUNCATED ...\n\n{last_part}"
...
def _tool_call_args(tool_call: Any) -> str:
    """Pull the JSON arguments string off a tool CALL for the StuckDetector / audit log.

    Pass the originating ``tool_call`` (``ResponseFunctionToolCall``), NOT the result ``run_item``: the
    run_item's raw_item is the function_call_OUTPUT dict (``call_id``/``output``/``type``) and carries no
    arguments, so reading args from it always yielded ``""`` — every same-tool call then hashed to
    ``(tool, "")`` and the detector false-aborted legitimate multi-endpoint recon at turn 0 (web_fetch_smart
    on /blog vs /wp-login vs /xmlrpc all collapsed; observed live on THM Internal). Accepts the pydantic
    call object (``.arguments``) or a dict, and degrades to ``""`` rather than raising.
    """
            ...
            for fr in function_results:
                try:
                    audit_log.append(
                        ...
                        args=_tool_call_args(fr.tool_call),
                        result=fr.output,
                        phase=audit_phase,
                        status="ok",
                    )
    ...
    async def execute_function_tool_calls(
        ...
        *,
        ...
        async def run_single_tool(func_tool: FunctionTool, tool_call: ResponseFunctionToolCall) -> Any:
            ...
            # PLACEHOLDER-TARGET GUARD — refuse any tool call whose target/url/
            # host arg (or shell command) is still a literal placeholder ("HOST",
            # "<target>"), so the model asks for a real objective instead of
            # looping recon against a host that never resolves. One chokepoint =
            # every tool (nmap, sqlmap, nuclei, gobuster, …) covered at once.
            try:
                from kryon.validation.target_guard import guard_tool_args

                _ph = guard_tool_args(func_tool.name, tool_call.arguments)
            except Exception:  # noqa: BLE001 — the guard must never crash the run
                _ph = None
            if _ph:
                logger.warning("placeholder-target guard BLOCKED tool %s", func_tool.name)
                return _ph
            # DESTRUCTIVE-ACTION GUARDIAN — refuse obviously-destructive commands
            # (rm -rf /, DROP DATABASE, mkfs, dd of=/dev/…, fork bomb, shutdown)
            # BEFORE they run. Deterministic tier only (no model call); it closes
            # the gap the other gates leave — none of authorization / target_guard /
            # recon_guard inspects the command text for a destructive pattern. It
            # scopes to EXECUTABLE args (command/query), not url/target, so a benign
            # probe of an endpoint named "/shutdown" isn't misread. Critical for the
            # appliance profile (Kryon on client infra): a runaway command against
            # production is a real risk. One chokepoint = every tool covered.
            try:
                from kryon.validation.action_guardian import (
                    assess_tool_call,
                    is_enabled as _guardian_model_on,
                    is_mutating_action,
                )

                _dg = assess_tool_call(func_tool.name, tool_call.arguments)
                # GRAY-ZONE MUTATION JUDGE (opt-in model tier) — only if the
                # deterministic tier passed, KRYON_GUARDIAN_MODEL is set, AND the
                # action changes state (POST/PUT/DELETE). build_judge returns None
                # in the banca-safe profile, so the non-deterministic judge never
                # runs there. Bridged off the event loop with to_thread (a model
                # call costs seconds) — never per-action.
                if _dg is None and _guardian_model_on() and is_mutating_action(func_tool.name, tool_call.arguments):
                    from kryon.intelligence.judge_client import build_judge

                    _judge = build_judge(max_tokens=400, timeout=40.0)
                    if _judge is not None:
                        from kryon.validation.action_guardian import assess_mutation

                        _dg = await asyncio.to_thread(
                            assess_mutation, func_tool.name, tool_call.arguments, _judge
                        )
            except Exception as _guard_exc:  # noqa: BLE001 — the guardian must never crash the run
                # Fail-open, but LOUDLY: a silently-broken safety gate (import
                # error, signature drift) would leave the appliance unguarded with
                # nobody aware. Log so it surfaces in monitoring.
                logger.warning("action guardian raised, failing open: %s", _guard_exc)
                _dg = None
            if _dg:
                logger.warning("action guardian BLOCKED tool %s", func_tool.name)
                return _dg
            # RECON-ONLY GATE — under KRYON_RECON_ONLY, refuse port-scan / exploit
            # tools even in --active mode. Keyed on the tool NAME so the dedicated
            # wrappers (nmap, sqlmap_scan, hydra, …) — which call the raw dispatcher
            # and bypass run_command's own string check — are caught here too. One
            # chokepoint = every tool covered, matching the placeholder guard above.
            try:
                from kryon.validation.recon_guard import recon_only_reason_for_tool

                _ro = recon_only_reason_for_tool(func_tool.name, tool_call.arguments)
            except Exception:  # noqa: BLE001 — the gate must never crash the run
                _ro = None
            if _ro:
                logger.warning("recon-only gate BLOCKED tool %s", func_tool.name)
                return _ro
