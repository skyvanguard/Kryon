...
def build_ffuf_dir_cmd(
    ...
    *,
    ...
    needing the vhost in /etc/hosts. ``proxy`` (e.g. ``socks5://127.0.0.1:9050``)
    routes ffuf through Tor/VPN via its native ``-x`` — OPSEC-critical because
    torsocks can't wrap ffuf (a Go binary). See :func:`kryon.util.env.anon_proxy`.
    """
    base = url.rstrip("/")
...
def build_ffuf_vhost_cmd(
    ...
    *,
    ...
    ``proxy`` routes ffuf through Tor/VPN via ``-x`` (see :func:`build_ffuf_dir_cmd`)."""
    base = base_url.rstrip("/")
    cmd = f'ffuf -u {base}/ -H "Host: FUZZ.{domain}" -w {wordlist} -ac -t {threads} -json -s'
    if proxy:
        cmd += f" -x {proxy}"
    return cmd
...
def run_web_enum(
    ...
    *,
    ...
    stdout (injected for tests). ``proxy`` routes ffuf through Tor/VPN (``-x``);
    when omitted it defaults to :func:`kryon.util.env.anon_proxy` so an anonymized
    engagement stays anonymized even though ffuf bypasses torsocks (OPSEC).
    """
    ...
    # T3-A4: fall back to a wordlist that EXISTS. The seclists path is absent on the
    # non-kali image (its Dockerfile installs dirb, not seclists), so ffuf ran with a
    # missing -w and failed silently → 0 dirs (the exact step weak models can't do).
    # FALLBACK_DIR_WORDLIST (dirb/common.txt) was defined for this but never used.
    if not os.path.exists(dir_wordlist):
        if os.path.exists(FALLBACK_DIR_WORDLIST):
            dir_wordlist = FALLBACK_DIR_WORDLIST
        # else: leave it — ffuf will error and the caller logs the failure (no silent 0).

    # OPSEC: default to the configured anonymizing proxy. ffuf is a Go binary so
    # torsocks can't wrap it — without its native -x it would leak the real IP.
    if proxy is None:
        from kryon.util.env import anon_proxy
    ...
    try:
        dir_out = runner(build_ffuf_dir_cmd(base, dir_wordlist, host_header=dir_host_header, proxy=proxy), timeout)
    ...
    if enable_vhost and domain:
        try:
            vhost_out = runner(build_ffuf_vhost_cmd(base, domain, vhost_wordlist, proxy=proxy), timeout)
