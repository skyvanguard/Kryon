...
_JS = (".js", ".jsx", ".ts", ".tsx")
_PHP = (".php", ".phtml")
# T4-M5: SAST only covered py/js/php. Add the other high-frequency server languages.
_JAVA = (".java",)
_GO = (".go",)
_RUBY = (".rb",)
...
_RULES: tuple[_Rule, ...] = (
    ...
    _Rule(
        ...
        _PY,
        re.compile(
            r"(?:os\.system|os\.popen|subprocess\.(?:call|run|Popen|check_output)\s*\([^)]*shell\s*=\s*True)", re.I
        ),
        re.compile(r"^\s*#"),
    ),  # T4-A5: anchor to a real comment line, not any '#'
    _Rule(
        "sast-py-eval",
        "CWE-94",
        "HIGH",
        0.5,
        _PY,
        re.compile(r"\b(?:eval|exec)\s*\(\s*(?!['\"])"),
        re.compile(r"^\s*#|ast\.literal_eval"),
    ),
    _Rule(
        "sast-py-deser", "CWE-502", "HIGH", 0.8, _PY, re.compile(r"\b(?:pickle\.loads?|marshal\.loads|yaml\.load)\s*\(")
    ),
    _Rule(
        "sast-py-weakcrypto",
        "CWE-327",
    ...
    _Rule(
        ...
        re.compile(r"^\s*//"),
    ),  # T4-A5: real comment line, not any '//' (http://)
    _Rule("sast-js-cmdi", "CWE-78", "CRITICAL", 0.6, _JS, re.compile(r"child_process\.exec\s*\(")),
    _Rule("sast-js-sqli", "CWE-89", "HIGH", 0.5, _JS, re.compile(r"(?:query|execute)\s*\(\s*`[^`]*\$\{")),
    # --- PHP ---
    _Rule(
        "sast-php-cmdi",
        "CWE-78",
    ...
    _Rule("sast-php-eval", "CWE-95", "HIGH", 0.6, _PHP, re.compile(r"\beval\s*\(")),
    _Rule(
        "sast-php-sqli",
        "CWE-89",
        "HIGH",
    ...
    _Rule(
        ...
        0.6,
        _PHP,
        re.compile(r"\bunserialize\s*\(\s*\$_(?:GET|POST|REQUEST|COOKIE)", re.I),
    ),
    # --- Java ---
    _Rule(
        "sast-java-cmdi",
    ...
    _Rule(
        ...
        0.6,
        _JAVA,
        re.compile(r"(?:createStatement\(\)|Statement)\s*.*\.(?:execute(?:Query|Update)?)\s*\(\s*[\"'].*\+", re.I),
    ),
    _Rule(
        "sast-java-deser",
        "CWE-502",
    ...
    _Rule(
        ...
        0.6,
        _GO,
        re.compile(r"exec\.Command\s*\(\s*[\"'](?:/bin/)?(?:sh|bash)[\"']|exec\.CommandContext\s*\("),
    ),
    _Rule(
        "sast-go-sqli",
        "CWE-89",
    ...
    _Rule(
        ...
        0.4,
        _GO,
        re.compile(r"http\.(?:Get|Post|Head)\s*\(\s*(?:r\.(?:URL|FormValue)|c\.Query|req\.)"),
    ),
    # --- Ruby ---
    _Rule(
        "sast-ruby-cmdi",
        ...
        0.6,
        _RUBY,
        re.compile(r"(?:system|exec|`|%x|Open3\.|IO\.popen)\s*[\(\{`].*#\{"),
        re.compile(r"^\s*#"),
    ),
    _Rule(
        "sast-ruby-eval",
        ...
        0.6,
        _RUBY,
        re.compile(r"\b(?:eval|instance_eval|class_eval|send)\s*\(\s*(?:params|request)"),
        re.compile(r"^\s*#"),
    ),
    _Rule(
        "sast-ruby-sqli",
        ...
        0.5,
        _RUBY,
        re.compile(r"\.(?:where|find_by_sql|execute)\s*\(\s*[\"'].*#\{"),
    ),
    _Rule(
        "sast-ruby-deser",
        "CWE-502",
        ...
        0.6,
        _RUBY,
        re.compile(r"(?:Marshal\.load|YAML\.load)\s*\(\s*(?:params|request)"),
    ),
)
