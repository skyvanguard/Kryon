...
# Shell metacharacters + whitespace. Structural facts (host/domain/user/path/
# share) are parsed from TARGET-CONTROLLED text and interpolated into shell=True
# commands by the chain planner; a legitimate host/user/path never contains any
# of these, so stripping them neutralizes command injection at the source without
# losing real values. Passwords are NOT stripped (they legitimately contain
# metachars) — they're shlex-quoted at the interpolation site instead.
_SHELL_UNSAFE_RE = re.compile(r"[`$;|&()<>'\"\\\s]")
# Users keep '$' (AD machine accounts like WEBSERVER$) but lose the execution
# metachars — without '(' / '`' a bare '$' can't form command substitution.
_USER_UNSAFE_RE = re.compile(r"[`;|&()<>'\"\\\s]")


def _safe_struct(value: str) -> str:
    """Strip shell metacharacters/whitespace from a structural fact value."""
    return _SHELL_UNSAFE_RE.sub("", value) if isinstance(value, str) else value


def _safe_user(value: str) -> str:
    """Like _safe_struct but preserves '$' so AD machine-account names survive."""
    return _USER_UNSAFE_RE.sub("", value) if isinstance(value, str) else value
...
class ExtractedFacts:
    ...
    def __post_init__(self) -> None:
        # Sanitize structural fields (shell-injection defense at the source, since
        # the chain planner interpolates these into shell=True commands). The
        # password half of each cred is preserved verbatim (shlex-quoted downstream).
        # shares are NOT sanitized: they legitimately end in '$' (ADMIN$/C$/IPC$)
        # and are never interpolated into a shell command by the planner.
        object.__setattr__(self, "users", tuple(_safe_user(u) for u in self.users))
        object.__setattr__(self, "hosts", tuple(_safe_struct(h) for h in self.hosts))
        object.__setattr__(self, "domains", tuple(_safe_struct(d) for d in self.domains))
        object.__setattr__(self, "paths", tuple(_safe_struct(p) for p in self.paths))
        object.__setattr__(self, "creds", tuple((_safe_user(u), pw) for (u, pw) in self.creds))
    ...
    def render_for_prompt(self, *, max_per_field: int = 20, high_value_cap: int = 200) -> str:
        """Markdown block suitable for injection into a reflection turn.

        Truncates long lists to keep the prompt under budget. High-value fields
        (users/hashes/creds) get ``high_value_cap`` because a single truncated
        hash/cred/user is a lost foothold the model can't act on; noisy fields
        (paths/hints/hosts) stay at ``max_per_field``. The model is told "...N more".
        """
        if self.is_empty():
            return ""

        def _list_line(label: str, items: tuple, *, fmt: Callable[[object], str] = str, cap: int | None = None) -> str:
            if not items:
                return ""
            limit = cap if cap is not None else max_per_field
            shown = items[:limit]
        ...
        parts.append(_list_line("users", self.users, cap=high_value_cap))
        parts.append(_list_line("shares", self.shares))
        parts.append(_list_line("hashes", self.hashes, cap=high_value_cap))
...
def _parse_etc_passwd(output: str) -> ExtractedFacts:
    ...
    users: list[str] = []
    for m in re.finditer(r"^([a-z_][a-z0-9_-]{0,31}):[^:]*:(\d+):\d+:[^:]*:([^:]*):(\S*)\s*$", output, re.MULTILINE):
...
# Linux privilege-escalation needles. run_command (the privesc workhorse) matches
# no _DISPATCH entry → falls here, so without these the entire recon→foothold→privesc
# phase produced zero durable facts (the NOPASSWD line the model saw once was lost).
_SUDO_NOPASSWD_RE = re.compile(r"\(([^)]*)\)\s*NOPASSWD:\s*(\S+)", re.IGNORECASE)
_SUID_RE = re.compile(r"-rws[rwxst-]{6}\s.*?\s(/\S+)")
_CAP_RE = re.compile(r"(/\S+)\s*=\s*(cap_\w+[+\w]*)", re.IGNORECASE)
_SHADOW_HASH_RE = re.compile(r"^([a-z_][a-z0-9_-]*):(\$(?:1|2[aby]?|5|6|y)\$[^\s:]+)", re.MULTILINE)
_SSH_PRIVKEY_RE = re.compile(r"-----BEGIN (?:OPENSSH|RSA|EC|DSA) PRIVATE KEY-----")


def _extract_privesc(output: str) -> tuple[list[str], list[str]]:
    """Scrape Linux privesc primitives → (hints, hashes). The reflective planner's
    privesc rules gate on these ``privesc:*`` hints; without them the model's own
    discovery (``sudo -l``, SUID, capabilities) never becomes durable context."""
    hints: list[str] = []
    for m in _SUDO_NOPASSWD_RE.finditer(output):
        hints.append(f"privesc:sudo-nopasswd:{m.group(2)}")
    for m in _SUID_RE.finditer(output):
        hints.append(f"privesc:suid:{m.group(1)}")
    for m in _CAP_RE.finditer(output):
        hints.append(f"privesc:cap:{m.group(1)}={m.group(2)}")
    if _SSH_PRIVKEY_RE.search(output):
        hints.append("privesc:ssh-private-key")
    hashes = [m.group(2) for m in _SHADOW_HASH_RE.finditer(output)]
    return hints, hashes
...
def _parse_generic(output: str) -> ExtractedFacts:
    ...
    # Web-cred bruteforce success — wpscan "Valid Combinations Found: | Username: admin, Password: x".
    # The AD-focused _parse_hydra/_parse_nxc patterns miss this comma-separated form, so the cracked
    # WordPress admin password never reached facts.creds and the wp-admin webshell rule couldn't fire.
    # The comma between the two keeps it from matching a login FORM's separate "Username:"/"Password:"
    # labels. Found live on THM Internal (admin:my2boys).
    creds: list[tuple[str, str]] = [
        (m.group(1), m.group(2))
        for m in re.finditer(r"Username:\s*([^\s,|]{1,64})\s*,\s*Password:\s*([^\s|]{1,128})", output, re.IGNORECASE)
    ]
    ...
    # Linux privesc primitives (sudo -l / SUID / capabilities / shadow / SSH keys).
    _pe_hints, _pe_hashes = _extract_privesc(output)
    hints.extend(_pe_hints)
    hashes.extend(_pe_hashes)
...
def extract_facts(tool_invocation: str, output: str) -> ExtractedFacts:
    ...
    if "account found:" in head or re.search(r"\[\d{1,5}\]\[\w+\]\s+host:.*login:.*password:", output, re.IGNORECASE):
