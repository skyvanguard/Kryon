...
def _get_collection():
    ...
    kwargs: dict = {
        ...
        # T4-C3: hnsw:space=cosine so the `similarity = 1 - distance` mapping below is
        # valid. The ChromaDB default is L2 (squared euclidean), which for the
        # non-normalized nomic-embed vectors gives distances ≫1 → `1 - dist` clamps to
        # 0 for EVERYTHING → novelty_gate always reports "likely-novel", anulando su
        # único propósito. NOTE: an existing L2 collection must be dropped + re-ingested
        # for the HNSW index to rebuild under cosine.
        "metadata": {"description": "CVE-with-diff corpus for 0-day hunter", "hnsw:space": "cosine"},
    }
