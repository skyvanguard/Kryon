...
class AsyncVectorDatabase:
    ...
    def __init__(self, persist_directory: str = ".kryon_knowledge/async_db", max_workers: int = 4):
        ...
        # Statistics
        self._stats = {"total_queries": 0, "total_adds": 0, "async_time_saved": 0.0}
    ...
    async def _save_async(self):
        ...
        await loop.run_in_executor(self.executor, self._save_vectors)
    ...
    def _compute_similarities(
        ...
        for doc_id, vec in self.vectors.items():
            ...
            if filter_metadata:
                ...
                match = all(doc_metadata.get(k) == v for k, v in filter_metadata.items())
                if not match:
                    continue
    ...
    async def query_batch_async(
        ...
        - Speedup: ~5x
        """
        tasks = [self.query_async(query, top_k, filter_metadata) for query in queries]
        return await asyncio.gather(*tasks)
...
async def query_async(query_text: str, top_k: int = 5, filter_metadata: Optional[dict] = None) -> list[dict[str, Any]]:
    ...
    Returns:
        List of results
    """
    return await get_async_vector_db().query_async(query_text, top_k, filter_metadata)
