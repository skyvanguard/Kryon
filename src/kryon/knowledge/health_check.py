...
def check_vector_db() -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "healthy", "document_count": count, "operational": True}
    except Exception as e:
        return {"status": "error", "error": str(e), "operational": False}
...
def check_llm() -> dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"status": "healthy", "models_available": len(models), "operational": True}
        else:
            return {"status": "error", "http_status": response.status_code, "operational": False}
    except requests.ConnectionError:
        return {"status": "offline", "error": "Ollama not running", "operational": False}
    except Exception as e:
        return {"status": "error", "error": str(e), "operational": False}
