...
def apply_grounding(
    ...
    *,
    ...
    # A capable model narrates its evidence in prose (e.g. "the jndi header returned a
    # callback in my listener"), not with the literal call_id/step-N/"según output de X"
    # keywords this gate matches. is_capable_model's own docstring flags require_grounding
    # as an over-filter for reasoning-class. Skip the confidence cap for capable — the
    # finding still carries needs_verification from the LLM-emit default (engage F134),
    # so this never promotes an ungrounded finding to confirmed, it only avoids the 0.3
    # cap that buried valid reasoner findings. KRYON_REQUIRE_GROUNDING (auto-on for
    # reasoning-class) then no longer strangles the capable reasoner it was meant to help.
    try:
        from kryon.util.env import is_capable_model  # noqa: PLC0415

        if is_capable_model():
            return 0
    except Exception:  # noqa: BLE001 — grounding must never break the finding pipeline
        pass
