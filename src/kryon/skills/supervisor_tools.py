...
def terminate_hunter(hunter_id: str, reason: str = "") -> str:
    ...
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # Fire-and-forget: we can't await pool.terminate()'s real bool here, so
            # base the reported outcome on whether the hunter actually EXISTS rather
            # than hardcoding True (which claimed success for a bogus/already-finished
            # id — 'terminated: true, status: unknown', a contradictory ack).
            if pool.get(hunter_id) is None:
                terminated = False
            else:
