...
class ValidatorAgent:
    ...
    def phase2b_taint_path(self, f: Finding) -> tuple[str, str]:
        ...
        # A degraded/partial scan is NOT authoritative: the old guard required
        # BOTH a bad status AND empty findings, so a "partial" scan that returned
        # SOME findings slipped through and its incomplete result set was treated
        # as ground truth — if the target function wasn't among the partial hits,
        # phase2b returned "absent" → a HARD REJECT of a real finding just because
        # the taint run didn't finish. Any non-ok status → not-checked.
        if doc.get("status") not in ("ok", None):
            return "not-checked", f"joern status={doc.get('status')} — partial/degraded, not authoritative"
