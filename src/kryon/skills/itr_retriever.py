...
def select_with_itr(
    ...
    *,
    ...
    for name in sorted(always_include):
        # Include UNCONDITIONALLY — the docstring promises always_include "MUST be
        # in the result". The old `if name in index` made it depend on the embedded
        # index being fresh: a newly-pinned tool (e.g. execute_planner_directive)
        # not yet re-embedded, or one skipped by an intermittent build failure, was
        # silently dropped while the function still returned a non-None result (so
        # the caller never fell back to the static selector). The caller resolves
        # names against the registry, so a not-indexed name still binds if it exists.
        if name not in seen:
