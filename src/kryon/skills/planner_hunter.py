...
class SemgrepHunter:
    ...
    def _run_sync(self, job: HunterJob) -> list[dict]:
        ...
        for hit in scan.get("findings") or []:
            ...
            # Include a line bucket in the dedup key: (cwe, rule_id) alone collapsed
            # the SAME rule firing on distinct call sites in one file (e.g. three
            # strcpy() at lines 10/50/90) down to one, silently dropping real
            # findings. The ±radius bucket still folds near-duplicate reports of the
            # same sink (line N vs N+1).
            _line = hit.get("start_line", 0) or 0
            line_bucket = _line // (_DEDUP_LINE_RADIUS * 2 + 1)
            dedup_key = (cwe, rule_id, line_bucket)
    ...
    async def __call__(self, job: HunterJob) -> list[dict]:
        ...
        try:
            ...
            agent = create_unified_agent(
                skills=skills,
                user_msg=job.prompt[: resolve_context_budget(1000)],
...
# Severity ordering for merge — higher value wins. Unifies BOTH vocabularies:
# semgrep/joern (INFO/WARNING/ERROR) AND the standard tiers (LOW/MEDIUM/HIGH/
# CRITICAL) that HeuristicHunter emits for ASAN-verified crashes. The old map
# only knew INFO/WARNING/ERROR, so a MEDIUM finding fell to rank 0 (.get default)
# and a WARNING (2) silently overwrote a confirmed MEDIUM crash in _merge_findings.
_SEV_RANK = {
    "INFO": 1,
    "LOW": 2,
    "WARNING": 2,
