...
from kryon.skills.pre_hook_spec import PreHookSpec, sanitize_ctx_value
...
def _safe_ctx(value: str) -> str:
    """Sanitize a ctx value at assembly time. Delegates to the shared
    sanitizer (kryon.skills.pre_hook_spec.sanitize_ctx_value) so the glue layer
    and the runner enforce ONE charset — no drift. The runner re-applies it at
    substitution time as defense-in-depth."""
    return sanitize_ctx_value(value)


def _clean_hook_name(hook: Any) -> str:
    """Friendly display name for a pre-hook: the tool name, or a python hook's
    basename stripped of path/extension/``_hook`` suffix
    (``./pre_hooks/web_common_paths_hook.py:run`` → ``web_common_paths``)."""
    tool = getattr(hook, "tool", None)
    if tool:
        return str(tool)
    python = str(getattr(hook, "python", "") or "")
    if not python:
        return "análisis"
    name = python.split(":", 1)[0].replace("\\", "/").rsplit("/", 1)[-1]
    if name.endswith(".py"):
        name = name[:-3]
    if name.endswith("_hook"):
        name = name[:-5]
    return name or "análisis"


def build_turn_ctx(user_input: str, session_target: str | None = None) -> dict[str, Any]:
    ...
    ``session_target`` is the REPL's sticky target — used ONLY as a last-resort
    fallback when neither this turn's text nor ``KRYON_TARGET_HOST`` names a host.
    Without it, a continuation turn ("segui con eso") gave the pre_hook a blank
    target while the engine phase still probed the real (sticky) host.
    ...
    # Sanitize each candidate INDEPENDENTLY, then or-chain the sanitized results —
    # with the SAME precedence for host and target. The old code sanitized
    # "env or detected" as one unit for host but "detected or env" for target, so a
    # dirty/URL-shaped KRYON_TARGET_HOST that sanitized to "" zeroed out host (no
    # fallback to the detected host) while target still resolved: a confusing
    # host/target split-brain that aborted host-keyed pre_hooks. Precedence:
    # explicit env var > this turn's detected host > sticky session target.
    env_host = _safe_ctx(os.environ.get("KRYON_TARGET_HOST", ""))
    det_host = _safe_ctx(detected_host)
    session_host = _safe_ctx(session_target or "")
    resolved_host = env_host or det_host or session_host
    return {
        "host": resolved_host,
...
def format_findings_block(findings: dict[str, str]) -> str:
    ...
    if is_capable_model():
        _intro = (
            "_The tools below ran deterministically BEFORE you — a confirmed SEED, "
            "not a ceiling. Build on them: chain toward access/impact and re-run any "
            "tool with different params/endpoints when your hypothesis warrants it._"
        )
    else:
        _intro = (
...
async def maybe_run_pre_hooks(
    ...
    ctx = build_turn_ctx(user_input, session_target)

    # Surface intent up-front so the user sees what's about to happen. Clean
    # (product-grade) by default; the "pre-hook"/path/timeout jargon is opt-in
    # via KRYON_VERBOSE.
    if console is not None:
        from kryon.util.env import is_verbose  # noqa: PLC0415
            ...
            try:
                if verbose:
                    console.print(
                        ...
                        f"[cyan]{hook.tool or hook.python}[/cyan] [dim](timeout {hook.timeout_s}s)[/dim]"
                    )
                else:
                    # Reads as a command being run, not internal scaffolding.
                    console.print(f"[dim cyan]▸[/dim cyan] [cyan]{_clean_hook_name(hook)}[/cyan]")
            except Exception:
                pass
