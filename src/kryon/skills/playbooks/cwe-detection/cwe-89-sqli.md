...
required_tools:
  - run_command
# F203.U.B — pre_hook removed: F191 sqlmap multi-endpoint probe (timeout
# 360s) regressed HTB bench from 33% → 0% pwn rate because it auto-fires
# for any webapp matching SQLi keywords, consuming the per-target wall
# budget. The skill body below still guides the LLM via methodology +
# sqlmap example commands; the operator can invoke sqlmap explicitly via
# `vuln-hunter` skill (which already has the F187 pre_hook).
---
