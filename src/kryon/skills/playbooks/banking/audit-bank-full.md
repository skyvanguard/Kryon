...
# Intent-driven ONLY. This is the heaviest skill (priority 40, runs framework=all
# + consolidated PDF + required compliance pre_hook) — it must NEVER auto-fire from
# port discovery (every host has SSH/HTTPS). Trigger it by explicit bank-audit
# intent, not by a matching port.
triggers:
  tech: []
  ports: []
  keywords:
...
7. Para marcos regulatorios (aplican al conjunto, no a un host específico):
   `pci-dss-4.0`, `swift-csp-2026`, `bcp-py-res-12-2021`.
