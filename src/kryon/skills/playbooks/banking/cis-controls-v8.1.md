...
# Intent-driven ONLY. CIS Controls is a governance framework, not a host
# fingerprint — ports 22/443/8443 are on every host, so port-matching auto-fired
# it (+ generate_compliance_pdf + required pre_hook) on any target. Request it by
# keyword ("cis controls", "ig1"…) or `kryon engage --framework cis-controls`.
triggers:
  tech: []
  ports: []
  keywords:
