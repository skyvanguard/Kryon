...
# Intent-driven ONLY. A PCI-DSS compliance audit has no fingerprint port —
# port 443/8443 is EVERY https site, and generic words like "audita"/"audit"/
# "compliance"/"hardening" match any audit request. Both auto-activated this
# heavy skill (+ its generate_compliance_pdf tool + required run_compliance_audit
# pre_hook) on a plain `audita <web>` — the 4B then foot-gunned into a 7-min PDF
# turn. PCI must be requested explicitly (pci/cardholder/tarjetas keywords, or
# `kryon engage --framework pci_dss`), never inferred from "has HTTPS".
triggers:
  tech: []
  ports: []
  keywords:
