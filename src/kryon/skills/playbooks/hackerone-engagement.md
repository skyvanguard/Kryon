...
### Step 3 — Pattern library lookup (from F64)

Before touching the target, pull prior findings on similar programs
/ tech stacks:

```python
query_similar_findings(
    query="H1 program <handle>",
    tech_fingerprint="<best-guess-from-scope>",
    k=10,
)
```

Returns top findings across prior engagements — CWE classes most
commonly found on similar targets. Use this to inform which banking
context you'll pass to run_web_pentest.

### Step 4 — Scope-guarded pentest
...
### Step 5 — Review findings with operator
...
### Step 6 — Submit report (approval-gated)
...
2. Próximo engagement en programa similar → `query_similar_findings`
   lo retrieve primero.
3. Con suficientes engagements, la biblioteca se convierte en el moat
