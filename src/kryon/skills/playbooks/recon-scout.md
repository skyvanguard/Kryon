...
2. Lograste el objetivo de tu régimen: en modo OFENSIVO (capable), IMPACTO real
   (RCE ejecutado / dump usado / creds validadas / admin) o vectores agotados —
   confirmar el vector NO es parar, encadená (SQLi→dump→creds→login, upload→RCE);
...
> ⚠ **`<TARGET>` es un marcador — reemplazalo por el objetivo REAL que te dio el
> operador (una URL/IP/host concreto). Si NO hay un objetivo concreto, PEDÍSELO
> al usuario y esperá su respuesta. NUNCA ejecutes contra el literal `<TARGET>`,
> `HOST` u otro placeholder: no resuelve y entrás en loop.**

Ejecutá en orden, **solo si el operador ya dio un target concreto**:

1. `nmap(target=<TARGET>, args="-sV -sC -T4")` — puertos y servicios
2. `whatweb_scan(target="<TARGET>")` — tech fingerprint
3. `run_command(command="curl -s <TARGET>/ | grep -oE 'href=\"[^\"]+\"' | head -50")` — extraer enlaces para mapear superficie real
4. `run_command(command="curl -s <TARGET>/robots.txt")` y `sitemap.xml` — paths declarados
5. `run_command(command="curl -sI <TARGET>/")` — security headers, server header
...
- **Stop conditions**: `KRYON_HUNT_MAX_TURNS` turns alcanzados, O lograste el objetivo del régimen (ofensivo: IMPACTO real / vectores agotados — no solo confirmar el vector, encadená hacia dump/creds/RCE/admin primero; recon/banca-safe: vuln crítica confirmada sin explotar), O operador dice stop.
