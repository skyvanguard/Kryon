...
**Regla**: el paso 3 (`uid=` confirmado sobre la sesión) es el PISO
mínimo, no el techo: no emitas resumen final antes de tenerlo, y si queda
una lead seguí encadenando (privesc, lateral, persistence) en vez de
resumir. El operator-input request es el off-ramp del modelo local chico
en loop, no un corte a los 4 turns.
