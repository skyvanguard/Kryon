...
from kryon.skills.pre_hook_spec import (
    _TEMPLATE_RE,  # shared: no parse/runtime drift
...
def _substitute_one(value: Any, ctx: dict[str, Any]) -> Any:
    ...
    Recurses into lists/dicts so a template nested in
    `args: {hosts: ["{ctx.host}"]}` is substituted too. Other non-string
    scalars pass through unchanged. Unknown vars (after parse-time validation)
    become an empty string with a warning — defensive, should not happen if
    the spec was parsed via `parse_pre_hooks`.
    """
    if isinstance(value, dict):
        return {k: _substitute_one(v, ctx) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_substitute_one(v, ctx) for v in value]
    ...
    def replace(match) -> str:
        ...
        # Defense-in-depth: sanitize the VALUE here, not just in the glue layer
        # (build_turn_ctx). The runner is the security-relevant primitive — this
        # holds even if a future caller passes a raw, unsanitized ctx straight to
        # run_pre_hooks. Values reach shell=True tools, so a metachar → "".
        return sanitize_ctx_value(str(sub_value) if sub_value is not None else "")
...
def _resolve_python_callable(hook: PreHookSpec) -> Callable[..., Any]:
    ...
    Sandbox rules (path-based — Fase 5 baseline):
      ...
      - The resolved file path MUST be inside its own skill's directory
        (`source_dir`). `.resolve()` follows symlinks, so escape-via-symlink
        is caught. This boundary is layout-independent (dev src/ tree, Docker
        site-packages install, and tests all work).
    ...
    source_root = Path(hook.source_dir).resolve()
    file_path = (source_root / rel).resolve()

    # Sandbox: the hook file MUST resolve inside its OWN skill's directory
    # (source_dir). This boundary is layout-independent — it holds in the
    # editable src/ tree, the Docker site-packages install, AND tests — unlike
    # the previous `parents[3]`, which in the Docker image (package installed
    # without the src/ prefix, see Dockerfile.kali) pointed at the entire venv
    # lib dir, silently widening containment to every installed dependency.
    # `.resolve()` above already collapsed any `..`/symlink, so a `python:
    # ../../etc/x.py` escaping the skill dir is caught here (the schema also
    # bans `..` — this is the defense-in-depth layer).
    try:
        file_path.relative_to(source_root)
    except ValueError as e:
        raise PreHookExecutionError(
            f"python hook path {file_path} resolves outside its skill dir ({source_root}) — refused"
        ) from e
...
async def run_pre_hooks(
    ...
    On a `required: true` failure: runs the REMAINING hooks anyway (an
    independent later hook shouldn't be discarded because an earlier required
    one failed/hung), then raises PreHookExecutionError aggregating all
    required failures once every hook has had its turn.
    ...
    for hook in hooks:
        try:
            ...
            label = hook.tool or hook.python
            if hook.required:
                # Collect, keep going — raise once all hooks have run so an
                # independent later hook still gets to inject its context.
                logger.warning("pre_hook (required) failed: %s", e)
                required_failures.append(str(e))
                out[hook.inject_as] = f"[pre_hook {label!r} FAILED (required): {e}]"
            else:
                logger.warning("pre_hook (required=false) failed: %s", e)
                out[hook.inject_as] = f"[pre_hook {label!r} failed: {e}]"

    if required_failures:
        raise PreHookExecutionError("required pre_hook(s) failed: " + " | ".join(required_failures))
