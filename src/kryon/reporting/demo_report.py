"""Demo-oriented report renderer for example engagements.
...
Usage:
    ...
    html_path, pdf_path = render_demo_report(
        ...
        context={
            "client_name": "example",
            "engagement_id": "example-demo-2026-04-15",
            "target_scope": "192.168.1.0/24",
            "auditor": "Your Security Firm / Kryon",
        },
        output_dir="/workspace/reports",
    )

The generated PDF is the file we hand to the client. No LLM required.
"""
...
def _needs_review(finding: dict) -> bool:
    """F210 Fase 5 — a finding flagged needs_verification is inferred /
    heuristic (banner-spoofable version, version→CVE backport-prone, or a
    check that couldn't complete). It is NOT ground truth."""
    return bool(finding.get("needs_verification"))


def _render_review_section(review: list[dict]) -> str:
    """F210 Fase 5 — dedicated "requiere verificación" section so the
    inferred/heuristic findings are visibly separated from confirmed ones.
    Empty string when there's nothing to review (backward compatible)."""
    if not review:
        return ""
    rows: list[str] = []
    for i, f in enumerate(review, 1):
        ...
        cls = sev.lower()
        level = _html.escape(str(f.get("verification_level", "") or "heuristic"))
        rows.append(
            f"<tr>"
            f"<td>{i}</td>"
    ...
    cards = "".join(_finding_card_html(f) for f in review)
    return f"""
<h2>⚠ Requiere verificación ({len(review)})</h2>
<div class="summary" style="border-left-color:#744210;background:#fffaf0;">
  <p>Estos hallazgos derivan de señales <strong>inferidas</strong>: versión/banner
  spoofeable, mapeo versión→CVE sujeto a backports del proveedor, o un chequeo que
  no pudo completarse. <strong>No son ground truth</strong> — confirmá manualmente
  cada uno (probe activo o revisión de configuración) antes de entregarlo al cliente.
  No se contabilizan como hallazgos confirmados.</p>
</div>
<table class="findings-table">
<thead><tr>
<th>#</th><th>Severidad</th><th>CWE</th><th>Host</th>
<th>Regla</th><th>Nivel</th><th>Descripción</th>
</tr></thead>
...
def render_html(findings: list[dict], context: dict) -> str:
    ...
    ctx.setdefault("auditor", "Your Security Firm / Kryon")
    ...
    # F210 Fase 5 — split confirmed (ground truth) from needs-review
    # (inferred/heuristic). The index + detail render confirmed only; the
    # review items get a dedicated section below. When nothing is flagged
    # (the common case) `confirmed == findings_sorted` → output unchanged.
    confirmed = [f for f in findings_sorted if not _needs_review(f)]
    review = [f for f in findings_sorted if _needs_review(f)]

    # Findings table (quick overview) — confirmed only.
    rows: list[str] = []
    ...
    table_html = "".join(rows) or (
        '<tr><td colspan="6" style="text-align:center;color:#718096;">Sin hallazgos confirmados</td></tr>'
    )

    # Finding cards (detailed) — confirmed only.
    cards_html = "".join(_finding_card_html(f) for f in confirmed)

    # Dedicated "requiere verificación" section (empty when none).
    review_html = _render_review_section(review)
...
<div class="footer">
  Reporte generado por Kryon — Your Security Firm. · Confidencial.
</div>
</body></html>
"""
