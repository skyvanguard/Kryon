...
def is_finding_shape(obj: Any) -> bool:
    """True when ``obj`` looks like a finding dict.

    ``severity`` is NOT required: the model routinely emits ``{cwe, message,
    host}`` without it, and requiring the key dropped those findings entirely. A
    strong marker (``cwe``/``rule_id``) identifies a finding on its own; otherwise
    ``severity`` plus a descriptive field is needed to avoid matching tool-calls or
    arbitrary dicts."""
    if not isinstance(obj, dict):
        return False
    if any(k in obj for k in ("cwe", "rule_id")):
        return True
    return "severity" in obj and any(k in obj for k in ("message", "host", "title"))
