...
class SimpleVectorDatabase:
    ...
    def query(
        ...
        for doc_id, doc_vector in self.vectors.items():
            ...
            if filter_metadata:
                match = all(
                    self.metadatas[doc_id].get(key) == value
    ...
    def __init__(self, persist_directory: str = ".skynet_knowledge/chromadb"):
        ...
        try:
            ...
            self._client = chromadb.Client(
                Settings(persist_directory=str(persist_path), anonymized_telemetry=False)
            )
    ...
    def query(
        ...
        if self.backend_type == "chromadb":
            results = self.backend.query(
                query_texts=[query_text], n_results=top_k, where=filter_metadata
            )
            formatted_results = []
