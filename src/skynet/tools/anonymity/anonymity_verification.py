...
def check_dns_leak(expected_dns_servers: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Method 2: Read /etc/resolv.conf (Linux/Mac)
            ...
            subprocess.run(["test", "-f", "/etc/resolv.conf"], capture_output=True).returncode == 0
        ):
...
def check_webrtc_leak() -> Dict[str, Any]:
    ...
    try:
        ...
        results["success"] = True
        results["error"] = (
            results.get("error") or "Visit https://browserleaks.com/webrtc to check manually"
        )
    ...
    try:
        ...
        if tz_result["leak_detected"]:
            ...
            results["issues"].append(
                f"Timezone leak: {tz_result.get('system_timezone', 'unknown')}"
            )
        else:
    ...
    try:
        ...
        if results["score"] >= 90:
            ...
            results["recommendation"] = (
                "Critical anonymity failure. Do not proceed with sensitive operations."
            )
