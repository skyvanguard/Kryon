...
def strip_exif_metadata(
    image_path: str, output_path: Optional[str] = None, tool: str = "exiftool"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if tool == "exiftool":
            ...
            if check.returncode != 0:
                results["error"] = (
                    "exiftool not installed. Install: apt install libimage-exiftool-perl"
                )
                return results
            ...
            metadata_lines = len(
                [line for line in metadata_result.stdout.split("\n") if ":" in line]
            )
            ...
            if output_path and output_path != image_path:
                # Create clean copy
                subprocess.run(
                    ["exiftool", "-all=", "-o", output_path, image_path], capture_output=True
                )
            else:
                # Overwrite original
                subprocess.run(
                    ["exiftool", "-all=", "-overwrite_original", image_path], capture_output=True
                )
...
def strip_pdf_metadata(pdf_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        check = subprocess.run(
            ["which", "exiftool"] if os.name != "nt" else ["where", "exiftool"], capture_output=True
        )
        ...
        if check.returncode == 0:
            ...
            if output_path and output_path != pdf_path:
                subprocess.run(
                    ["exiftool", "-all=", "-o", output_path, pdf_path], capture_output=True
                )
            else:
                subprocess.run(
                    ["exiftool", "-all=", "-overwrite_original", pdf_path], capture_output=True
                )
            ...
            try:
                ...
                results["error"] = (
                    "exiftool and PyPDF2 not available. Install one: pip install PyPDF2"
                )
            except Exception as e:
...
def strip_office_metadata(
    doc_path: str, output_path: Optional[str] = None, doc_type: str = "auto"
) -> Dict[str, Any]:
    """
        ...
        if doc_type in ["docx", "xlsx", "pptx"]:
            try:
                ...
                with zipfile.ZipFile(
                    output_path or doc_path, "w", zipfile.ZIP_DEFLATED
                ) as zip_write:
            ...
            if check.returncode == 0:
                subprocess.run(
                    ["exiftool", "-all=", "-overwrite_original", doc_path], capture_output=True
                )
                results["success"] = True
            else:
    ...
    try:
        ...
        if check_ffmpeg.returncode == 0:
            ...
            if check_exiftool.returncode == 0:
                ...
                # Strip metadata
                subprocess.run(
                    ["exiftool", "-all=", "-overwrite_original", video_path], capture_output=True
                )
    ...
    try:
        ...
        if strip_metadata:
            if ext in [".jpg", ".jpeg", ".png", ".tiff", ".gif"]:
                ...
                if check.returncode == 0:
                    subprocess.run(
                        ["exiftool", "-all=", "-overwrite_original", file_path], capture_output=True
                    )
        ...
        if randomize_timestamps:
            ...
            from skynet.tools.evasion.timestomping import stomp_file_timestamps

            # Random timestamp from past 1-3 years
            days_ago = random.randint(365, 1095)
            random_timestamp = time.time() - (days_ago * 24 * 3600)

            stomp_result = stomp_file_timestamps(
                file_path=output_path or file_path, timestamp=random_timestamp
            )
...
def timezone_from_metadata(file_path: str) -> Dict[str, Any]:
    ...
    Example:
        >>> from skynet.tools.anonymity import timezone_from_metadata
        >>>
    ...
    try:
        ...
        metadata = subprocess.run(
            ["exiftool", "-time:all", "-gps:all", file_path], capture_output=True, text=True
        )
