"""
SKYNET WiFi Penetration - Attack Tools
...
def scan_wifi_networks(
    interface: str = "wlan0", timeout: int = 30, channel: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if "Mode:Monitor" not in check_iface.stdout:
            results["error"] = (
                f"Interface {interface} not in monitor mode. Run enable_monitor_mode() first."
            )
            return results
...
def capture_handshake(
    ...
    try:
        ...
        airodump_process = subprocess.Popen(
            airodump_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
