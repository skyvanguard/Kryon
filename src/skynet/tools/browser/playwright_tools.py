...
from skynet.sdk.tools import function_tool
...
class BrowserManager:
    ...
    async def initialize(cls, headless: bool = True):
        ...
        if cls._page is None:
            ...
            cls._network_logs = []
            cls._page.on("request", lambda request: cls._network_logs.append({
                "type": "request",
                "url": request.url,
            ...
            cls._page.on("response", lambda response: cls._network_logs.append({
                "type": "response",
                "url": response.url,
...
def browser_fill_form(
    ...
    async def _fill_form():
        ...
        try:
            ...
            return json.dumps({
                "success": False,
                "error": "Invalid JSON in form_data parameter"
            }, indent=2)
        except Exception as e:
            return json.dumps({
                "success": False,
