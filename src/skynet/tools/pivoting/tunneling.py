...
def ssh_remote_port_forward(
    ...
    try:
        ...
        if background:
            check = subprocess.run(
                ["pgrep", "-f", f"ssh.*-R.*{remote_port}"], capture_output=True, text=True
            )
...
def ssh_dynamic_port_forward(
    ...
    try:
        ...
        if background:
            check = subprocess.run(
                ["pgrep", "-f", f"ssh.*-D.*{socks_port}"], capture_output=True, text=True
            )
...
def setup_chisel_tunnel(
    ...
    try:
        ...
        if check.returncode != 0:
            results["error"] = (
                "chisel not found - download from https://github.com/jpillora/chisel/releases"
            )
            return results
