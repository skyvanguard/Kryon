...
def run_linpeas(output_file: str = "/tmp/linpeas.txt", thorough: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        download_result = generic_linux_command("sh", f"-c '{download_cmd}'")
        ...
        cmd_result = generic_linux_command("sh", f"{linpeas_path} {linpeas_args}")
    ...
    try:
        ...
        if binary_lower in gtfobins_db:
            if escalation_type in gtfobins_db[binary_lower]:
                ...
                result["success"] = True
                result["description"] = f"Binary found in GTFOBins but no {escalation_type} technique available. Check {result['gtfobins_url']}"
        else:
            result["success"] = True
