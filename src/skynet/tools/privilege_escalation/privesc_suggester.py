...
def _generate_linux_recommendations(analysis: Dict[str, Any]) -> List[str]:
    ...
    if analysis.get("suid_binaries"):
        recommendations.append(
            "Review SUID binaries with GTFOBins (https://gtfobins.github.io/) "
    ...
    if analysis.get("writable_files"):
        recommendations.append(
            "Check writable files for sensitive configuration files or scripts "
            "that run with elevated privileges"
        )

    if analysis.get("cron_jobs"):
        recommendations.append(
            "Review cron jobs for writable scripts or paths that can be hijacked"
        )
...
def _generate_windows_recommendations(analysis: Dict[str, Any]) -> List[str]:
    ...
    if analysis.get("unquoted_services"):
        recommendations.append(
            "Exploit unquoted service paths by placing malicious executables "
            "in paths with spaces"
        )

    if analysis.get("always_install_elevated"):
        recommendations.append(
            "Generate malicious MSI using msfvenom and install to gain SYSTEM privileges"
        )

    if analysis.get("weak_permissions"):
        recommendations.append(
            "Check service binary permissions and replace with malicious binaries"
        )

    if analysis.get("auto_logon"):
        recommendations.append(
            "Auto-logon credentials found in registry - use to authenticate as that user"
        )

    if not recommendations:
        recommendations.append(
            "No obvious privilege escalation vectors found. "
            "Consider token abuse, DLL hijacking, or kernel exploits"
        )
