...
def auto_enumerate_target(
        ...
        for line in nmap_output.stdout.split("\n"):
            ...
            if port_match:
                ...
                if port_info["service"] in ["http", "https", "ssl/http"]:
                    protocol = (
                        "https"
                        if "ssl" in port_info["service"] or port_info["port"] == 443
                        else "http"
                    )
        ...
        for url in web_targets:
            try:
                ...
                for line in gobuster_output.stdout.split("\n"):
                    ...
                    if dir_match:
                        directories.append(
                            {"path": dir_match.group(1), "status": int(dir_match.group(2))}
                        )
    ...
    # SMB enumeration
    if "smb" in results["interesting_services"] or any(
        p["port"] in [139, 445] for p in results["open_ports"]
    ):
    ...
    if results["web_services"]:
        results["recommendations"].append(
            "Check web services for common vulnerabilities (SQLi, LFI, RFI)"
        )
...
def search_exploits(
    ...
    try:
        searchsploit_output = subprocess.run(
            searchsploit_cmd, capture_output=True, text=True, timeout=30
        )
        ...
        try:
            ...
            for line in searchsploit_output.stdout.split("\n"):
                if "|" in line and "Exploit Title" not in line:
                    ...
                    if len(parts) >= 2:
                        results["searchsploit_results"].append(
                            {"title": parts[0].strip(), "path": parts[1].strip()}
                        )
    ...
    if results["searchsploit_results"]:
        results["recommendations"].append(
            f"Copy exploit: searchsploit -m {results['searchsploit_results'][0]['path']}"
        )

    if results["metasploit_modules"]:
        results["recommendations"].append(
            f"Use Metasploit: use {results['metasploit_modules'][0]['name']}"
        )

    if results["cve_references"]:
        results["recommendations"].append(
            f"Research CVEs: {', '.join(set(results['cve_references'][:3]))}"
        )
...
def auto_privilege_escalation(
            ...
            if linpeas_result.get("critical_findings"):
                for finding in linpeas_result["critical_findings"][:3]:
                    results["quick_wins"].append(
                        {"source": "linpeas", "type": "critical", "description": finding}
                    )

            print(
                f"[+] LinPEAS found {len(linpeas_result.get('critical_findings', []))} critical findings"
            )
    ...
    if check_capabilities:
        ...
        try:
            ...
            cap_output = subprocess.run(
                getcap_cmd, shell=True, capture_output=True, text=True, timeout=120
            )
    ...
    if results["quick_wins"]:
        results["recommendations"].append(
            f"[PRIORITY] Quick win available: {results['quick_wins'][0]['description']}"
        )
        results["recommendations"].append(f"Execute: {results['quick_wins'][0]['command']}")

    if results["sudo_exploits"]:
        results["recommendations"].append(
            f"Try sudo exploit: {results['sudo_exploits'][0]['command']}"
        )

    if results["suid_exploits"]:
        results["recommendations"].append(
            f"Try SUID exploit: {results['suid_exploits'][0]['command']}"
        )

    if not results["quick_wins"]:
        results["recommendations"].append(
            "No quick wins found - check LinPEAS output for manual exploitation"
        )
...
def hunt_flags(
    ...
    if results["root_flag"] and not results["user_flag"]:
        results["recommendations"].append(
            "Root flag found but no user flag - check /home directories"
        )
    ...
    if results["interesting_files"]:
        results["recommendations"].append(
            f"Check {len(results['interesting_files'])} interesting files for more flags"
        )
...
def generate_ctf_report(
    ...
    if enumeration_results:
        ...
        if enumeration_results.get("open_ports"):
            ...
            for port in enumeration_results["open_ports"]:
                report_lines.append(
                    f"- {port['port']}/{port['protocol']}: {port['service']} {port.get('version', '')}"
                )
        ...
        if enumeration_results.get("interesting_services"):
            ...
            for service, details in enumeration_results["interesting_services"].items():
                if isinstance(details, dict):
                    report_lines.append(
                        f"- **{service.upper()}:** {details.get('version', 'No version info')}"
                    )
    ...
    # Footer
    report_lines.append("\n---\n")
    report_lines.append("*Report generated by SKYNET CTF Master*")
