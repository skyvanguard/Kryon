...
def analyze_target(
    ...
    recommended_tools = _get_recommended_tools(
        target_type, scope, stealth_level, time_constraint
    )

    # Create execution strategy
    strategy = _create_execution_strategy(
        target, target_type, recommended_tools, scope
    )
...
def _sort_by_priority(tools: List[Dict], priority: str) -> List[Dict]:
    ...
    if sort_key == "capabilities":
        tools.sort(key=lambda x: len(TOOL_CAPABILITIES.get(x["tool"], {}).get("capabilities", [])), reverse=True)
    else:
...
def _generate_reasoning(keywords: List[str], tools: List[Dict]) -> str:
    ...
    reasons = [
        ...
        f"Tools are ordered by relevance and suitability scores."
    ]
