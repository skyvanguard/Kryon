...
def _get_workspace_dir() -> str:
    ...
    if workspace_name:
        if not all(c.isalnum() or c in ['_', '-'] for c in workspace_name):
            print(color(f"Invalid SKYNET_WORKSPACE name '{workspace_name}'. "
...
def _get_container_workspace_path() -> str:
    ...
    if workspace_name:
        if not all(c.isalnum() or c in ['_', '-'] for c in workspace_name):
            print(color(f"Invalid SKYNET_WORKSPACE name '{workspace_name}' for container. "
...
async def _run_local_async(command, stdout=False, timeout=100, stream=False, call_id=None, tool_name=None, workspace_dir=None, custom_args=None):
    ...
    try:
        ...
        if stream:
            ...
            from skynet.util import start_tool_streaming, update_tool_streaming, finish_tool_streaming
