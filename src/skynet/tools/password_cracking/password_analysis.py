    ...
    try:
        ...
        patterns = {
            ...
            "contains_season": sum(
                1
                for p in passwords
                if any(s in p.lower() for s in ["spring", "summer", "fall", "winter", "autumn"])
            ),
            "contains_month": sum(
                1
                for p in passwords
            ...
            "leet_speak": sum(
                1 for p in passwords if any(c in p for c in ["@", "3", "1", "0", "$", "7"])
            ),
        }
        ...
        results["top_passwords"] = [
            {"password": pwd, "count": count} for pwd, count in password_counts.most_common(10)
        ]
        ...
        results["character_analysis"] = {
            "most_common_chars": [
                {"char": char, "count": count} for char, count in char_freq.most_common(10)
            ],
            "digit_usage": {str(i): all_chars.count(str(i)) for i in range(10)},
            "special_char_usage": {
                c: all_chars.count(c) for c in string.punctuation if all_chars.count(c) > 0
            },
        }
        ...
        if results["statistics"]["min_length"] >= 12:
            recommendations.append(
                "Passwords are relatively long (12+ chars) - use GPU acceleration"
            )
        else:
            recommendations.append(
                "Passwords are short - brute force feasible for remaining hashes"
            )
        ...
        if results["common_patterns"]["contains_year"] > 50:
            recommendations.append(
                "Year patterns common (2020-2029) - generate wordlist with year suffixes"
            )
...
def generate_custom_wordlist(
    ...
    try:
        ...
        base_words = list(set([w for w in base_words if len(w) >= 3]))
