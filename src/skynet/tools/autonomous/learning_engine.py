...
class LearningEngine:
    ...
    def get_learned_recommendations(
        ...
        for pattern in matching_patterns:
            if pattern["confidence_score"] >= min_confidence:
                ...
                exploit_scores[exploit_name]["score"] = max(
                    exploit_scores[exploit_name]["score"], score
                )
        ...
        ranked_exploits = sorted(exploit_scores.items(), key=lambda x: x[1]["score"], reverse=True)[
            :top_n
        ]
        ...
        recommendations = {
            ...
            "recommendation_confidence": sum(d[1]["confidence"] for d in ranked_exploits)
            / len(ranked_exploits)
            if ranked_exploits
            else 0.0,
        }
    ...
    def _create_pattern_from_exploit(self, operation: dict, exploit: dict, success: bool) -> dict:
        ...
        pattern_id = hashlib.sha256(
            f"{json.dumps(target_chars)}{exploit['name']}".encode()
        ).hexdigest()[:16]
        ...
        if existing:
            ...
            if pattern["success"]:
                ...
                if avg_time and pattern["time_taken"]:
                    avg_time = (
                        avg_time * (success_count - 1) + pattern["time_taken"]
                    ) / success_count
    ...
    def export_knowledge(self, export_path: str) -> dict[str, Any]:
        ...
        columns = [desc[0] for desc in cursor.description]
        export_data["data"]["exploit_stats"] = [
