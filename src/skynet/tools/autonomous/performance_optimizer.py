...
class PerformanceOptimizer:
    ...
    def analyze_performance(
        self, time_window_days: int = 30, min_samples: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for op_type, data in timing_data.items():
            ...
            if len(success_times) >= 3:
                insights[op_type] = {
                    ...
                    "success_stdev": statistics.stdev(success_times)
    ...
    def _generate_recommendations(self, report: Dict) -> List[str]:
        ...
        if success_rate < 0.3:
            recommendations.append(
                "LOW_SUCCESS_RATE: Consider reviewing exploit database and updating techniques"
            )
        ...
        top_exploits = [
            e for e in report["exploit_rankings"] if e["recommendation"] == "HIGHLY_RECOMMENDED"
        ]
        ...
        if report["timing_insights"]:
            recommendations.append(
                "OPTIMIZE_TIMEOUTS: Apply recommended timeouts from timing analysis"
            )
