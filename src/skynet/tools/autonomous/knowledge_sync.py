...
class KnowledgeSync:
    ...
    def _get_instance_id(self) -> str:
        ...
        instance_id = hashlib.md5(f"{time.time()}{id_file}".encode()).hexdigest()[:16]
    ...
    def export_knowledge(
        ...
        for row in cursor:
            ...
            if filter_sensitive:
                pattern["target_characteristics"] = self._anonymize_characteristics(
        ...
        for pattern in imported.get("patterns", []):
            ...
            if existing:
                ...
                if merge_strategy == "best":
                    ...
                    pattern["pattern_id"] = (
                        f"{pattern['pattern_id']}_imported_{source_instance[:8]}"
                    )
    ...
    def _merge_patterns_avg(
        self, existing: Dict, imported: Dict, imported_confidence: float
    ) -> Dict:
        ...
        if direction in ["pull", "both"]:
            try:
                response = requests.get(
                    f"{remote_url}/api/knowledge/export", headers=headers, timeout=30
                )
