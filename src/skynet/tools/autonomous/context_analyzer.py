...
class ContextAnalyzer:
    ...
    def _build_hint_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "port_hint": re.compile(
                r"(?:port|service)\s+(\d{1,5})\s+(?:is|runs?|listening|open)", re.IGNORECASE
            ),
    ...
    def _build_entity_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "hostname": re.compile(
                r'(?:host(?:name)?|server)\s*[=:]\s*["\']?([a-zA-Z0-9_.-]+)["\']?', re.IGNORECASE
            ),
            "database": re.compile(
                r'(?:database|db|schema)\s*[=:]\s*["\']?([a-zA-Z0-9_.-]+)["\']?', re.IGNORECASE
            ),
            "table": re.compile(
                r'(?:table|collection)\s*[=:]\s*["\']?([a-zA-Z0-9_.-]+)["\']?', re.IGNORECASE
            ),
            "version": re.compile(
                r'(?:version|ver|v)\s*[=:]\s*["\']?([\d.]+)["\']?', re.IGNORECASE
            ),
        }
    ...
    def autonomous_context_analysis(
        ...
        results["secrets_found"] = list(set(tuple(s.items()) for s in results["secrets_found"]))
    ...
    def autonomous_hint_following(
        ...
        for hint in hints:
            ...
            if hint_type == "todo":
                ...
                if any(
                    keyword in content.lower() for keyword in ["sql", "injection", "xss", "vuln"]
                ):
                    tasks.append(
                        {
    ...
    def _generate_recommendations(
        self, analysis_results: Dict[str, Any], objective: str
    ) -> List[str]:
        ...
        if creds_count > 0:
            recommendations.append(
                f"Try {creds_count} discovered credential(s) on all accessible services"
            )

        # Hint recommendations
        high_priority_hints = [
            h for h in analysis_results["hints_discovered"] if h.get("priority") == "high"
        ]
        if high_priority_hints:
            recommendations.append(
                f"Investigate {len(high_priority_hints)} high-priority hint(s) immediately"
            )
        ...
        if usernames:
            recommendations.append(
                f"Attempt password spray/bruteforce with {len(usernames)} discovered username(s)"
            )

        # Secret recommendations
        if analysis_results["secrets_found"]:
            recommendations.append(
                "Review discovered secrets for API keys or tokens that may grant access"
            )
...
def analyze_context(
    target_data: Dict[str, Any], operation_objective: str = "general"
) -> Dict[str, Any]:
    """
...
def follow_hints(
    hints: List[Dict[str, Any]], current_access: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
