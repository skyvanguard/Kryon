...
class CVEScraper:
    ...
    def _scrape_nvd(
        self, services: Optional[List[str]], days_back: int, severity_min: str
    ) -> List[Dict]:
                    ...
                    if output.returncode == 0:
                        try:
                            ...
                            for exploit in exploits[:10]:  # First 10 per service
                                results.append(
                                    {
                                        "cve_id": exploit.get("Codes", ["N/A"])[0]
                                        if exploit.get("Codes")
                                        else "N/A",
                    ...
                    if response.status_code == 200:
                        ...
                        for repo in repos:
                            results.append(
                                {
                                    "cve_id": "GITHUB_POC",
                                    "description": repo.get("description", repo.get("name", ""))[
                                        :200
                                    ],
                                    "severity": "UNKNOWN",
                                    "source": "GitHub",
