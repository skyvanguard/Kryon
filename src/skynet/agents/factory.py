...
def discover_agent_factories() -> dict[str, Callable[[], Agent]]:
    ...
    import skynet.agents

    agent_factories = {}

    # Scan the agents module for all agent definitions
    for _importer, modname, ispkg in pkgutil.iter_modules(skynet.agents.__path__, skynet.agents.__name__ + "."):
        if ispkg:
    ...
    if os.path.exists(patterns_path):
        for _importer, modname, ispkg in pkgutil.iter_modules([patterns_path], skynet.agents.__name__ + ".patterns."):
            if ispkg:
                continue
