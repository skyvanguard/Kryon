...
from skynet.sdk.agents.handoffs import handoff as handoff
...
def _get_target_validator():
    ...
    if _target_validator is None:
        from skynet.agents.target_validator import target_validator
...
def _get_transfer_to_flag_discriminator():
    ...
    if _transfer_to_flag_discriminator is None:
        from skynet.agents.target_validator import transfer_to_flag_discriminator
...
def get_available_agents() -> dict[str, Agent]:  # pylint: disable=R0912  # noqa
    ...
    from skynet.agents.patterns import PATTERNS
...
def get_agent_module(agent_name: str) -> str:
    ...
    Returns:
        ...
        is defined (e.g., 'skynet.sdk.agents.basic')
    """
    # Try to import all agents from the agents folder
    for _, name, _ in pkgutil.iter_modules(__path__, __name__ + "."):
        try:
...
def get_agent_by_name(
    ...
    from skynet.agents.factory import get_agent_factory
