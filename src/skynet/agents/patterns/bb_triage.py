...
_bb_triage_swarm_pattern = None


def _initialize_bb_triage_pattern():
    """Initialize the bb_triage swarm pattern lazily."""
    global _bb_triage_swarm_pattern

    if _bb_triage_swarm_pattern is not None:
        return _bb_triage_swarm_pattern

    # Import here to avoid circular imports
    from skynet.agents.retester import retester_agent
    from skynet.agents.t1000_hunter import bug_bounter_agent
    from skynet.sdk.agents import handoff
    from skynet.util import append_instructions
    ...
    _bug_bounter_agent_copy.description = (
        "Agent that specializes in vulnerability discovery and bug bounty hunting without false positives"
    )

    # Add handoff instructions to Bug Bounter agent
    append_instructions(
        _bug_bounter_agent_copy,
        "\n\nWhen you discover potential vulnerabilities, transfer to the Retester Agent for verification and triage.",
    )
    ...
    _bug_bounter_agent_copy.pattern = "swarm"
    _bb_triage_swarm_pattern = _bug_bounter_agent_copy
...
def __getattr__(name):
    ...
    if name == "bb_triage_swarm_pattern":
        return _initialize_bb_triage_pattern()
