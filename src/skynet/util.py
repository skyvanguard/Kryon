...
_CLAUDE_THINKING_PANELS: dict[str, Any] = {}
            ...
            if callable(handoff_fn) and not hasattr(handoff_fn, "agent_name"):
                ...
                try:
                    ...
                    for module_name, module in sys.modules.items():
                        if module_name.startswith("skynet.agents"):
                    ...
                    if next_agent:
                        ...
                        transfers_node.add(f"[yellow]🤖 {handoff_name} via {handoff_fn.tool_name}[/yellow]")
                except Exception as e:
...
def cli_print_tool_output(
                ...
                if call_id in _LIVE_STREAMING_PANELS:
                    with _PANEL_UPDATE_LOCK:
                        ...
                        if isinstance(panel_info, dict) and panel_info.get("type") == "static":
                            ...
                            # For parallel/container mode, only update if final with different content
                            if execution_info and execution_info.get("is_final", False):
                                # Debug output
                                ...
                                # Don't show another one - the original panel is complete
                                # Panel title shows "Running" initially, can't update to "Completed"
    ...
    try:
        ...
        # When SKYNET_STREAM=false and this is the first display (not a duplicate),
