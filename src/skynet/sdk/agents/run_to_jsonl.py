...
from skynet.util import get_active_time, get_idle_time
            ...
            if i + 1 < len(records) and "choices" in records[i + 1]:
                ...
                if "message" in choice:
                    ...
                    is_duplicate = any(
                        ...
                        and str(msg.get("tool_calls", [])) == str(response_msg.get("tool_calls", []))
                        for msg in model_messages
                    )
                    if not is_duplicate:
    ...
    for i, msg in enumerate(messages):
        ...
        if msg.get("role") == "user":
            ...
            for existing_msg in unique_messages:
                if existing_msg.get("role") == "user" and existing_msg.get("content") == msg.get("content"):
                    ...
                    for k in range(existing_idx + 1, len(unique_messages)):
                        if unique_messages[k].get("role") == "assistant" and unique_messages[k].get("agent_name"):
