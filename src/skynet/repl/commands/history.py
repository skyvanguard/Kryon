...
class HistoryCommand(Command):
    ...
    def handle_control_panel(self) -> bool:
        ...
        for display_name, agent_info in sorted(agents_to_show.items(), key=lambda x: get_sort_key(x)):
            ...
            memory_indicator = ""
            try:
                from skynet.repl.commands.memory import APPLIED_MEMORY_IDS, COMPACTED_SUMMARIES
            ...
            if is_current:
                branch_text = (
                    f":robot: [bold cyan]{display_name}[/bold cyan] ({msg_count} messages) {status}{memory_indicator}"
                )
            else:
    ...
    def handle_all(self, args: Optional[list[str]] = None) -> bool:
        ...
        for idx, msg in enumerate(all_messages, 1):
            ...
            if role == "tool":
                ...
                formatted_content = (
                    f"[dim]Tool ID: {tool_call_id}[/dim]\n{content[:300] if len(content) > 300 else content}"
                )
            else:
            ...
            table.add_row(str(idx), agent_name, f"[{role_style}]{role}[/{role_style}]", formatted_content)
    ...
    def handle_search(self, args: Optional[list[str]] = None) -> bool:
        ...
        for agent_name, msg_idx, msg in found_messages:
            ...
            highlighted_content = formatted_content.replace(search_term, f"[bold red]{search_term}[/bold red]").replace(
                search_term.capitalize(), f"[bold red]{search_term.capitalize()}[/bold red]"
            )
    ...
    def handle_index(self, args: Optional[list[str]] = None) -> bool:
        ...
        if role_filter:
            filtered_messages = [
                (i, msg) for i, msg in enumerate(history) if msg.get("role", "").lower() == role_filter
            ]
