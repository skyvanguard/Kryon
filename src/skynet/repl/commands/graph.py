...
class GraphCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(name="/graph", description="Visualize the agent interaction graph", aliases=["/g"])
    ...
    def _handle_single_agent_graph(self, agent_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            def render_graph(G):
                ...
                for i, (_idx, data) in enumerate(node_list):
        ...
        for display_name, history in agents_to_show:
            ...
            try:
                ...
                if nodes:
                    ...
                    for i, (_idx, data) in enumerate(nodes):
                        role = data.get("role", "unknown")
                        turn_number = data.get("turn_number", 0)
    ...
    def handle_timeline(self, args: Optional[list[str]] = None) -> bool:
        ...
        for event in timeline_events:
            ...
            if event["role"] == "user":
                action = (
                    f"User: {event['content'][:80]}..." if len(event["content"]) > 80 else f"User: {event['content']}"
                )
            ...
            table.add_row(time_str, agent_str, f"[{role_style}]{event['role']}[/{role_style}]", action)
    ...
    def handle_stats(self, args: Optional[list[str]] = None) -> bool:
        ...
        for display_name, history in sorted(all_histories.items()):
            ...
            total_tool_calls = sum(len(msg.get("tool_calls", [])) for msg in history if msg.get("role") == "assistant")

            # Calculate average message length
            content_lengths = [len(str(msg.get("content", ""))) for msg in history if msg.get("content")]
