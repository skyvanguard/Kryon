...
from skynet.repl.commands.base import COMMAND_ALIASES, COMMANDS
from skynet.repl.commands.model import get_predefined_model_names
...
class FuzzyCommandCompleter(Completer):
    ...
    def get_command_suggestions(self, current_word: str) -> list[Completion]:
        ...
        if (
            ...
            and current_time - self._command_suggestions_last_update < self._command_suggestions_update_interval
        ):
            ...
            if alias.startswith(current_word):
                suggestions.append(
                    Completion(
                        alias,
                        start_position=-len(current_word),
                        display=HTML(f"<ansigreen><b>{alias:<15}</b></ansigreen> {cmd} - {cmd_description}"),
                ...
                suggestions.append(
                    Completion(
                        alias,
                        start_position=-len(current_word),
                        display=HTML(f"<ansigreen>{alias:<15}</ansigreen> {cmd} - {cmd_description}"),
                        style="fg:ansigreen",
                    )
                )
    ...
    def get_command_shadow(self, text: str) -> Optional[str]:
        ...
        if (
            ...
            and current_time - self._command_shadow_last_update < self._command_shadow_update_interval
        ):
    ...
    def get_subcommand_suggestions(self, cmd: str, current_word: str) -> list[Completion]:
        ...
        if (
            ...
            and current_time - self._subcommand_suggestions_last_update < self._subcommand_suggestions_update_interval
        ):
                ...
                if subcmd.startswith(current_word):
                    suggestions.append(
                        Completion(
                            subcmd,
                            start_position=-len(current_word),
                            display=HTML(f"<ansiyellow><b>{subcmd:<15}</b></ansiyellow> {subcmd_description}"),
                ...
                # Fuzzy match
                    ...
                    suggestions.append(
                        Completion(
                            subcmd,
                            start_position=-len(current_word),
                            display=HTML(f"<ansiyellow>{subcmd:<15}</ansiyellow> {subcmd_description}"),
                            style="fg:ansiyellow",
                        )
                    )
    ...
    def get_model_suggestions(self, current_word: str) -> list[Completion]:
        ...
        for model in self._cached_models:
            ...
            elif current_word.lower() in model_name.lower() and not model_name.startswith(current_word):
                suggestions.append(
                    Completion(
                        model_name,
                        start_position=-len(current_word),
    ...
    def get_agent_suggestions(self, current_word: str) -> list[Completion]:
        ...
        for agent_key in self._cached_agents:
            ...
            elif current_word.lower() in agent_key.lower() and not agent_key.startswith(current_word):
                suggestions.append(
                    Completion(
                        agent_key,
                        start_position=-len(current_word),
    ...
    def get_mcp_server_suggestions(self, current_word: str) -> list[Completion]:
        ...
        try:
            ...
            from skynet.repl.commands.mcp import _GLOBAL_MCP_SERVERS
                ...
                if server_name.startswith(current_word):
                    suggestions.append(
                        Completion(
                            ...
                            start_position=-len(current_word),
                            display=HTML(
                                f"<ansicyan><b>{server_name}</b></ansicyan> <ansiwhite>({server_type})</ansiwhite>"
                            ),
                            style="fg:ansicyan bold",
                        )
                    )
                # Fuzzy match
                elif current_word.lower() in server_name.lower() and not server_name.startswith(current_word):
                    suggestions.append(
                        Completion(
                            server_name,
                            start_position=-len(current_word),
                            display=HTML(f"<ansicyan>{server_name}</ansicyan> <ansiwhite>({server_type})</ansiwhite>"),
                            style="fg:ansicyan",
                        )
                    )
                        ...
                        if transport.startswith(current_word):
                            suggestions.append(
                                Completion(
                                    ...
                                    start_position=-len(current_word),
                                    display=HTML(
                                        f"<ansiyellow><b>{transport}</b></ansiyellow> <ansiwhite>- {desc}</ansiwhite>"
                                    ),
                                    style="fg:ansiyellow bold",
                                )
                            )
