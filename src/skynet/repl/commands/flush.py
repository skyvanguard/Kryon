...
class FlushCommand(Command):
    ...
    def handle_all(self, args: Optional[list[str]] = None) -> bool:
        ...
        if PARALLEL_ISOLATION.is_parallel_mode():
            for _agent_id, history in PARALLEL_ISOLATION._isolated_histories.items():
                if history:
                    agent_count += 1
        ...
        for _key, model_ref in list(ACTIVE_MODEL_INSTANCES.items()):
    ...
    def show_flush_help(self) -> bool:
        ...
        try:
            from skynet.sdk.agents.models.openai_chatcompletions import get_all_agent_histories
        except ImportError:
            console.print("[red]Error: Could not access conversation history[/red]")
            return False
