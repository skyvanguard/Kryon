...
from skynet.util import COST_TRACKER, get_ollama_api_base
...
class ModelCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(name="/model", description="View or change the current LLM model", aliases=["/mod"])
    ...
    def handle_model_command(self, args: list[str]) -> bool:
        ...
        self.cached_model_numbers = {str(i): model_name for i, model_name in enumerate(self.cached_models, 1)}
        ...
        if not args:  # pylint: disable=too-many-nested-blocks
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for i, model in enumerate(ollama_models, start_index):
                        ...
                        model_table.add_row(str(i), model_name, "Ollama", "Local", "Free", "Free", model_description)
            ...
            console.print("  [bold]/model-show[/bold]         - Show all available models from LiteLLM repository")
            return True
    ...
    def handle(self, args: Optional[list[str]] = None) -> bool:  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,line-too-long # noqa: E501
        ...
        try:
            ...
            try:
                ...
                console.print("[dim]Ollama models not available[/dim]", style="dim")
            ...
            console.print("  [bold]/model-show supported[/bold]      - Show only models with function calling")
            console.print("  [bold]/model-show <search>[/bold]       - Filter models by search term")
            console.print("  [bold]/model-show supported <search>[/bold] - Filter supported models by search term")
            console.print("  [bold]/model <model_name>[/bold]        - Select a model to use")
            console.print("  [bold]/model <number>[/bold]            - Select a model by its number")

            # Data source attribution
            data_source = "https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json"
