...
class WorkspaceCommand(Command):
    ...
    def handle_get(self, _: Optional[list[str]] = None) -> bool:
        ...
        if active_container:
            try:
                ...
                if result.returncode == 0:
                    ...
                    try:
                        from skynet.tools.common import (
                ...
                # Use common._get_workspace_dir() for consistency
                try:
                    from skynet.tools.common import _get_workspace_dir as get_common_workspace_dir
        ...
        console.print("  [bold]/workspace exec <cmd>[/bold]      - Execute a command in the workspace")

        if active_container:
            console.print("  [bold]/workspace copy <src> <dst>[/bold] - Copy files between host and container")
    ...
    def handle_set(self, args: Optional[list[str]] = None) -> bool:
        ...
        if not all(c.isalnum() or c in ["_", "-"] for c in workspace_name):
            console.print("[red]Invalid workspace name. Use alphanumeric, underscores, or hyphens only.[/red]")
            return False

        # Import the necessary modules for setting environment variables
        # And for getting workspace dir consistently
        try:
            from skynet.repl.commands.config import set_env_var
            from skynet.tools.common import (
    ...
    def _list_host_files(self, workspace_dir: str) -> None:
        ...
        try:
            result = subprocess.run(["ls", "-la", workspace_dir], capture_output=True, text=True, check=False)
