...
# Local imports
from skynet.agents import get_agent_module, get_available_agents
from skynet.repl.commands.base import Command, register_command
from skynet.sdk.agents import Agent
from skynet.util import visualize_agent_graph
...
class AgentCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(name="/agent", description="Manage and switch between agents", aliases=["/a"])
    ...
    def handle_select(self, args: Optional[list[str]] = None) -> bool:  # pylint: disable=too-many-branches,line-too-long # noqa: E501
            ...
            if hasattr(pattern, "type"):
                ...
                if pattern_type_str == "parallel":
                    ...
                    if hasattr(AGENT_MANAGER, "_pending_history_transfer") and AGENT_MANAGER._pending_history_transfer:
                        ...
                        for _agent_name, hist in AGENT_MANAGER._message_history.items():
                            if hist:
        ...
        from skynet.repl.commands.parallel import PARALLEL_AGENT_INSTANCES, PARALLEL_CONFIGS
        from skynet.sdk.agents.simple_agent_manager import AGENT_MANAGER
        ...
        if PARALLEL_CONFIGS:
            ...
            if hasattr(AGENT_MANAGER, "_pending_history_transfer") and AGENT_MANAGER._pending_history_transfer:
                ...
                if not current_history:
                    for _display_name, hist in all_histories.items():
                        if hist:
                            current_history = hist
                            break
        ...
        if len(instructions) > 500:
            ...
            console.print("[dim italic](Truncated for display - full prompt used by agent)[/dim italic]")
        else:
    ...
    def handle_current(self, args: Optional[list[str]] = None) -> bool:
        ...
        if parallel_enabled and has_parallel_configs:
            ...
            if current_agent_key not in agents_to_display:
                ...
                console.print(f"[yellow]Available agents: {', '.join(agents_to_display.keys())}[/yellow]")
                return False
