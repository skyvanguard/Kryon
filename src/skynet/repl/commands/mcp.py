...
from skynet.sdk.agents.mcp import (
    MCPServer,
    MCPServerSse,
    MCPServerSseParams,
    MCPServerStdio,
...
class GlobalMCPUtil(MCPUtil):
    ...
    def to_function_tool(cls, tool, server_name: str) -> FunctionTool:
        ...
        # For STDIO servers, capture the command
            ...
            server_config["encoding_error_handler"] = getattr(server.params, "encoding_error_handler", "strict")
        ...
        async def invoke_with_fresh_connection(config, context, input_json):
            ...
            from skynet.sdk.agents.exceptions import AgentsException, ModelBehaviorError
            from skynet.sdk.agents.mcp import MCPServerSse, MCPServerStdio

            try:
                json_data = json.loads(input_json) if input_json else {}
            except Exception as e:
                raise ModelBehaviorError(f"Invalid JSON input for tool {config['tool_name']}: {input_json}") from e
            ...
            from skynet.sdk.agents.tracing import FunctionSpanData, get_current_span
        ...
        if _GLOBAL_MCP_SERVERS:
            ...
            with warnings.catch_warnings():
                ...
                async def cleanup_all():
                    tasks = []
                    for _name, server in _GLOBAL_MCP_SERVERS.items():
                        try:
...
"""
    ...
    def _load_stdio_server(self, name: str, command: str, cmd_args: list[str]) -> bool:
        ...
        console.print(f"Starting stdio server '{name}' with command: {command} {' '.join(cmd_args)}")
    ...
    def handle_list(self, args: Optional[list[str]] = None) -> bool:
        ...
        for name, server in _GLOBAL_MCP_SERVERS.items():
            ...
            try:
                ...
                async def get_tools(server=server):
    ...
    def handle_add(self, args: Optional[list[str]] = None) -> bool:
        ...
        console.print(f"Adding tools from MCP server '{server_name}' to agent '{agent_display_name}'...")
        ...
        try:
            ...
            async def validate_connection():
                try:
                    ...
                    console.print("[yellow]Warning: Server connection may be lost, attempting to reconnect...[/yellow]")
                    # Try to reconnect
                    await server.connect()
                    tools = await server.list_tools()
        ...
        try:
            ...
            console.print(
                f"[green]Added {len(tools)} tools from server '{server_name}' to agent '{agent_display_name}'.[/green]"
            )
            ...
            async def test_agent_tools():
                ...
                all_mcp_tools = (
                    await agent.get_mcp_tools() if hasattr(agent, "mcp_servers") and agent.mcp_servers else []
                )
                return all_regular_tools + all_mcp_tools

            self._run_async(test_agent_tools())
    ...
    def handle_status(self, args: Optional[list[str]] = None) -> bool:
        ...
        for name, server in _GLOBAL_MCP_SERVERS.items():
            ...
            try:
                ...
                async def test_connection(server=server):
                    tools = await server.list_tools()
                ...
                try:
                    ...
                    async def reconnect(server=server):
                        await server.connect()
                        tools = await server.list_tools()
                        return len(tools)
