...
from skynet.sdk.agents.run_to_jsonl import load_history_from_jsonl
...
class LoadCommand(Command):
        ...
        try:
                ...
                if agent_counts[config.agent_name] > 1:
                    ...
                    if best_match:
                        ...
                        if PARALLEL_ISOLATION.is_parallel_mode():
                            ...
                            PARALLEL_ISOLATION.get_isolated_history(agent_id)
            ...
            if loaded_count > 0:
                ...
                if not agent_conversations:
                    ...
                    for agent, messages in sorted(agent_conversations.items(), key=lambda x: len(x[1]), reverse=True)[
                        :5
                    ]:
    ...
    def handle_load_default(self, jsonl_file: Optional[str] = None) -> bool:
        ...
        try:
            ...
            from skynet.repl.commands.parallel import ParallelCommand
            from skynet.sdk.agents.models.openai_chatcompletions import (
                ACTIVE_MODEL_INSTANCES,
                PERSISTENT_MESSAGE_HISTORIES,
            )
            ...
            for agent_name, original_history in all_histories.items():
                ...
                # Find the matching model instance
                model_instance = None
                for (model_agent_name, _inst_id), model_ref in ACTIVE_MODEL_INSTANCES.items():
    ...
    def handle_all(self, args: Optional[list[str]] = None) -> bool:
        ...
        agent_ids = {}
        for agent_name, _history in all_histories.items():
